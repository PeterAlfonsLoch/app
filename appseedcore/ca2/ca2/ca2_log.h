...
namespace ca2
{
   ...
   public:
      ...
      void trace_v(const char *pszFileName, int nLine, unsigned int dwCategory, unsigned int nLevel, const char * pszFmt, va_list args) const;

      void __cdecl trace(const char * pszFormat, ...);
      void __cdecl trace2(DWORD_PTR dwCategory, UINT nLevel, const char * pszFormat, ...);

      void set_trace_category(unsigned int dwCategory, const char * pszName, unsigned int uiLevel);
