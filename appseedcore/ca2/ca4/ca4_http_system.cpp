   ...
   namespace http
   {
      ...
      void system::auto_config_proxy(int i)
      {
         ...
         else 
         {
            ...
            System.file().put_contents(System.dir().appdata("proxy.xml"), "proxy");
         }
      }
      ...
      void system::defer_auto_initialize_proxy_configuration()
      {
         ...
         if(!strcmp(System.get_module_name(), "netnodeapp")
            ...
            || System.get_license_id() == "netnodecfg"
            || System.get_license_id() == "netnode"
            || System.command_line().m_varQuery["app"] == "netnodecfg")
         {
            return;
         }
         else if(System.command_line().m_varQuery["app"] == "mydns"
         ...
         bool bOk = false;
         for(int i = 0; i < auto_config_proxy_count(); i++)
         {
            auto_config_proxy(i);
            string str = get("https://fontopus.com/hello");
            if(str == "Hello!!")
            {
               bOk = true;
               break;
                     ...
                     e_status * pestatus)
      {
         ...
         {
            strUrl = System.url().set(strUrl, "sessid", puser->m_strSessid);
         }

         ::sockets::http_client_socket * psocket;

         bool bPost;
         if(post.m_propertya.get_count() > 0)
         {
            bPost = true;
         ...
         if(puser != NULL && puser->m_phttpcookies != NULL)
         {
         ...
         while(handler.get_count() > 0)
         {
            ...
            {
               if(psocket->m_content_length > set["file_out"].ca2 < ::ex1::timeout_file >()->m_uiExpectedSize 
                  || set["file_out"].ca2 < ::ex1::timeout_file >()->m_uiExpectedSize == ((DWORD_PTR)-1))
               {
      ...
      bool system::get(const char * pszUrl, primitive::memory & storage, ::fontopus::user * puser)
      {
         class signal signal;
      ...
      void system::set_proxy_auth(::fontopus::user * puser, ::sockets::http_client_socket * psocket)
      {
         ...
         bool bOk = true;
         if(!System.crypt().file_get(strUserNameFile, strUserName, NULL)
         || strUserName.is_empty())
         {
            bOk = false;
         }
         if(!System.crypt().file_get(strPasswordFile, strPassword, NULL)
         ...
         else
         {
            ...
            {
               System.crypt().file_set(strUserNameFile, strUserName, NULL);
               System.crypt().file_set(strPasswordFile, strPassword, NULL);
               psocket->m_strUserNameFile = strUserNameFile;
               psocket->m_strPasswordFile = strPasswordFile;
            }
         }
