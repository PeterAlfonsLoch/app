...
#define DECLARE_SE_EXCEPTION_CLASS(name) class CLASS_DECL_ca name : public se_exception \
	...
	protected: \
   name (::ca::application * papp, EXCEPTION_POINTERS * ppointers) : ::ca::ca(papp), se_exception(papp, ppointers) {} \
	};
...
protected:
   se_exception(::ca::application * papp, EXCEPTION_POINTERS * ppointers) : ::ca::ca(papp), m_ppointers(ppointers) { _ASSERTE(ppointers != 0); }	
   se_exception(const se_exception& se) : ::ca::ca(((se_exception &) se).get_app()), m_ppointers(se.m_ppointers) {}
...
class se_access_violation : public se_exception
{
   friend class se_translator;
protected:
   se_access_violation (::ca::application * papp, EXCEPTION_POINTERS * ppointers) : ::ca::ca(papp), se_exception(papp, ppointers) {}
public:
...
class se_no_memory : public se_exception
{
   friend class se_translator;
protected:
   se_no_memory (::ca::application * papp, EXCEPTION_POINTERS * ppointers) : ::ca::ca(papp), se_exception(papp, ppointers) {}
public:		 
