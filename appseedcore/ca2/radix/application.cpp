   ...
   bool application::check_exclusive()
   {
      ...
      if ( bInitOk )
      {
         ...
         if ( bSetOk )
         {
            ...
            if(m_eexclusiveinstance == ExclusiveInstanceLocal
               && ::GetLastError() == ERROR_ALREADY_EXISTS)
            {
               // Should in some way activate the other instance
               //System.simple_message_box("A instance of the application:<br><br>           - " + string(m_strAppName) + "<br><br>seems to be already running at the same account.<br>Only one instance of this application can run locally: at the same account.<br><br>Exiting this new instance.");
...
//               TRACE(::radix::trace::category_AppMsg, 0, "Error: illegal string index requested %d.\n", i);
            ...
            else if (rglpsz[i] != NULL)
            {
               ...
               _template::checked::tcscpy_s(pchDest, nTotalLen + 1, rglpsz[i]);
               nTotalLen -= nLen;
               pchDest += nLen;
            }
         }
   ...
   {
      m_pdocmanager->request(varFile, varQuery);
   }
...
#include "StdAfx.h"
   ...
   var & application::oprop(::radix::object * pobject, id id)
   {
      CSingleLock sl(&m_mutexObjectVar, TRUE);
      return m_mapObjectVar[pobject][id];
   }
   ...
   bool application::activate_app()
   {
      ...
      ::user::keyboard_focus * application::get_keyboard_focus()
      {
         if(is_system())
         {
            ...
            if(m_pkeyboardfocus->get_wnd() == get_focus_guie()
            || (m_pkeyboardfocus->get_wnd() != NULL
             && m_pkeyboardfocus->get_wnd()->m_pguie != NULL
             && m_pkeyboardfocus->get_wnd()->m_pguie->get_os_focus_uie() != NULL 
             && m_pkeyboardfocus->get_wnd()->m_pguie->get_os_focus_uie() == get_focus_guie()))
            {
            ...
            return System.get_keyboard_focus();
      }
      ...
      void application::set_keyboard_focus(::user::keyboard_focus * pkeyboardfocus)
      {
         if(is_system())
         {
            ...
            System.set_keyboard_focus(pkeyboardfocus);
      }
   ...
   */
   ...
   void application::on_delete(::ca::object * pobject)
   {
      try
      {
         ::radix::object * pobj = dynamic_cast < ::radix::object * >(pobject);
         if(pobj != NULL)
         {
            mutex * pmutex = get_mutex(pobj);
            if(pmutex != NULL)
            {
               CSingleLock sl(&m_mutexObjectLock, TRUE);
               delete pmutex;
   ...
   mutex * application::get_mutex(::radix::object * pobject)
   {
      CSingleLock sl(&m_mutexObjectLock, TRUE);
      mutex * pmutex;
   ...
   bool application::lock(::radix::object * pobject, DWORD dwTimeout)
   {
      mutex * pmutex = get_mutex(pobject);
      if(pmutex == NULL)
         return false;
      return pmutex->Lock(dwTimeout) != FALSE;
   }

   bool application::unlock(::radix::object * pobject)
   {
      mutex * pmutex = get_mutex(pobject);
      if(pmutex == NULL)
         return false;
   ...
   CEvent * application::get_event(::radix::object * pobject, int iEvent)
   {
      CSingleLock sl(&m_mutexObjectEvent, TRUE);
      ::collection::map < int, int, CEvent *, CEvent * > * peventmap;
      if(!m_mapObjectEvent.Lookup(pobject, peventmap))
      {
         peventmap = new ::collection::map < int, int, CEvent *, CEvent * >();
   ...
   bool application::event_lock(::radix::object * pobject, int iEvent, DWORD dwTimeout)
   {
      CEvent * pevent = get_event(pobject, iEvent);
      if(pevent == NULL)
         return false;
      return pevent->Lock(dwTimeout) != FALSE;
   }
