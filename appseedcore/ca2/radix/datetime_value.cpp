...
namespace datetime
{
   ...
   value span_strtotime(::ca::application * pcaapp, ::user::str_context * pcontext, const char * pszSpanExpression)
   {
      ...
      for(int i = 0;*psz; psz = gen::str::utf8_inc(psz))
      {
         ...
         else if(strChar == "+")
         {
            if(bAdd)
            {
               pcaapp->m_psystem->log().trace("strtotime: invalid char +");
            }
            else if(bMinus)
            {
               pcaapp->m_psystem->log().trace("strtotime: invalid char + on Minus state");
            }
            bAdd = true;
            bMinus = false;
            strNumber.Empty();
         ...
         else if(strChar == "-")
         {
            if(bAdd)
            {
               pcaapp->m_psystem->log().trace("strtotime: invalid char - on add state");
            }
            else if(bMinus)
            {
               pcaapp->m_psystem->log().trace("strtotime: invalid char - on Minus state");
   ...
   value strtotime(::ca::application * pcaapp, ::user::str_context * pcontext, const char * psz, int & iPath, int & iPathCount)
   {
      ...
      if(str.get_length() >= 19)
      {
         ...
         {
            bBaseTime = true;
            pcaapp->m_psystem->datetime().international().parse_str(str, set);
      ...
      if(!bBaseTime && PcreUtil::match(stra, str, "/^\\s*((\\d+)\\s*/\\s*(\\d+))((\\d|$)?!)/", true, 4) > 0)
      {
         ...
         && i1 >= 1 && i1 <= 12 
         && i2 >= 1 && i2 <= 
            pcaapp->m_psystem->datetime().get_month_day_count(time.GetYear(), i1))
         {
            bFirst = true;
            iCount++;
         }
         if(i2 >= 1 && i2 <= 12 
         && i1 >= 1 && i1 <=
            pcaapp->m_psystem->datetime().get_month_day_count(time.GetYear(), i2))
         {
            iCount++;
         }
         if(iCount > 0)
   ...
   string value::to_string(::ca::application * pcaapp, ::user::str_context * pcontext)
   {
      ...
      else
      {
         ...
         if(time.GetSecond() == 0)
         {
            ...
            {
               str = time.Format("%Y-");
               pcaapp->m_psystem->datetime().get_month_str(pcontext, time.GetMonth());
               str += time.Format("-%d");
            }
            else
            {
               str = time.Format("%Y-");
               str += pcaapp->m_psystem->datetime().get_month_str(pcontext, time.GetMonth());
