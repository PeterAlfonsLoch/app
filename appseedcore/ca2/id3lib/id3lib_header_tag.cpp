...
void ID3_TagHeader::to(ID3_Writer& writer) const
{
  ...
  writer.writeChar(static_cast<uchar>(_flags.get() & ID3LIB_MASK8));
  io::writeUInt28(writer, this->GetDataSize()); //now includes the extended header

  // now we to the extended header
...
*/
    ...
    const int extflagbytes = 0;
    reader.read((char *) &extflagbytes); //Number of flag bytes
    ...
    if (extflags[0]->test(EXT_HEADER_FLAG_BIT2))
    {
      ...
      const int extheaderflagdatasize = 0;
         reader.read((char *) &extheaderflagdatasize);
