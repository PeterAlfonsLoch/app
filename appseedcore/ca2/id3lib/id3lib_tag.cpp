...
size_t ID3_Tag::Parse(const uchar header[ID3_TAGHEADERSIZE], const uchar *buffer)
{
  ...
  memcpy(buf.GetAllocation(),reinterpret_cast<const char *>(header),
             ID3_TagHeader::SIZE);
  memcpy(&buf.GetAllocation()[ID3_TagHeader::SIZE],reinterpret_cast<const char *>(buffer), size);
  return this->Parse(buf.GetAllocation(), buf.GetStorageSize());
}
...
size_t ID3_Tag::to(uchar* buffer, ID3_TagType tt) const
{
  ...
  return this->to(mw, tt);
}

size_t ID3_Tag::to(ID3_Writer& writer, ID3_TagType tt) const
{
