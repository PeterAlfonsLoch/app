                               ...
                               string lang)
{
  ...
  for(index i = 0; i < tag.NumFrames(); i++)
  {
    frame = tag.frames().ptr_at(i);
    if (frame == NULL)
    {
      continue;
    }
...
size_t id3::v2::removeComments(ID3_TagImpl& tag, string desc)
{
  ...
  for(index i = 0; i < tag.NumFrames(); i++)
  {
    ID3_Frame* frame = tag.frames().ptr_at(i);
    if (frame == NULL)
    {
      continue;
    }
...
{
  ID3_Frame* frame = NULL;
  string track = gen::str::itoa((size_t)trk);
  if (ttl > 0)
  {
    track += "/";
    track += gen::str::itoa((size_t)ttl);
  }
...
size_t id3::v2::getGenreNum(const ID3_TagImpl& tag)
{
  ...
  size_t ulGenre = 0xFF;
  size_t size = sGenre.get_length();

  // If the genre string begins with "(ddd)", where "ddd" is a number, then
  // "ddd" is the genre number---get it
  size_t i = 0;
  if (i < size && size && sGenre.operator[](i) == '(')
  {
    ++i;
    while (i < size && isdigit(sGenre.operator[](i)))
    {
      ++i;
    }
    if (i < size && sGenre.operator[](i) == ')')
    {
      // if the genre number is greater than 255, its invalid.
      ulGenre = min(0xFF, atoi(&*(const char *)sGenre[1]));
    }
  }
...
ID3_Frame* id3::v2::setSyncLyrics(ID3_TagImpl& tag, primitive::memory data,
                                  ...
                                  string lang, ID3_ContentType type)
{
  ...
  frame->GetField(ID3FN_DATA).Set(data.GetAllocation(), data.GetStorageSize());

  return frame;
}

primitive::memory id3::v2::getSyncLyrics(const ID3_TagImpl& tag, string lang, string desc)
{
  ...
  return primitive::memory(reinterpret_cast<const char *>(fld->GetRawBinary()), fld->size());
}
