...
bool ID3_FrameHeader::Parse(ID3_Reader& reader)
{
  ...
  ID3_FrameID fid = ID3_FindFrameID(textID);
  if (ID3FID_NOFRAME == fid)
  {
    this->SetUnknownFrame(textID);
  ...
  _flags.add((ID3_Flags::TYPE) flags);
...
void ID3_FrameHeader::to(ID3_Writer& writer) const
{
  //size_t size = 0;
  ...
  {
    // TODO: log this
    ID3D_WARNING( "ID3_FrameHeader::to(): _frame_def is NULL!" );
    return;
  ...
  ID3D_NOTICE( "ID3_FrameHeader::to(): writing " << textID << ", " << (int) _info->frame_bytes_size << " bytes");
