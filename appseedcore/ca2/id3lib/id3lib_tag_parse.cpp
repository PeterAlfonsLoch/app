  ...
  bool parseFrames(ID3_TagImpl& tag, ID3_Reader& rdr)
  {
    ...
    while (rdr.peek(&ch) && ch != '\0')
    {
      ...
      else
      {
        ...
        if (fld)
        {
          ID3_MemoryReader mr((char *) fld->GetRawBinary(), fld->BinSize());
          char ch;
          mr.read(&ch);
          if (ch != 'z')
          {
...
//            size_t oldSize = f->GetDataSize() - sizeof(uint32) - 1;
            ...
            if (!cr.atEnd())
            {
              ...
              ID3D_WARNING( "id3::v2::parseFrames(): didn't parse entire " <<
                            "id3v2.2.1 compressed primitive::memory stream");
            }
          }
        }
        delete f;
...
bool id3::v2::parse(ID3_TagImpl& tag, ID3_Reader& reader)
{
  ...
  else
  {
    ...
    tag.SetUnsync(true);
    primitive::memory raw = io::readAllBinary(wr);
    ...
    primitive::memory synced = io::readAllBinary(ur);
...
void ID3_TagImpl::ParseReader(ID3_Reader &reader)
{
  ...
  if (wr.peek(&ch) && ch == '\0')
  {
    ...
    do
    {
      ...
      wr.setBeg(cur);
      wr.setCur(cur);
    } while (!wr.atEnd() &&  cur > last && wr.peek(&ch) && ch == '\0');
  }
  if (!wr.atEnd() && _file_size - (cur - beg) > 4 && wr.peek(&ch) && ch == 255)
  { //unfortunatly, this is necessary for finding an invalid padding
    wr.setCur(cur + 1); //cur is known by peek
    if (wr.read(& ch) && ch == '\0' && wr.read(&ch) && ch == '\0' && wr.peek(&ch) && ch == '\0')
    { //three is_empty bytes found, enough for me, this is stupid padding
      cur += 3; //those are now allready read in (excluding the peek, since it will be added by do{})
      do
      {
        last = cur;
        cur = wr.getCur() + 1;
  ...
  beg  = wr.getBeg();
  if (!wr.atEnd() && wr.peek(&ch) && ch != -1) //no sync byte, so, either this is not followed by a mp3 file or it's a fLaC file, or an encapsulating format, better check it
  {
    ...
    { //there is room to search for some kind of ID
      ...
      if (strncmp((char*)buf, "RIFF", 4) == 0 || strncmp((char*)buf, "RIFX", 4) == 0)
      {
        ...
        if (!wr.atEnd() && wr.peek(&ch) && ch != -1)
        {
          do
          {
            ...
            cur = wr.getCur() + 1;
            wr.setCur(cur);
          } while (!wr.atEnd() &&  cur > last && wr.peek(&ch) && ch != -1);
        }
      }
      ...
      { //since we set the cursor 4 bytes ahead for looking for RIFF, RIFX or fLaC, better set it back
        // but peek allready checked the first one, so we add one
        cur = cur + 1;
        wr.setCur(cur);
        //go looking for a sync byte
        if (!wr.atEnd() && wr.peek(&ch) && ch != -1) //no sync byte, we have an unknown byte
        {
          do
          {
            last = cur;
  ...
  if (_file_size > _prepended_bytes)
  {
    ...
    this->SetPadding(false); //no need to pad an is_empty file
}
