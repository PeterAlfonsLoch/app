 ...
 **/
size_t ID3_FieldImpl::SetBinary(primitive::memory data) //< data to assign to this field.
{
  size_t size = 0;
  if (this->GetType() == ID3FTY_BINARY)
  {
    ...
    {
      _binary.assign(data.GetAllocation(), 0, min(size, fixed));
      if (size < fixed)
      {
...
primitive::memory ID3_FieldImpl::GetBinary() const
{
  primitive::memory data;
  if (this->GetType() == ID3FTY_BINARY)
  {
    data = _binary;
  }
                          ...
                          ) const
{
  ...
  if (this->GetType() == ID3FTY_BINARY)
  {
    ...
    {
      ::memcpy(buffer, _binary.GetAllocation(), bytes);
    }
  }
  return bytes;
}
                           ...
                           ) const
{
  ...
  if (size > 0)
  {
    ...
    if (temp_file != NULL)
    {
      ::fwrite(_binary.GetAllocation(), 1, size, temp_file);
      ::fclose(temp_file);
    }
  }
