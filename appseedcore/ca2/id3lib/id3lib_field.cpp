...
/** \class ID3_Field field.h field.h
 ...
 **
 ** A Unicode field also supports get(wchar_t*, size_t, size_t),
 ** Set(const wchar_t*), add(const wchar_t*), and
 ** operator=(const wchar_t*).  Without elaborating, the Unicode
 ** methods behave exactly the same as their ASCII counterparts, taking
 ** \c wchar_t pointers in place of \c char pointers.
 **
 ...
 ** an ASCII string, then try to read the string into a \c wchar_t buffer,
 ...
 ** Unicode string contains characters that don't ::collection::map to ISO-8859-1.
 **
...
/** Clears any data and frees any primitive::memory associated with the field
 **
 ** \sa ID3_Tag::clear()
 ** \sa ID3_Frame::clear()
 **/
 ...
 ** get(wchar_t*, size_t, size_t) method anyway.  For binary and
...
size_t ID3_FieldImpl::size() const
{
  ...
  {
     size = 0;
     for(int i = 0; i < m_stra.get_count(); i++)
     {
        size += m_stra[i].get_length();
        size++;
     }
    if(size > 0)
       size--;
...
void ID3_FieldImpl::to(ID3_Writer& writer) const
{
  switch (this->GetType())
  {
    case ID3FTY_INTEGER:
    ...
    default:
    {
      ID3D_WARNING ( "ID3D_FieldImpl::to(): unknown field type" );
      break;
    }
  }
}
...
/** \class ID3_FrameInfo field.h field.h
