...
size_t RenderV1ToFile(ID3_TagImpl& tag, ex1::file& file)
{
  if (!file)
  {
    return 0;
  ...
  else
  {
    ...
    file.seek(0-ID3_V1_LEN, ::ex1::seek_end);
...
size_t RenderV2ToFile(const ID3_TagImpl& tag, ex1::file & file)
{
  ...
  id3::v2::to(writer, tag);
  ID3D_NOTICE( "RenderV2ToFile: rendered v2" );

  const char* tagData = tagstring;
  size_t tagSize = tagstring.get_length();
  ...
  else
  {
    ...
    if (filename.get_length() + sTmpSuffix.get_length() > ID3_PATH_LENGTH)
    {
      // log this
      return 0;
    ...
    tmpOut.write(tagData, tagSize);
    file.seekg(tag.GetPrependedBytes(), std::ios::beg);
    ...
    if(stat(filename, &fileStat) == 0)
    {
      ...
      remove(filename);
      rename(sTempFile, filename);
#if defined(HAVE_SYS_STAT_H)
//xxx      _chmod(filename, fileStat.st_mode);
    }
#endif //defined(HAVE_SYS_STAT_H)
...
#endif
      int nBytesRead = file.read((char *)aucBuffer, nBytesToRead);

      if(natural(nBytesRead) != nBytesToRead)
      {
        // TODO: log this
