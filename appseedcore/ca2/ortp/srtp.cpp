...
static int srtp_recvfrom(RtpTransport *t, mblk_t *m, int flags, struct sockaddr *from, socklen_t *fromlen){
   ...
   int err;
   int slen;
   err=recvfrom(t->session->rtp.socket,(char *)m->b_wptr,m->b_datap->db_lim-m->b_datap->db_base,flags,from,fromlen);
   if (err>0){
...
static int srtcp_recvfrom(RtpTransport *t, mblk_t *m, int flags, struct sockaddr *from, socklen_t *fromlen){
   ...
   int err;
   int slen;
   err=recvfrom(t->session->rtcp.socket,(char *)m->b_wptr,m->b_datap->db_lim-m->b_datap->db_base,flags,from,fromlen);
   if (err>0){
      slen=err;
