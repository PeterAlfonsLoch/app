...
{   
   ...
   {
      sdes_item_t si;
      si.item_type=(uint8_t)sdes_type;
...
static void sender_info_init(sender_info_t *info, RtpSession *session){
   ...
   session->rtp.last_rtcp_packet_count=(uint32_t)session->rtp.stats.packet_sent;
}
...
static void report_block_init(report_block_t *b, RtpSession *session){
   ...
   if (stream->hwrcv_seq_at_last_SR!=0){
      ...
      loss_fraction=(uint8_t)(256.0*(float)packet_loss/(float)stream->hwrcv_since_last_SR);
   }
   /* reset them */
