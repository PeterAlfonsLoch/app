 ...
 *   it can be interesting to set it to a lower value in order to save primitive::memory.
 *
 * @param session a rtp session
 ...
 *   a ::fontopus::user supplied function in charge of processing it. The application can register
 ...
 *   packet concatenation involving copies to be done in ::fontopus::user-space.
...
mblk_t * rtp_session_create_packet_with_data(RtpSession *session, uint8_t *payload, int payload_size, void (*freefn)(void *))
{
   ...
   rtp_header_init_from_session(rtp,session);
   mp->b_wptr+=header_size;
   /* create a mblk_t around the ::fontopus::user supplied payload buffer */
 ...
 *   and the ::fontopus::user timestamp given here.
 *
...
rtp_session_recvm_with_ts (RtpSession * session, uint32_t user_ts)
{
   ...
   /*calculate the stream timestamp from the ::fontopus::user timestamp */
 ...
 *   where the ::fontopus::user supplied buffer @buffer is not large enough to get all the data 
 ...
 *   of this packet is then copied into the ::fontopus::user supplied buffer in an intelligent manner:
 ...
 *   then the number of bytes written in the ::fontopus::user supplied buffer is returned. If no packets
 ...
 *@param session a rtp session.
 *@param buffer a ::fontopus::user supplied buffer to write the data.
 *@param len the length in bytes of the ::fontopus::user supplied buffer.
...
void rtp_session_set_time_jump_limit(RtpSession *session, int milisecs){
   ...
   if (ts==0)
      session->rtp.ts_jump = (uint32_t) 1<<31;   /* do not detect ts jump */
 ...
 * All primitive::memory allocated for the RtpSession is freed.
 *
 * @param session a rtp session.
**/
...
namespace rtp
{
   ...
   session::session(int mode)
   {
      m_psession = rtp_session_new(mode);
   }


   void session::set_scheduling_mode(int yesno)
   {
      rtp_session_set_scheduling_mode(m_psession, yesno);
   }

   void session::set_blocking_mode(int yesno)
   {
   ...
   int session::set_payload_type(int pt)
   {
      return rtp_session_set_payload_type(m_psession, pt);
   }

   int session::recv_with_ts(void * buffer, int len, uint32_t ts, int *have_more)
   {
      return rtp_session_recv_with_ts(m_psession, (uint8_t *)buffer, len, ts, have_more);
   }

   int session::send_with_ts(const void * buffer, int len, uint32_t userts)
   {
      return rtp_session_send_with_ts(m_psession, (const uint8_t *) buffer, len, userts);
   }




   void session::init(int mode)
   {
      rtp_session_init(m_psession, mode);
   }

   void session::set_flag(int flag)
   {
      m_psession->flags |= flag;
   }

   void session::unset_flag(int flag)
   {
      m_psession->flags &= ~flag;
   }
