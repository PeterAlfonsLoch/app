...
namespace user
{
   ...
   void control::GetWindowRect(LPRECT lprect)
   {
      ...
      {
         get_form_list()->control_get_window_rect(lprect);
      }
      else
      {
   ...
   void control::GetClientRect(LPRECT lprect)
   {
      if(get_form_list() != NULL)
      {
         get_form_list()->control_get_client_rect(lprect);
      }
      else
      {
   ...
   LRESULT control_view_impl::BaseControlExOnIdleUpdateCmdUI(WPARAM wParam, LPARAM)
   {
      ::user::interaction * pview = GetWnd();
       ...
       if (bVis)
       {
           userbase::frame_window* pTarget = dynamic_cast < userbase::frame_window * > (pview->GetOwner());
           if (pTarget == NULL || !pTarget->IsFrameWnd())
               pTarget = dynamic_cast < userbase::frame_window * > (pview->GetParentFrame());
           if (pTarget != NULL)
