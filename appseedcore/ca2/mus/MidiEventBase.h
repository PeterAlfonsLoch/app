...
class CLASS_DECL_ca MidiEventBase
{
public:
   ...
   virtual MidiEventBase & operator = (const MidiEventBase & eventSrc);
   virtual MidiEventBase & operator = (const MidiEventV008 & eventSrc);

   virtual imedia::position GetDelta() const  = 0;
   virtual void SetDelta(imedia::position tkDelta) = 0;
   virtual imedia::position GetPosition() const = 0;
