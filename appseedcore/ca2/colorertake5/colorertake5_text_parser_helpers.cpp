...
parse_cache *parse_cache::searchLine(int ln, parse_cache **cache)
{
...
{
VTList *newitem;
  if(!node || node->virtualEntryVector.get_size() == 0) return false;
  newitem = new VTList();
  if(last->next){
...
VTList *curvl = 0;
  ...
  for(VTList *vl = last; vl && vl->prev; vl = vl->prev){
    for(int idx = 0; idx < vl->vlist->get_size(); idx++){
