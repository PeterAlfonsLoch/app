...
base_editor::base_editor(::ca::application * papp) :
   ca(papp)
{
   textParser = NULL;
   lrSupport = NULL;
...
PairMatch *base_editor::getPairMatch(int lineNo, int linePos)
{
...
LineRegion *base_editor::getLineRegions(int lno){
  /*
...
void base_editor::modifyEvent(int topLine){
  CLR_TRACE("base_editor", "modifyEvent:%d", topLine);
  if (invalidLine > topLine){
    invalidLine = topLine;
    for(int idx = editorListeners.get_size()-1; idx >= 0; idx--){
      editorListeners.element_at(idx)->modifyEvent(topLine);
    }
  }
}

void base_editor::modifyLineEvent(int line){
  if (invalidLine > line){
    invalidLine = line;
  }
  // changedLine = topLine;!!!
...
void base_editor::visibleTextEvent(int wStart, int wSize){
  CLR_TRACE("base_editor", "visibleTextEvent:%d-%d", wStart, wSize);
  this->wStart = wStart;
  this->wSize = wSize;
}

void base_editor::lineCountEvent(int newLineCount){
  CLR_TRACE("base_editor", "lineCountEvent:%d", newLineCount);
  lineCount = newLineCount;
}


inline int base_editor::getLastVisibleLine(){
  int r1 = (wStart+wSize);
  int r2 = lineCount;
  return ((r1 > r2)?r2:r1)-1;
}

void base_editor::validate(int lno, bool rebuildRegions)
{
