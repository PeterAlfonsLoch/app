 ...
 *
 *	LiteHTMLReader.cpp - LiteHTMLReader implementation
 *	
...
UINT LiteHTMLReader::parseDocument(void)
{
	...
	LiteHTMLTag	oTag;			// tag information
		...
		case _T('&'):
			{
				...
				if (m_bResolveEntities)
					lTemp = LiteHTMLEntityResolver::resolveEntity(&m_lpszBuffer[m_dwBufPos], ch);
 ...
 */
UINT LiteHTMLReader::Read(LPCTSTR lpszString)
{
 ...
 * @author Gurmeet S. Kochar
 */
UINT LiteHTMLReader::ReadFile(HANDLE hFile)
{
	...
	{
		TRACE1("(Error) LiteHTMLReader::Read:"
				" GetFileSize() failed;"
