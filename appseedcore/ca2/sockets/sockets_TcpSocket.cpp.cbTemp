...
namespace sockets
{
   ...
   bool tcp_socket::open(sockets::address& ad,sockets::address& bind_ad,bool skip_socks)
   {
	   ...
	   if (!skip_socks && GetSocks4Host() && GetSocks4Port())
	   {
		   ...
		   {
			   string sockshost;
			   Application.net().l2ip(GetSocks4Host(), sockshost);
			   Handler().LogError(this, "open", 0, "Connecting to socks4 server @ " + sockshost + ":" +
				   gen::str::l2string(GetSocks4Port()), ::gen::log::level::info);
		   }
		   SetSocks4();
		   n = connect(s, sa, sa);
		   SetRemoteAddress(sa);
   ...
   bool tcp_socket::open(const string &host,port_t port)
   {
	   if (IsIpv6())
	   {
		   if (!Handler().ResolverEnabled() || Application.net().isipv6(host) )
		   {
			   in6_addr a;
			   if (!Application.net().u2ip(host, a))
			   {
				   SetCloseAndDelete();
				   return false;
			   }
	   ...
	   if (!Handler().ResolverEnabled() || Application.net().isipv4(host) )
	   {
		   ipaddr_t l;
		   if (!Application.net().u2ip(host,l))
		   {
			   SetCloseAndDelete();
			   return false;
		   }
   ...
   {
	   /* create our context*/
	   static ::collection::string_map < SSL_CTX * > client_contexts;
...
//         const char * pszReason = ERR_reason_error_string(err);
   ...
   void tcp_socket::OnRawData(char * buf_in, size_t len)
   {
      if(m_pcallback != NULL)
      {
         m_pcallback->OnRawData(this, buf_in, len);
      }
   }
   ...
   string tcp_socket::CircularBuffer::ReadString(size_t l)
   {
