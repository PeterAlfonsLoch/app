...
namespace sockets
{
   ...
   int udp_socket::Bind(sockets::address& ad, int range)
   {
	   ...
	   if (GetSocket() != INVALID_SOCKET)
	   {
		   ...
		   if (n == -1)
		   {
			   ...
			   SetCloseAndDelete();
			   throw Exception("bind() failed for udp_socket, port:range: " + ::gen::str::l2string(ad.GetPort()) + ":" + ::gen::str::l2string(range));
			   return -1;
		   }
		   m_bind_ok = true;
		   m_port = ad.GetPort();
   ...
   void udp_socket::AddMulticastMembership(const string & group, const string & local_if, int if_index)
   {
	   ...
	   if (IsIpv6())
	   {
		   ...
		   if (Application.net().u2ip( group, addr ))
		   {
	   ...
	   if (Application.net().u2ip( group, addr ))
	   {
		   ...
		   Application.net().u2ip( local_if, addr);
