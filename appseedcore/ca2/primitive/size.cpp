...
size size::operator+(SIZE size) const throw()
   { return class size(cx + size.cx, cy + size.cy); }
size size::operator-(SIZE size) const throw()
   { return class size(cx - size.cx, cy - size.cy); }
size size::operator-() const throw()
   { return size(-cx, -cy); }
point size::operator+(POINT point) const throw()
   { return class point(cx + point.x, cy + point.y); }
point size::operator-(POINT point) const throw()
   { return class point(cx - point.x, cy - point.y); }
rect size::operator+(const RECT* lpRect) const throw()
...
size64::size64(unsigned __int64 dwSize) throw()
   {
...
size64 size64::operator+(__size64 size64) const throw()
   { return class size64(cx + size64.cx, cy + size64.cy); }
size64 size64::operator-(__size64 size64) const throw()
   { return class size64(cx - size64.cx, cy - size64.cy); }
size64 size64::operator-() const throw()
   { return size64(-cx, -cy); }
point64 size64::operator+(__point64 point64) const throw()
   { return class point64(cx + point64.x, cy + point64.y); }
point64 size64::operator-(__point64 point64) const throw()
   { return class point64(cx - point64.x, cy - point64.y); }
