...
void LiteHTMLElemAttr::putValue(::lite_html_reader * preader, const char * lpszValue)
{
   ...
   do
   {
      ...
      iParseLen = preader->m_papp->m_psystem->html_ex().resolve_entity(m_strValue.Mid(iCurPos), chSubst);
      if (iParseLen)
      {
         m_strValue.replace
         (
            m_strValue.Mid(iCurPos, iParseLen), 
            string(chSubst)
         );
      }
   }
...
bool LiteHTMLElemAttr::isNamedColorValue(::lite_html_reader * preader) const
{
   if ( (m_strValue.get_length()) && (::_istalpha(m_strValue[0])) )
   {
      ...
      strKey.make_lower();
      if (preader->m_papp->m_psystem->html_ex().m_namedColors.Lookup(m_strValue, crTemp))
         return (true);
   }
   return (false);
}
...
bool LiteHTMLElemAttr::isSysColorValue(::lite_html_reader * preader) const
{
   if ( (m_strValue.get_length()) && (::_istalpha(m_strValue[0])) )
   {
      ...
      strKey.make_lower();
      if (preader->m_papp->m_psystem->html_ex().m_namedColors.Lookup(strKey, crTemp))
