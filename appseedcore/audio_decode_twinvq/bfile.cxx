...
BFILE *bopen(CFile * pfile,	/* Input: Data file name */
...
int bseek(BFILE *stream, long offset, int origin)
{
	...
	case BSEEK_CUR:
		{
			...
			if (0 <= offsetTest && offsetTest < stream->nbuf){ // if seeking is within the buffer
               ...
               fs_ret = stream->fp->Seek( nSeekStep*BBUFSIZ, CFile::current);
                  ...
                  stream->fp->Seek(nSeekStep*BBUFSIZ, CFile::current);
	...
	case BSEEK_SET: // top of the file
      ...
      fs_ret =  stream->fp->Seek((offset/BBUFLEN)*BBUFSIZ, CFile::begin);
		if (fs_ret != 0) return fs_ret;
		stream->readable = 0;
		stream->nbuf *= 8;
	...
	case BSEEK_END:
      ...
      fs_ret = stream->fp->Seek(((offset-BBUFLEN+1)/BBUFLEN-1)*BBUFSIZ, CFile::end);
		if (fs_ret != 0) return fs_ret;
		stream->readable = 0;
		stream->nbuf *= 8;
		  ...
		  BFILE	*stream ) /* Input:  File pointer */
{
    ...
    for ( ibits=0; ibits<nbits; ibits++ ){
		if ( stream->readable == 0 ){  /* when the file data buffer is empty */
         stream->nbuf = stream->fp->Read(stream->buf, BBUFSIZ);
