	...
	ex1::file_exception_sp* pException)
{
   if (m_hFile != (UINT)hFileNull)
	...
	ASSERT(pException == NULL ||
		AfxIsValidAddress(pException, sizeof(ex1::file_exception_sp)));
   ...
   if(nOpenFlags & ex1::file::defer_create_directory)
   {
	...
	m_bCloseOnDelete = FALSE;
	m_hFile = (UINT)hFileNull;
	m_wstrFileName.Empty();

	string strTemp;
	Application.file_system().FullPath(strTemp, lpszFileName);
	gen::international::utf8_to_unicode(m_wstrFileName, strTemp);
   ...
   gen::international::utf8_to_unicode(wstrFileName, lpszFileName);

	// attempt file creation
	HANDLE hFile = WindowsShell::CreateFile(wstrFileName, dwAccess, dwShareMode, &sa,
...
void WinFile::write(const void * lpBuf, DWORD_PTR nCount)
{
	...
	if (!::WriteFile((HANDLE)m_hFile, lpBuf, nCount, &nWritten, NULL))
		WinFileException::ThrowOsError(get_app(), (LONG)::GetLastError(), m_wstrFileName);

	// Win32s will not return an error all the time (usually DISK_FULL)
	if (nWritten != nCount)
		vfxThrowFileException(get_app(), ::ex1::file_exception::diskFull, -1, m_wstrFileName);
}
...
void WinFile::LockRange(DWORD dwPos, DWORD dwCount)
{
	ASSERT_VALID(this);
	ASSERT(m_hFile != (UINT)hFileNull);
...
void WinFile::UnlockRange(DWORD dwPos, DWORD dwCount)
{
	ASSERT_VALID(this);
	ASSERT(m_hFile != (UINT)hFileNull);
...
void WinFile::SetLength(DWORD dwNewLen)
{
	ASSERT_VALID(this);
	ASSERT(m_hFile != (UINT)hFileNull);
...
DWORD WinFile::get_length() const
{
	ASSERT_VALID(this);
...
wstring CLASS_DECL_VMSWIN vfxStringFromCLSID(REFCLSID rclsid)
{
	WCHAR szCLSID[256];
...
void CLASS_DECL_VMSWIN vfxGetModuleShortFileName(HINSTANCE hInst, wstring& strShortName)
{
	WCHAR szLongPathName[_MAX_PATH];
	::GetModuleFileNameW(hInst, szLongPathName, _MAX_PATH);
	if (::GetShortPathNameW(szLongPathName,
		strShortName.GetBuffer(_MAX_PATH), _MAX_PATH) == 0)
	{
		// rare failure case (especially on not-so-modern file systems)
		strShortName = szLongPathName;
...
#ifdef _DEBUG
void WinFile::AssertValid() const
{
	base_object::AssertValid();
	// we permit the descriptor m_hFile to be any value for derived classes
}

void WinFile::Dump(CDumpContext & dumpcontext) const
{
...
// IMPLEMENT_DYNAMIC(WinFile, base_object)
...
void CLASS_DECL_VMSWIN vfxGetRoot(const wchar_t * lpszPath, wstring& wstrRoot)
{
	ASSERT(lpszPath != NULL);
	// determine the root name of the volume
	wchar_t * lpszRoot = wstrRoot.GetBuffer(_MAX_PATH);
	memset(lpszRoot, 0, _MAX_PATH);
	lstrcpynW(lpszRoot, lpszPath, _MAX_PATH);
   wchar_t * lpsz;
...
string WinFile::GetFileName() const
{
	...
	GetStatus(status);
	string wstrResult;
   Application.file_system().GetFileName(status.m_strFullName, wstrResult);
	return wstrResult;
}
...
string WinFile::GetFileTitle() const
{
	...
	GetStatus(status);
	string wstrResult;
	Application.file_system().GetFileTitle(status.m_strFullName, wstrResult);
	return wstrResult;
}
...
void PASCAL WinFileException::ThrowOsError(::ca::application * papp, LONG lOsError,
	const wchar_t * lpszFileName /* = NULL */)
{
	if (lOsError != 0)
		vfxThrowFileException(papp, WinFileException::OsErrorToException(lOsError),
			lOsError, lpszFileName);
}

void PASCAL WinFileException::ThrowErrno(::ca::application * papp, int nErrno,
	const wchar_t * lpszFileName /* = NULL */)
{
	if (nErrno != 0)
		vfxThrowFileException(papp, WinFileException::ErrnoToException(nErrno),
...
#ifdef _DEBUG
void WinFileException::Dump(CDumpContext & dumpcontext) const
{
	base_object::Dump(dumpcontext);
...
void CLASS_DECL_VMSWIN vfxThrowFileException(::ca::application * papp, int cause, LONG lOsError,
	const wchar_t * lpszFileName /* == NULL */)
{
#ifdef _DEBUG
	const char * lpsz;
...
#endif
   string strFileName;
   gen::international::unicode_to_utf8(strFileName, lpszFileName);
	throw new WinFileException(papp, cause, lOsError, strFileName);
}
...
BOOL WinFile::GetStatus(::ex1::file_status& rStatus) const
{
   ...
   gen::international::unicode_to_utf8(rStatus.m_strFullName, m_wstrFileName);
	...
	if (m_hFile != hFileNull)
	{
		...
		if (m_wstrFileName.is_empty())
			rStatus.m_attribute = 0;
		else
		{
			DWORD dwAttribute = ::GetFileAttributesW(m_wstrFileName);
...
BOOL PASCAL WinFile::GetStatus(const char * lpszFileName, ::ex1::file_status& rStatus)
{
   ...
   wstring wstrFileName;
   gen::international::utf8_to_unicode(wstrFileName, lpszFileName);
	if (!vfxFullPath(wstrFullName.GetBufferSetLength(MAX_PATH), wstrFileName))
	{
		rStatus.m_strFullName.Empty();
		return FALSE;
	}
   wstrFullName.ReleaseBuffer();
...
{
   wstring wstrNewName;
   gen::international::utf8_to_unicode(wstrNewName, lpszNewName);
	ASSERT_VALID(this);
	ASSERT(AfxIsValidString(lpszNewName));
	m_wstrFileName = wstrNewName;
}
