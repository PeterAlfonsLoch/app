...
_AFX_THREAD_STATE::_AFX_THREAD_STATE()
{
	...
	m_nMsgLast = WM_NULL;
	//Application.get_cursor_pos(&(m_ptCursorLast));
...
#endif
{
   /* xxx xxx xxx
	...
	// cast starts out in "::user::profile control"
	m_bUserCtrl = TRUE;
...
{
	AFX_MANAGE_STATE(_afxBaseModuleState.get_data());
...
{
	application* pApp = AfxGetModuleState()->m_pCurrentWinApp;
