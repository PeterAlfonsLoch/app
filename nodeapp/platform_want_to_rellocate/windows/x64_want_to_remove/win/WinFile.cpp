...
INT_PTR WinFile::seek(INT_PTR lOff, UINT nFrom)
{
	...
	DWORD_PTR dwNew = ::SetFilePointer((HANDLE)m_hFile, lOff, NULL, (DWORD)nFrom);
	if (dwNew  == (DWORD)-1)
		WinFileException::ThrowOsError(get_app(), (LONG)::GetLastError());
...
DWORD_PTR WinFile::GetPosition() const
{
	...
	DWORD_PTR dwPos = ::SetFilePointer((HANDLE)m_hFile, 0, NULL, FILE_CURRENT);
	if (dwPos  == (DWORD)-1)
		WinFileException::ThrowOsError(get_app(), (LONG)::GetLastError());
...
BOOL CLASS_DECL_VMSWIN vfxFullPath(wstring & wstr, const wchar_t * lpszFileIn)
	// lpszPathOut = buffer of _MAX_PATH
	// lpszFileIn = file, relative path or absolute path
	// (both in ANSI character set)
{
   WCHAR psz[_MAX_PATH];
   if(!vfxFullPath(psz, lpszFileIn))
   {
      wstr.Empty();
      return false;
   }
