...
public:
   ...
   static BOOL SHGetPathFromIDList(LPCITEMIDLIST pidl, wstring & wstr);
   ...
   static WH_SHSTDAPI(DWORD_PTR) SHGetFileInfo(      
      const wchar_t * pszPath,
      DWORD dwFileAttributes,
      SHFILEINFOW *psfi,
      UINT cbFileInfo,
   ...
   static BOOL SHGetPathFromIDList(LPCITEMIDLIST pidl, string & wstr);


};

inline BOOL WindowsShell::SHGetPathFromIDList(LPCITEMIDLIST pidl, wstring & wstr)
{
   wchar_t * pszPath = wstr.GetBuffer(MAX_PATH * 2);
   if(!(*theWindowsShell.m_pfnSHGetPathFromIDList)(pidl, pszPath))
   {
      wstr.ReleaseBuffer();
      return FALSE;
   }
...
inline DWORD_PTR WindowsShell::SHGetFileInfo(
   const wchar_t * pszPath,
   DWORD dwFileAttributes,
   SHFILEINFOW *psfi,
   UINT cbFileInfo,
...
inline DWORD WindowsShell::GetTempPath(
   string & str)
{
   wstring wstr;
   DWORD dwResult = (*theWindowsShell.m_pfnGetTempPath)(
      MAX_PATH * 8,
      wstr.GetBufferSetLength(MAX_PATH * 8));
   wstr.ReleaseBuffer();
   gen::international::unicode_to_utf8(str, wstr);
   return dwResult;
}

inline UINT WindowsShell::GetTempFileName(
   const char * lpPathName,
   const char * lpPrefixString,
   UINT uUnique,
   string & str)
{
   ...
   wstring wstrPrefixString;
   gen::international::utf8_to_unicode(wstrPathName, lpPathName);
   gen::international::utf8_to_unicode(wstrPrefixString, lpPrefixString);
...
{
   return MoveFile(
      gen::international::utf8_to_unicode(lpExistingFileName),
      gen::international::utf8_to_unicode(lpNewFileName));
...
{
   wstring wstr;
   BOOL b = SHGetPathFromIDList(pidl, wstr);
   gen::international::unicode_to_utf8(str, wstr);
   return b;
}
