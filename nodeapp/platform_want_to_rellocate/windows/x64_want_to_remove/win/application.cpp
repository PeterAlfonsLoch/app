      ...
      */
      ...
      string strTitle = Application.load_string("Application.title");
	   ...
	   if (m_strAppName == NULL)
	   {
		   ...
		   {
			   m_strAppName = _tcsdup(strTitle);    // human readable title
		   }
		   else
		   {
			   m_strAppName = _tcsdup(m_pszExeName);   // same as EXE
		   }
		   AfxEnableMemoryTracking(bEnable);
		   if(!m_strAppName)
		   {
         ...
         if(Application.m_strAppName == NULL)
         {
            Application.m_strAppName = m_strAppName;
         }
	   }

      AfxGetModuleState()->m_lpszCurrentAppName = m_strAppName;
