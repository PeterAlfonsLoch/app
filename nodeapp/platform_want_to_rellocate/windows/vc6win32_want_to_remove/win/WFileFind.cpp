...
BOOL FileFind::FindFile(const char * pstrName /* = NULL */,
	DWORD dwUnused /* = 0 */)
{
   ...
   gen::international::utf8_to_unicode(wstrName, pstrName);
   ...
   m_hContext = WindowsShell::FindFirstFile(wstrName, (WIN32_FIND_DATAW*) m_pNextInfo);
	...
	wchar_t * pstrRoot = m_strRoot.GetBufferSetLength(_MAX_PATH);
...
string FileFind::GetRoot() const
{
   ...
   string strRoot;
   gen::international::unicode_to_utf8(strRoot, m_strRoot);
...
DWORD FileFind::get_length() const
{
	ASSERT(m_hContext != NULL);
	ASSERT_VALID(this);
...
__int64 FileFind::GetLength64() const
{
	ASSERT(m_hContext != NULL);
	ASSERT_VALID(this);
...
#ifdef _DEBUG
void FileFind::Dump(CDumpContext & dumpcontext) const
{
	base_object::Dump(dumpcontext);
	dumpcontext << "\nm_hContext = " << (UINT) m_hContext;
}

void FileFind::AssertValid() const
{
...
// IMPLEMENT_DYNAMIC(FileFind, base_object)
