...
{
	static void PASCAL Construct(base_object* pObject)
	{ 
		new (pObject) TYPE; 
	}
	static void PASCAL Destruct(base_object* pObject)
	{
...
//	ASSERT(m_pClass != NULL);
	...
	else if ((pObject = lookup_temporary(h)) != NULL)
	{
	...
	// C++ object to wrap it.  We don't want the ::user::profile to see this memory
   ...
   HANDLE* ph = pTemp->m_handlea;  // after base_object
	ph[0] = h;
	if (HT::s_iHandleCount == 2)
		ph[1] = h;
...
void handle_map < HT, CT >::delete_temp()
{
	...
	while (pos != NULL)
	{
		...
		CT * pTemp;
		m_temporaryMap.GetNextAssoc(pos, h, pTemp);
...
{ return m_permanentMap.get(h, (CT*) NULL); }
template < class HT, class CT >
inline CT* handle_map <HT, CT>::lookup_temporary(HANDLE h)
{ return m_temporaryMap.get(h, (CT*) NULL); }
