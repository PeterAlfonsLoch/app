...
UINT APIENTRY _AfxThreadEntry(void * pParam)
{
   ...
   pThread->m_evFinish.ResetEvent();
   pThread->_001InstallMessageHandling(pThread);
   pThread->m_p->_001InstallMessageHandling(pThread);
   ::ca::window threadWnd;
   try
   {
      ...
      // set current thread pointer for Application.GetThread
      AFX_MODULE_STATE* pModuleState = AfxGetModuleState();
      AFX_MODULE_THREAD_STATE* pState = pModuleState->m_thread;
      pState->m_pCurrentWinThread = dynamic_cast < ::win::thread * > (pThread);

      // forced initialization of the thread
      AfxInitThread();

      // thread inherits cast's main ::ca::window if not already set
      ::application* pApp = dynamic_cast < application * > (pThread->get_app());
      if (pApp != NULL && pThread->GetMainWnd() == NULL)
      {
         // just attach the HWND
         // trans			threadWnd.Attach(pApp->GetMainWnd()->get_handle());
         //pThread->GetMainWnd() = pApp->GetMainWnd();
      }
   }
   ...
   pThread->m_ptimera = new ::user::interaction::timer_array;
   pThread->m_puiptra = new user::LPWndArray;
   pThread->m_bRun = true;
   ...
   if(!pThread->m_p->PreInitInstance())
   {
      return 0;
   }
   ...
   else if (!pThread->m_p->initialize_instance())
   {
      ASSERT_VALID(pThread);
      try
      {
         nResult = pThread->exit();
      }
      catch(...)
      {
         nResult = (DWORD) -1;
...
run:
      try
      {
         try
         {
            nResult = pThread->m_p->run();
         }
         catch(const ::ca::exception & e)
         {
            if(pThread->on_run_exception((::ca::exception &) e))
               goto run;
            if(pThread->get_app()->final_handle_exception((::ca::exception & ) e))
               goto run;
            try
            {
               nResult = pThread->exit();
...
void AfxInternalPreTranslateMessage(gen::signal_object * pobj)
{
   try
   {
      ...
      ::user::interaction* pMainWnd = App(pobj->get_app()).GetMainWnd();
      if(pMainWnd != NULL)
      {
      ...
      user::LPWndArray wnda = Application.frames();
...
BOOL AfxInternalIsIdleMessage(gen::signal_object * pobj)
{
   ...
   // affect the state of the ::user::profile interface and happen very
...
{
   win::thread * pThread = WIN_THREAD(Application.GetThread());
   if(pThread)
...
void CLASS_DECL_VMSWIN AfxEndThread(UINT nExitCode, BOOL bDelete)
{
   ...
   {
      ASSERT_VALID(pThread);
      //ASSERT(pThread != Application::smart_pointer < ex2::application *>::m_p);
...
void CLASS_DECL_VMSWIN AfxTermThread(HINSTANCE hInstTerm)
{
	try
	{
#ifdef _DEBUG
...
namespace win
{
   ...
   void thread::CommonConstruct()
   {
      ...
      Application.get_cursor_pos(&(pState->m_ptCursorLast));
   ...
   void thread::OnPointerClear(::user::interaction * p)
   {
      ::user::interaction * pui = dynamic_cast < ::user::interaction * > (p);
      remove(pui);
      App(::get_app()).remove(pui);
      WIN_THREAD(App(::get_app()).smart_pointer < ::ca::thread >::m_p)->remove(p);
   }
   ...
   ::user::interaction * thread::SetMainWnd(::user::interaction * pui)
   {
      if(pui != NULL)
      {
         pointer_array::add(dynamic_cast < pointer < ::user::interaction > * > (pui));
      }
      ::user::interaction * puiPrevious = m_pMainWnd;
      m_pMainWnd  = pui;
      return puiPrevious;
   }
   ...
   void thread::set_timer(::user::interaction * pui, UINT_PTR nIDEvent, UINT nEllapse)
   {
      ...
      int iMin = 100;
      for(int i = 0; i < m_ptimera->base_array<::user::interaction::timer_item,::user::interaction::timer_item &>::get_count(); i++)
      {
         if(m_ptimera->base_array<::user::interaction::timer_item,::user::interaction::timer_item &>::element_at(i).m_uiElapse < natural(iMin))
         {
            iMin = m_ptimera->base_array<::user::interaction::timer_item,::user::interaction::timer_item &>::element_at(i).m_uiElapse;
         }
      }
      pui->get_wnd()->SetTimer((UINT_PTR)-2, iMin, NULL);
   }
   ...
   {
      ::user::interaction * puiPrevious = m_pActiveWnd;
      m_pActiveWnd = pui;
      return puiPrevious;
   }
      ...
      LPSECURITY_ATTRIBUTES lpSecurityAttrs)
{
	...
	{
		TRACE(traceAppMsg, 0, "Warning: CreateEvent failed in thread::CreateThread.\n");
...
//	   _AFX_THREAD_STATE* pState = AfxGetThreadState();
   ...
   void thread::ProcessMessageFilter(int code, gen::signal_object * pobj)
   {
      ...
      case MSGF_DIALOGBOX:    // handles message boxes as well.
            ...
            pbase->m_uiMessage >= WM_KEYFIRST && pbase->m_uiMessage <= WM_KEYLAST)
         {
            ...
            if (m_pActiveWnd->IsWindowEnabled())
            {
   ...
   ::user::interaction* thread::GetMainWnd()
   {
      ...
      return Application.get_active_guie();
   }
   ...
   BOOL thread::pump_message()
   {
      try
      {
         ...
         {
            TRACE(traceAppMsg, 1, "thread::pump_message - Received WM_QUIT.\n");
         ...
         {
            TRACE(traceAppMsg, 0, "Error: thread::pump_message called when not permitted.\n");
            ASSERT(FALSE);
         }
         ...
         {
            ::ca::smart_pointer < ::user::win::message::base > spbase(get_base(&msg));
...
#ifdef _DEBUG
   void thread::AssertValid() const
   {
	   command_target::AssertValid();
   }
   void thread::Dump(CDumpContext & dumpcontext) const
  {
...
/*thread* CLASS_DECL_VMSWIN Application.GetThread()
{
// check for current thread in module thread state
AFX_MODULE_THREAD_STATE* pState = AfxGetModuleThreadState();
...
thread* pThread = dynamic_cast < thread * > (get_app()->alloc(pThreadClass));
if (pThread == NULL)
...
{
   thread* pthread;
   if (afxContextIsDLL || (code < 0 && code != MSGF_DDEMGR) || (pthread = Application.GetThread()) == NULL)
   {
      return ::CallNextHookEx(_afxThreadState->m_hHookOldMsgFilter, code, wParam, lParam);
   }
   ASSERT(pthread != NULL);
   ::ca::smart_pointer < ::user::win::message::base > spbase(pthread->get_base((LPMSG)lParam));
   ...
   case MSGF_MENU:
      ...
      if (pMsgWnd != NULL)
      {
         ...
         if (pTopFrameWnd != NULL && pTopFrameWnd->IsTracking() &&
            pTopFrameWnd->m_bHelpMode)
         {
            pMainWnd = Application.GetMainWnd();
