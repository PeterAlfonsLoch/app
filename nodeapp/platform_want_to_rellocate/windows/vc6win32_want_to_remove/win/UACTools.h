...
ptet
	...
	TokenElevationTypeDefault - User is not using a "split" token. 
		...
		by a standard ::user::profile (not a member of the Administrators group).

	The following two values can be returned only if both the UAC is enabled and
	the ::user::profile is a member of the Administrator's group (that is, the ::user::profile has a "split" token):
...
pbElevated
	...
	TRUE - the current process is elevated.
		...
		the administrator, or that UAC is disabled and the process was started by a ::user::profile 
	...
	FALSE - the current process is not elevated (limited).
		...
		without the elevation, or that UAC is disabled and the process was started by a standard ::user::profile. 
