...
namespace win
{
   ...
   ::ca::bitmap * dib::get_bitmap()
   {
      return m_spbitmap;
   }

   ::ca::bitmap * dib::detach_bitmap()
   {
   ...
   void dib::ToAlphaAndFill(int i, COLORREF cr)
   {
      ...
      BYTE uchB = GetBValue(cr);
      BYTE uchG = GetGValue(cr);
      BYTE uchR = GetRValue(cr);
...
//	   COLORREF color=RGB ( B, G, R );
   ...
   {
      COLORREF crFind = RGB(GetBValue(crMask), GetGValue(crMask), GetRValue(crMask));
      COLORREF crSet = RGB(GetBValue(crInMask), GetGValue(crInMask), GetRValue(crInMask));
      COLORREF crUnset  = RGB(GetBValue(crOutMask), GetGValue(crOutMask), GetRValue(crOutMask));
