...
void __cdecl _AfxCrtDumpClient(void * pvData, size_t nBytes)
{
   ...
   try
   {
      ...
      ::ca::ca * pca = (::ca::ca * ) pvData;

      base_object * pobject = NULL;

      try
      {
         pobject = dynamic_cast < base_object * > (pca);
      }
      catch(...)
      {
         pobject = NULL;
...
_AFX_DEBUG_STATE::~_AFX_DEBUG_STATE()
{
   ...
   {
      TRACE("~_AFX_DEBUG_STATE _CrtdumpMemoryLeaks std::__non_rtti_object\n");
      TRACE(e.what());
      TRACE("\n");
   }
   catch(...)
   {
      TRACE("~_AFX_DEBUG_STATE _CrtdumpMemoryLeaks exception\n");
   }
