...
void CLASS_DECL_VMSWIN AfxUnregisterWndClasses()
{
	...
	int start=0;
	string className = pModuleState->m_strUnregisterList.Tokenize("\n",start);
	while (!className.is_empty())
	{
		UnregisterClass(static_cast<const char *>(className), Application.m_hInstance);
		className = pModuleState->m_strUnregisterList.Tokenize("\n",start);
	}
