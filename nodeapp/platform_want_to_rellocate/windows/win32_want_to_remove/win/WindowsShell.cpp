...
*
*Purpose:
*       _getcwd gets the current working directory for the ::user::profile,
...
*
*Entry:
*       _TSCHAR *pnbuf = pointer to a buffer maintained by the ::user::profile;
...
*
*Purpose:
*       _getdcwd gets the current working directory for the ::user::profile,
*       placing it in the buffer pointed to by pnbuf.  It returns
*       the length of the string put in the buffer.  If the length
*       of the string exceeds the length of the buffer, maxlen,
*       then NULL is returned.  If pnbuf = NULL, maxlen is ignored,
...
/*            len = GetCurrentDirectory( sizeof(dirbuf) / sizeof(_TSCHAR),
  ...
  /*      if ( len == 0 || ++len > sizeof(dirbuf)/sizeof(_TSCHAR) )
    ...
    /*    if ( (p = pnbuf) == NULL ) {
            /*
             * allocate a buffer for the ::user::profile.
             */
...
//                errno = ENOMEM;
            ...
            /*
             * Won't fit in the ::user::profile-supplied buffer!
             */
//            errno = ERANGE; /* Won't fit in ::user::profile buffer */
        /*    return NULL;
        }

        /*
         * Place the current directory string into the ::user::profile buffer
         */
/*
...
DWORD WindowsShell::_SHGetFileInfo(      
