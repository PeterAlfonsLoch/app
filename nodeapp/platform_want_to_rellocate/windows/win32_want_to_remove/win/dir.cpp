...
namespace win
{
   ...
   {
      if(::ca::dir::is(lpcsz)) // if base class "already" "says" it is a dir, let it handle it: may be not a operational system dir, e.g., zip or compressed directory...
      {
         return ::ca::dir::ls_pattern(lpcsz, pszPattern, pstraPath, pstraTitle, pbaIsDir, piaSize);
      }
      FileFind filefind;
      BOOL bWorking;
   ...
   {
      return path(ca2("appdata", lpcszPrefix), lpcsz, lpcsz2);
   }
   ...
   {
      return path(path(ca2("user"), lpcszPrefix, lpcszLogin), pszRelativePath);
   }
