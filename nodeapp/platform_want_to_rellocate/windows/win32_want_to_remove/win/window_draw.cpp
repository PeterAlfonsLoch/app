...
namespace win
{
      ...
      message_window_simple_callback(papp)
   {
      ...
      AfxBeginThread(papp, &window_draw::ThreadProcRedraw, (LPVOID) this);
   }
...
//	   DWORD dwTimeIn = GetTickCount();
      ...
      {
         ::user::window_interface * ptwi = ::user::get_window_map()->get((INT_PTR) hwndParam);
...
//	   DWORD dwTimeOut = GetTickCount();
   ...
   UINT window_draw::RedrawProc()
   {
      ...
      ::AttachThreadInput(::GetCurrentThreadId(), WIN_THREAD(Application.::ca::thread_sp::m_p)->m_nThreadID, TRUE);
      MSG msg;
      s_bRunning = true;
      while(m_bRun)
      {
...
//	   DWORD dwTime2 = GetTickCount();
   ...
   /*         else if(::GetWindowLong(wndaApp[j], GWL_EXSTYLE) & WS_EX_LAYERED)
         ...
         for(int j = wndaApp.get_upper_bound(); j >= 0; j--)
         {
            ...
            ::ca::window * pwnd = dynamic_cast < ::ca::window * > (window::FromHandlePermanent(hwndTopic));
            if(pwnd == NULL)
            {
            ...
            if(rectIntersect.area() <= 0)
            {
               ...
               {
                  class rect rect = Application.m_monitorinfoa[0].rcMonitor;
...
//	   DWORD dwTimeIn = GetTickCount();
	   ...
	   LPCRECT lpcrect)
   {
      ...
      ::user::window_interface * ptwi = ::user::get_window_map()->get((INT_PTR) hwnd);
   ...
   //   ::ca::window * pwnd = ::win::window::from_handle(hwnd);
      ...
      ::user::window_interface * pwndi = ::user::get_window_map()->get((INT_PTR) hwnd);
   ...
   {
      Application.frames().get_wnda(hwnda);
   }
...
//	   DWORD dw = GetLastError();
   ...
   /*	if(!::DrawDibDraw ( pbuffer->m_drawdib.m_hdrawdib, 
      ...
      if(::GetWindowLong(hwndParam, GWL_EXSTYLE) & WS_EX_LAYERED)
      {
         ...
         sz.cx = (LONG) min(rectWindow.right - pt.x, Application.m_monitorinfoa[0].rcMonitor.right - pt.x);
         sz.cy = (LONG) min(rectWindow.bottom - pt.y, Application.m_monitorinfoa[0].rcMonitor.bottom - pt.y);
