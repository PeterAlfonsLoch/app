...
namespace file
{
   ...
   namespace dir
   {
      ...
      {
         return m_paceapp->m_pacesystem->m_spdir->locale_schema(get_app(), pszLocale, pszStyle);
      }
      ...
      {
         return m_paceapp->m_pacesystem->m_spdir->matter(get_app(), stra, bDir, pszRoot, pszApp);
      }

      ::file::path application::usersystemappdata(const string & lpcszPrefix)
      {
         return m_paceapp->m_pacesystem->m_spdir->usersystemappdata(get_app(), lpcszPrefix);
      }

      ::file::path application::userappdata()
      {
         return m_paceapp->m_pacesystem->m_spdir->userappdata(get_app());
      }

      ::file::path application::userdata()
      {
         return m_paceapp->m_pacesystem->m_spdir->userdata(get_app());
      }

      ::file::path application::userfolder()
      {
         return m_paceapp->m_pacesystem->m_spdir->userfolder(get_app());
      }

      ::file::path application::default_os_user_path_prefix()
      {
         return m_paceapp->m_pacesystem->m_spdir->default_os_user_path_prefix(get_app());
      }

      ::file::path application::default_userappdata(const string & lpcszPrefix, const string & lpcszLogin)
      {
         return m_paceapp->m_pacesystem->m_spdir->default_userappdata(get_app(), lpcszPrefix, lpcszLogin);
      }

      ::file::path application::default_userdata(const string & lpcszPrefix,const string & lpcszLogin)
      {
         return m_paceapp->m_pacesystem->m_spdir->default_userdata(get_app(), lpcszPrefix, lpcszLogin);
      }

      ::file::path application::default_userfolder(const string & lpcszPrefix,const string & lpcszLogin)
      {
         return m_paceapp->m_pacesystem->m_spdir->default_userfolder(get_app(), lpcszPrefix, lpcszLogin);
      }

      ::file::path application::userquicklaunch()
      {
         return m_paceapp->m_pacesystem->m_spdir->userquicklaunch(get_app());
      }
      ...
      ::file::path application::userprograms()
      {
         ...
         return m_paceapp->m_pacesystem->m_spdir->userprograms(get_app());
      ...
      ::file::listing & application::ls(listing & listing)
      {
         ...
         return m_paceapp->m_pacesystem->m_spdir.m_p->ls(m_paceapp, listing);
      ...
      {
         return m_paceapp->m_pacesystem->m_spdir.m_p->has_subdir(m_paceapp, lpcsz);
      }


      bool application::is(const ::file::path & lpcsz)
      {
         return m_paceapp->m_pacesystem->m_spdir.m_p->is(lpcsz, m_paceapp);
      }

      bool application::is_inside(const ::file::path & lpcszDir,const ::file::path & lpcszPath)
      {
         return m_paceapp->m_pacesystem->m_spdir.m_p->is_inside(lpcszDir, lpcszPath, m_paceapp);
      }

      bool application::is_inside_time(const ::file::path & lpcsz)
      {
         return m_paceapp->m_pacesystem->m_spdir.m_p->is_inside_time(lpcsz, m_paceapp);
      }


      ::file::listing & application::root_ones(::file::listing & listing)
      {
         return m_paceapp->m_pacesystem->m_spdir.m_p->root_ones(listing, m_paceapp);
      }


      bool application::mk(const ::file::path & lpcsz)
      {
         return m_paceapp->m_pacesystem->m_spdir.m_p->mk(lpcsz, m_paceapp);
      }


      bool application::rm(const ::file::path & psz,bool bRecursive)
      {
         return m_paceapp->m_pacesystem->m_spdir.m_p->rm(m_paceapp, psz, bRecursive);
      }


      ::file::path application::pathfind(const string & pszEnv, const string & pszTopic, const string & pszMode)
      {
         return m_paceapp->m_pacesystem->m_spdir->pathfind(pszEnv, pszTopic, pszMode, m_paceapp);
      }
