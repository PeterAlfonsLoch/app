...
namespace ios
{
   ...
   window_draw::window_draw(::ace::application * papp) :
   ::object(papp),
   ::thread(papp),
   ...
   {
      SCAST_PTR(::message::ace, pbase, pobj);
      ...
      if(dwTakeTime > iFailureTime)
      {
         ...
         if(s_dwLastFrameFrame > iFrameTimeFrame)
         {
            ...
            if(s_iFrameFailureCount > iMaxFailureCount)
            {
               ...
               if(!Session.savings().is_trying_to_save(::ace::resource_blur_background))
               {
                  ...
                  Session.savings().try_to_save(::ace::resource_blur_background);
               }
               else if(!Session.savings().is_trying_to_save(::ace::resource_blurred_text_embossing))
               {
                  ...
                  Session.savings().try_to_save(::ace::resource_blurred_text_embossing);
               }
               else if(!Session.savings().is_warning(::ace::resource_processing))
               {
                  ...
                  Session.savings().warn(::ace::resource_processing);
               }
               else if(!Session.savings().is_trying_to_save(::ace::resource_processing))
               {
...
//            bool bWin4 = FALSE;
            ...
            /*            ::DefWindowProc(
             ...
             (WPARAM)((dynamic_cast<::lnx::graphics * >(pgraphics))->get_os_data()),
...
//      if(!slSemaphoreBuffer.lock(duration::zero()))
      ...
      ::draw2d::graphics * pgraphics = (dynamic_cast < ::draw2d::graphics * > (m_pbuffer->GetBuffer()));
