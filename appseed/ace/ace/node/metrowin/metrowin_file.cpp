...
namespace metrowin
{
   ...
   file::file(::ace::application * papp) :
      ::object(papp)
   {
   ...
   file::file(::ace::application * papp, int hFile) :
      ::object(papp)
   {
   ...
   file::file(::ace::application * papp, const char * lpszFileName, UINT nOpenFlags) :
      ::object(papp)
   {
...
//      if (!::LockFile((HANDLE)m_hFile, LODWORD(dwPos), HIDWORD(dwPos), LODWORD(dwCount), HIDWORD(dwCount)))
   ...
   */
   ...
   string CLASS_DECL_ACE vfxStringFromCLSID(REFCLSID rclsid)
   {
      CHAR szCLSID[256];
   ...
   void CLASS_DECL_ACE vfxGetModuleShortFileName(HINSTANCE hInst, string& strShortName)
   {
#ifdef WINDOWSEX
...
#define EDEADLOCK       EDEADLK
   ...
   UINT CLASS_DECL_ACE vfxGetFileName(const unichar * lpszPathName, unichar * lpszTitle, UINT nMax)
   {
      ASSERT(lpszTitle == NULL ||
         __is_valid_address(lpszTitle, _MAX_FNAME));
      ASSERT(__is_valid_string(lpszPathName));
   ...
   void WinFileException::ThrowOsError(::ace::application * papp, LONG lOsError, const char * lpszFileName /* = NULL */)
   {
      if (lOsError != 0)
         ::file::throw_exception(papp, WinFileException::OsErrorToException(lOsError), lOsError, lpszFileName);
   }

   void WinFileException::ThrowErrno(::ace::application * papp, int nErrno, const char * lpszFileName /* = NULL */)
   {
      if (nErrno != 0)
   ...
   void CLASS_DECL_ACE throw_exception(sp(::ace::application) papp, int cause, LONG lOsError, const char * lpszFileName /* == NULL */)
   {
#ifdef DEBUG
      const char * lpsz;
   ...
   // IMPLEMENT_DYNAMIC(WinFileException, ::exception::ace)
   ...
   */
   ...
   bool CLASS_DECL_ACE vfxResolveShortcut(string & strTarget, const char * pszSource, ::user::primitive * puiMessageParentOptional)
   {
...
///*void CLASS_DECL_ACE ::file::throw_exception(::ace::application * papp, int cause, LONG lOsError,   const char * lpszFileName /* == NULL */)
//{
