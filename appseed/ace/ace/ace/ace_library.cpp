...
namespace ace
{
   ...
   library::library(::ace::application * papp,int iDesambig,const char * pszRoot):
      object(papp)
   {
   ...
   bool library::open(const char * pszPath,bool bAutoClose,bool bCa2Path)
   {
      ...
      ::output_debug_string("\n\n::ace::library::open Going to load library : " + string(pszPath) + " (bCa2Path = "+::str::from((int)bCa2Path)+")\n\n");
      ...
      try
      {
         ...
         if(m_plibrary == NULL)
         {
            ...
            ::output_debug_string("\n\n::ace::library::open :: Failed to open library : " + string(pszPath) + " (bCa2Path = "+::str::from((int)bCa2Path)+") \n\n");
      ...
      catch(...)
      {
         ...
         ::output_debug_string("\n\n::ace::library::open :: An exception occurred while opening library : " + string(pszPath) + "\n\n");
...
void * library::get_os_data()
{
   ...
   sp(::ace::application) library::get_new_app(const char * pszAppId)
   {
      ...
      try
      {
         ...
         if(get_ca2_library() != NULL)
         {
            ...
            sp(::ace::application) papp = get_ca2_library()->get_new_app(strAppName);
   ...
   sp(::object) library::create_object(::ace::application * papp,const char * pszClassId, object * p)
   {
...
string_map < sp(::ace::library) > * g_pmapLibCall = NULL;


::ace::library * lib(const char * psz)
{

   if(g_pmapLibCall == NULL)
      return NULL;

   sp(::ace::library) & lib = g_pmapLibCall->operator[](psz);

   if(lib.is_null())
   {
      lib = canew(::ace::library(::get_thread_app()));
      lib->open(psz);
   }
