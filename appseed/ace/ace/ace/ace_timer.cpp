...
timer::timer(::ace::application * papp, uint_ptr uiTimer, PFN_TIMER pfnTimer, void * pvoidData, mutex * pmutex) :
   object(papp),
   m_nIDEvent(uiTimer),
   m_pfnTimer(pfnTimer),
...
bool timer::call_on_timer()
{
   if(!g_bAura)
   {
      output_debug_string("there is timer on (timer::call_on_timer) and ace has gone (!g_bAura)\n");
      return  false;
   }
...
VOID CALLBACK aura_timer_TimerRoutine(PVOID lpParam, BOOLEAN TimerOrWaitFired)
{
   ...
   ::ace::Timer * ptimer = (::ace::Timer *)lpParam;
