...
namespace ace
{
   ...
   ipi::ipi(::ace::application * papp,const string & strApp):
      ::object(papp),
      m_rx(papp)
   {
   ...
   bool ipi::start(const string & strApp)
   {
      ...
      ::ace::app_launcher launcher(strApp);
      ...
      if(m_txmap[strKey].is_null())
      {
         ...
         m_txmap[strKey] = canew(::ace::ipc::tx(get_app()));
   ...
   ::ace::ipc::tx & ipi::tx(const string & strApp, int iPid)
   {
   ...
   void ipi::on_receive(::ace::ipc::rx * prx,const char * pszMessage)
   {
