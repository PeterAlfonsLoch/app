...
{
   ::ace::cmd_msg msg(pcmdui);
   return _001OnCmdMsg(&msg);
}

bool command_target_interface::_001OnCmdMsg(::ace::cmd_msg * pcmdmsg)
{

   if(pcmdmsg->m_etype == ::ace::cmd_msg::type_command)
   {
...
command_target_interface::command_target_interface(::ace::application * papp)
   : object(papp)
{
}
...
bool command_target_interface::on_simple_action(id id)
{
   ...
   get_command_signal_array(::ace::cmd_msg::type_command,signalptra,id);
   bool bOk = false;
   for(int32_t i = 0; i < signalptra.get_size(); i++)
   {
      ::ace::command command(signalptra[i]->m_psignal);
      command.m_id = id;
...
{
   ::dispatch::signal_item_ptr_array signalptra;
   get_command_signal_array(::ace::cmd_msg::type_cmdui,signalptra,pcmdui->m_id);
   bool bOk = false;
   for(int32_t i = 0; i < signalptra.get_size(); i++)
   {
      ::ace::cmd_ui cmdui(signalptra[i]->m_psignal);
...
void command_target_interface::get_command_signal_array(::ace::cmd_msg::e_type etype,::dispatch::signal_item_ptr_array & signalptra,id id)
{
   ...
   for(int32_t i = 0; i < m_signalidaCommand.get_size(); i++)
   {
      ...
      {
         if(etype == ::ace::cmd_msg::type_command)
         {
            m_dispatchCommand.m_signala.GetSignalsById(signalptra,&signalid);
         }
         else if(etype == ::ace::cmd_msg::type_cmdui)
         {
