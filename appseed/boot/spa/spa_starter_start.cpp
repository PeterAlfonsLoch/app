...
{
   return file_as_string_dup(dir::element("appdata", spa_get_platform(), "build.txt"));
}

const char * get_ca2_version()
{
   return file_as_string_dup(dir::element("appdata", spa_get_platform(), "ca2_build.txt"));
}
...
{
   dir::mk(dir::element() + "\\appdata\\" + spa_get_platform());
   file_put_contents_dup(dir::element() + "\\appdata\\" + spa_get_platform() + "\\ca2_build.txt", pszBuild);
}
...
{
   dir::mk(dir::element() + "\\appdata\\" + spa_get_platform());
   file_put_contents_dup(dir::element() + "\\appdata\\" + spa_get_platform() + "\\build.txt", pszBuild);
}
