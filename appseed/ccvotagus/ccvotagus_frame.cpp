// MainFrm.cpp : implementation of the paint_frame class
//
...
IMPLEMENT_DYNCREATE(paint_frame, SimpleFrameWnd)

BEGIN_MESSAGE_MAP(paint_frame, SimpleFrameWnd)
	//{{AFX_MSG_MAP(paint_frame)
	ON_WM_CREATE()
	ON_WM_GETMINMAXINFO()
	ON_WM_MOVE()
	ON_WM_CLOSE()
...
/////////////////////////////////////////////////////////////////////////////
// paint_frame construction/destruction
...
int paint_frame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
BOOL paint_frame::PreCreateWindow(CREATESTRUCT& cs)
{
...
/////////////////////////////////////////////////////////////////////////////
// paint_frame diagnostics

#ifdef _DEBUG
void paint_frame::AssertValid() const
{
	SimpleFrameWnd::AssertValid();
}

void paint_frame::Dump(CDumpContext& dc) const
{
...
/////////////////////////////////////////////////////////////////////////////
// paint_frame message handlers


void paint_frame::WfiOnFullScreen(bool bFullScreen)
{
   if(bFullScreen)
   {
      ModifyStyle(
...
void paint_frame::OnViewFullScreen() 
{
	ToggleFullScreen();
}

void paint_frame::OnUpdateViewFullScreen(CCmdUI* pCmdUI) 
{
   pCmdUI->Enable();	
   pCmdUI->SetCheck(WfiIsFullScreen() ? 1 : 0);	
}

void paint_frame::ToggleFullScreen()
{
   WfiFullScreen(!WfiIsFullScreen(), true);
}

void paint_frame::OnGetMinMaxInfo(MINMAXINFO FAR* lpMMI) 
{
	if (IsFullScreen())
	{
...
bool paint_frame::IsFullScreen()
{
   return WfiIsFullScreen();
}


void paint_frame::OnMove(int x, int y) 
{
	SimpleFrameWnd::OnMove(x, y);
}

void paint_frame::OnClose() 
{
	SimpleFrameWnd::OnClose();
}

void paint_frame::OnSysCommand(UINT nID, LPARAM lParam) 
{
...
void paint_frame::OnTimer(UINT nIDEvent) 
{
   static float theta;
   if(nIDEvent == 3)
   {
...
void paint_frame::SetAnimatedStatusBarText(LPCTSTR lpcsz)
{
...
void paint_frame::AnimateStatusBar()
{
   if(m_strAnimatedStatusBarText.is_empty())
      return;
...
void paint_frame::ShowControlBars(bool bShow)
{
   UINT nShow;
   if(bShow)
   {
...
void paint_frame::OnShowWindow(BOOL bShow, UINT nStatus) 
{
...
void paint_frame::OnTogglecustomframe() 
{
	SetCustomFrame(!GetCustomFrame());
	
}

void paint_frame::OnUpdateTogglecustomframe(CCmdUI* pCmdUI) 
{
   pCmdUI->Enable();
...
BOOL paint_frame::OnCopyData(base_wnd * pwnd, COPYDATASTRUCT* pcds)
{
...
BOOL paint_frame::OnCmdMsg(UINT nID, int nCode, void* pExtra, CAFX_CMDHANDLERINFO* pHandlerInfo) 
{
...
void paint_frame::OnInitMenuPopup(CMenu* pPopupMenu, UINT nIndex, BOOL bSysMenu) 
{
...
void paint_frame::OnInitMenu(CMenu* pMenu) 
{
...
void paint_frame::OnActivate(UINT nState, base_wnd* pWndOther, BOOL bMinimized) 
{
...
void paint_frame::MouseMessage(          int nCode,
    WPARAM wparam,
    LPARAM lparam
)
{
...
LRESULT CALLBACK paint_frame::MouseProc(          int nCode,
    WPARAM wparam,
    LPARAM lparam
)
{
...
void paint_frame::OnHoverAction()
{
   ShowWindow(SW_RESTORE);
}
