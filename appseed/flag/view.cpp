...
namespace flag
{
   ...
   void view:: _001OnDraw(::ca::graphics * pdc)
   {
      ...
      dib->create((int) (pdib->width() * r1 + pdib->height() * r2 + zamp * r3 + 5), (int) (pdib->width() * r4 + pdib->height() * r5 + zamp * r6 + 5));
      int aw = pdib->width();
      int bh = pdib->height();
      int w = dib->width();
//      int h = dib->height();
      ...
      for(int a = 0; a < aw; a++)
      {
         ...
         for(int b = 0; b < bh; b++)
         {
            ...
            int yref = (int) dy;
            {
               ...
               int R = (int) (GetRValue(cr) * darken + m);
               int G = (int) (GetGValue(cr) * darken + m);
               int B = (int) (GetBValue(cr) * darken + m);
               int A = GetAValue(cr);
...
//               if((crDst & 0x00ffffff) == 0x00ffffff)
  //             {
    //              crDst = RGBA(R, G, B, A);
      //         }
        //       else
               {
                  ...
                  int A2 = GetAValue(crDst);
                  crDst = RGBA((R2 + R) / 2, (G2 + G) / 2, (B2 + B) / 2, (A + A2) / 2);
               }
               pdst[x + w * y] = crDst;
            }
            ...
            if((dy - (double) yref) >= 0.5)
            {
               ...
               //else
               {
                  ...
                  int B2 = GetBValue(crDst);
                  int A2 = GetAValue(crDst);
                  crDst = RGBA((R2 + R) / 2, (G2 + G) / 2, (B2 + B) / 2, (A2 + A) / 2);
               }
               pdst[x + w * y] = crDst;
            }
            if((dx - (double) xref) >= 0.5 &&
