...
//      SCAST_PTR(::user::win::message::create, pcreate, pobj)
   ...
   cc->m_puiParent = this;
            document * pdoc = dynamic_cast < document * > (app.devedge::application_interface::m_ptemplate_devedge->open_document_file(cc));
            if(pdoc != NULL)
            {
   ...
   cc->m_spCommandLine->m_varFile = "http://localhost:10011/";
            devedgeFrontDoc * pdoc = dynamic_cast < devedgeFrontDoc * > ((dynamic_cast < devedge::application * > ((dynamic_cast < userbase::frame_window * > (GetParentFrame()))->get_app()))->devedge::application_interface::m_ptemplateFront->open_document_file(cc));
            if(pdoc != NULL)
            {
               ::view * pview = pdoc->get_view();
               if(pview != NULL)
   ...
   cc->m_puiParent = this;
            ...
            document * pdoc = dynamic_cast < document * > (app.m_ptemplateHtmlEdit->open_document_file(cc));
            if(pdoc != NULL)
            {
               ::view * pview = pdoc->get_view();
               if(pview != NULL)
      ...
      case PaneViewSolution:
         {
            ::ca::create_context_sp cc(get_app());
            cc->m_puiParent = this;
            devedge::solution_document * pdoc = dynamic_cast < devedge::solution_document * > (app.m_ptemplate_solution->open_document_file(cc));
            if(pdoc != NULL)
            {
               ::view * pview = pdoc->get_view();
               if(pview != NULL)
   ...
   ::document * pane_view::open_file(::ca::create_context * pcreatecontext)
   {
      return ensure("devedge://" + string(pcreatecontext->m_spCommandLine->m_varFile))->m_pdoc;
   }
