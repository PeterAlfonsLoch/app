...
XfplayerOpenFileDialog::XfplayerOpenFileDialog(
		UINT nIDSTitle,
		DWORD dwFlags, base_wnd* pParentWnd)
...
BEGIN_MESSAGE_MAP(XfplayerOpenFileDialog, BaseFileDialog)
...
//xxx		papp->GetDefaultSongFolderPath(m_strInitialDir);
   ...
   strInitialDir = m_strInitialDir;
	...
	if(nResult == IDCANCEL)
	{
		...
		String strFileName = GetPathName();

		if(!strFileName.IsEmpty())
		{
			m_straFile.Add(m_strFile);

	      String strFilePath(m_strFile);
...
_AfxCommDlgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if (pThreadState->m_pAlternateWndInit != NULL && base_wnd::FromHandlePermanent(hWnd) == NULL)
	{
...
//	}
	...
	CDialog* pDlg = (CDialog*)base_wnd::FromHandlePermanent(hWnd);
	if(pDlg == NULL)
		return 0;
//	ASSERT(pDlg != NULL);
...
/*	if (pDlg->IsKindOf(RUNTIME_CLASS(BaseCommonDialog)))
	...
	// base_wnd::GetCurrentMessage and base_wnd::Default will NOT work
...
String XfplayerOpenFileDialog::GetNextPathName(POSITION& pos) const
{
   String strRet;
