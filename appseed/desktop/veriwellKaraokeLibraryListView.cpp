...
bool veriwellKaraokeLibraryListView::_001GetItemText(String &str, int iItem, int iSubItem, int iListItem)
{
   if(iItem < 0 || iItem >= m_itema.GetSize())
      return false;
   switch(iSubItem)
...
bool veriwellKaraokeLibraryListView::_001SearchGetItemText(String &wstr, int iItem, int iSubItem, int iListItem)
{
...
void veriwellKaraokeLibraryListView::OnUpdate(BaseView* pSender, LPARAM lHint, base_object* phint) 
{
   if(phint == NULL)
   {
      _001UpdateColumns();
...
bool veriwellKaraokeLibraryListView::GetSongPath(String &wstr, int iItem)
{
...
void veriwellKaraokeLibraryListView::PostFillTask(String &wstrFile, UINT uiTimer)
{
   ASSERT(FALSE);
