...
void MainFrame::OnSysCommand(UINT nID, LPARAM lParam) 
{
   ...
   {
      if(GetActiveView() != NULL
         && GetActiveView()->IsKindOf(RUNTIME_CLASS(DesktopView)))
      {
         DesktopView * pview = (DesktopView *) GetActiveView();
      }
   }
...
{
   ShowView(PaneViewDesktop);
}

void MainFrame::OnUpdateViewKaraoke(CCmdUI* pCmdUI) 
{
   pCmdUI->Enable(GetViewId() != PaneViewDesktop);
   pCmdUI->SetCheck(GetViewId() == PaneViewDesktop);
}
...
BOOL MainFrame::OnCopyData(base_wnd * pwnd, COPYDATASTRUCT* pcds)
{
   if(pcds->dwData == 123)
   {
...
BOOL MainFrame::OnCmdMsg(UINT nID, int nCode, void* pExtra, CAFX_CMDHANDLERINFO* pHandlerInfo) 
{
...
void MainFrame::OnActivate(UINT nState, base_wnd* pWndOther, BOOL bMinimized) 
{
...
LRESULT MainFrame::OnAppMessage(WPARAM wparam, LPARAM lparam)
{
   ...
   else if(wparam == 0 && lparam == 2)
   {
      return 23416234;
   }
   else if(wparam == 0 && lparam == 3)
   {
      return 11455;
   }
   else if(wparam == 0 && lparam == 4)
   {
      return 452345;
   }
   else if(wparam == 1)
   {
      SendMessageToDescendants(WM_APP + 351, 1, lparam);
   }
   else if(wparam == 2)
   {
      if(lparam)
