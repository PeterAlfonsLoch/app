...
namespace file
{
   ...
   timeout_file::timeout_file(::aura::application * papp, ::file::file_sp  pfile, uint64_t uiExpectedSize) :
      object(papp)
   {
   ...
   file_position_t timeout_file::seek(file_offset_t lOff, ::file::e_seek nFrom)
   {
   ...
   file_position_t timeout_file::get_position() const
   {
      single_lock sl((const_cast < timeout_file * > (this))->m_spmutex, TRUE);
      return m_pfile->get_position();
   }

   file_size_t timeout_file::get_length() const
   {
   ...
   file_size_t timeout_file::get_length(single_lock * psl) const
   {
   ...
   memory_size_t timeout_file::read(void *lpBuf, memory_size_t nCount)
   {
   ...
   void timeout_file::write(const void * lpBuf, memory_size_t nCount)
   {
      single_lock sl(m_spmutex, TRUE);
   ...
   void timeout_file::set_length(file_size_t dwNewLen)
   {
      single_lock sl(m_spmutex, TRUE);
