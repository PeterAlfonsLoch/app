...
static const struct { unsigned short aura; unsigned short composed; } cp1255_comp_table_data[] = {
...
struct cp1255_decomp { unsigned short composed; unsigned short aura; int comb1 : 8; signed int comb2 : 8; };
...
cp1255_mbtowc (conv_t conv, ucs4_t *pwc, const unsigned char *s, int n)
{
  ...
  if (last_wc) {
    if (wc >= 0x05b0 && wc < 0x05c5) {
      ...
      if (last_wc >= cp1255_comp_table_data[i1].aura
          && last_wc <= cp1255_comp_table_data[i2].aura) {
        unsigned int i;
        for (;;) {
          i = (i1+i2)>>1;
          if (last_wc == cp1255_comp_table_data[i].aura)
            break;
          if (last_wc < cp1255_comp_table_data[i].aura) {
            if (i1 == i)
              goto not_combining;
