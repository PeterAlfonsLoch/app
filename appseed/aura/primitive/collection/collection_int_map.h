...
public:
   ...
   int_map(sp(::aura::application) papp = NULL, ::count nBlockSize = 10);
...
int_map < VALUE, ARG_VALUE, HASH, EQUALS >::int_map(sp(::aura::application) papp, ::count nBlockSize) :
   element(papp),
...
typedef CLASS_DECL_AURA int_map < int, int > int_to_int;

typedef CLASS_DECL_AURA int_map < string, const string & > int_to_string;
