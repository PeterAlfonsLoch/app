...
static INT_PTR CALLBACK iexca2_dialog_bix_window_proc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
   ...
   case WM_PAINT:
      ...
      if( GetUpdateRect(hWnd, &pr, FALSE) )
      {
         ...
         BeginPaint(hWnd, &ps);
         p_instance->onPaint(ps.hdc, &bounds);
         EndPaint(hWnd, &ps);
      }
      return 0L;
...
namespace iexca2
{
      ...
      HDC hdcDraw, LPCRECTL lprcBounds, LPCRECTL lprcWBounds)
   {
      ...
      {
         onBarePaint(hdc, lpcrect);
      }
      else if(is_ca2_installed() && !is_installing_ca2())
      {
         onCa2Paint(hdc, lpcrect);
      }
      else
      {
         onPaint(hdc, lpcrect);
   ...
   void plugin::onPositionChange(LPCRECT lprcPosRect, LPCRECT lprcClipRect)
   {
      ...
      */
      ...
      if(m_puiHost != NULL)
      {
         ...
         m_puiHost->SetWindowPos(
            0,
            rectSite.left + rectWindow.left, 
            rectSite.top +  rectWindow.top,
   ...
   npca2::host_interaction * plugin::create_host_interaction()
   {
      npca2::host_interaction * pinteraction = new npca2::host_interaction(m_psystem);
      pinteraction->subclass_window(_inplacewnd);
      return pinteraction;
   }
