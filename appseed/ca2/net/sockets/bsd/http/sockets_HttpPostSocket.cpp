...
namespace sockets
{
      ...
      m_bMultipart(false)
   {
      ...
      {
         char ca = System.sockets().m_countHttpPostBoundary++ % 128;
         while (!isalnum(ca))
            ca = System.sockets().m_countHttpPostBoundary++ % 128;
         m_boundary += ca;
      }
      ...
      // send files
      {
         ...
         for(; pos != NULL;)
         {
            ...
            Send( tmp );
            {
               ...
               if(file->open(filename, ::file::file::type_binary | ::file::file::mode_read))
               {
