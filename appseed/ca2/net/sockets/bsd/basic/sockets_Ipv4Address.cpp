...
namespace sockets
{
   ...
   ipv4_address::ipv4_address(sp(::application) papp, port_t port) :
      element(papp),
      m_bValid(true)
   {
...
/*   ipv4_address::ipv4_address(sp(::application) papp, ipaddr_t a,port_t port) : element(papp), m_bValid(true)
   {
      memset(&m_addr, 0, sizeof(m_addr));
      m_addr.sin_family = AF_INET;
      m_addr.sin_port = htons( port );
   ...
   ipv4_address::ipv4_address(sp(::application) papp, const in_addr& a,port_t port) : element(papp), m_bValid(true)
   {
      memset(&m_addr, 0, sizeof(m_addr));
      m_addr.sin_family = AF_INET;
      m_addr.sin_port = htons( port );
   ...
   ipv4_address::ipv4_address(sp(::application) papp, const string & host,port_t port) : element(papp), m_bValid(false)
   {
      memset(&m_addr, 0, sizeof(m_addr));
      m_addr.sin_family = AF_INET;
      m_addr.sin_port = htons( port );
   ...
   ipv4_address::ipv4_address(sp(::application) papp, const sockaddr_in& sa) : element(papp)
   {
      m_addr = sa;
...
/*   bool ipv4_address::Resolve(sp(::application) papp, const string & hostname, in_addr& a)
   {
   ...
   bool ipv4_address::Reverse(sp(::application) papp, in_addr& a,string & name)
   {
   ...
   */
   ...
   /*
   string ipv4_address::Convert(sp(::application) papp, struct in_addr& a)
   {
