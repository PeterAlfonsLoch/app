...
namespace
{
  uint32 readIntegerstring(ID3_Reader& reader, size_t numBytes)
  {
    uint32 val = 0;
    char ch;
    for (size_t i = 0; i < numBytes && reader.peek(&ch) && ch >= 0 && ch < 256 && isdigit(ch); ++i)
    {
      val = (val * 10) + (reader.read(&ch) - '0');
    }
  ...
  bool isTimeStamp(ID3_Reader& reader)
  {
    ...
    char ch;
    bool its = (reader.read(&ch) && ch == '[' &&
                reader.read(&ch) && isdigit(ch) &&
                reader.read(&ch) && isdigit(ch) &&
                reader.read(&ch) && ch == ':' &&
                reader.read(&ch) && isdigit(ch) &&
                reader.read(&ch) && isdigit(ch) &&
  ...
  bool findText(ID3_Reader& reader, string text)
  {
    ...
    {
      if (ch == text.operator[](index))
      {
        index++;
      }
      else if (ch == text.operator[](0))
      {
        index = 1;
      }
      else
      ...
      if (index == natural(text.get_length()))
      {
...
bool lyr3::v2::parse(ID3_TagImpl& tag, ID3_Reader& reader)
{
  ...
  if (end < (ID3_Reader::pos_type)(reader.getBeg() + lyrSize + 6 + 9 + 128))
  {
  ...
  while (!wr.atEnd())
  {
    ...
    if (fldName == "IND")
    {
      has_time_stamps = (fldData.get_length() > 1 && fldData[1] == '1');
    }
