...
namespace 
{
  void renderFields(ID3_Writer& writer, const ID3_FrameImpl& frame)
  {
    ...
    {
      ID3_Field* fld = (ID3_Field*) frame.fields().ptr_at(i);
...
void ID3_FrameImpl::to(ID3_Writer& writer) const
{
  // Return immediately if we have no fields, which (usually) means we're
  // trying to to a frame which has been Cleared or hasn't been initialized
  if (!this->NumFields())
  {
    return;
  }
...
//  const size_t hdr_size = hdr.size();
  ...
  if (!this->GetCompression())
  {
    ...
    origSize = flds.get_length();
    ID3D_NOTICE ( "ID3_FrameImpl::to(): uncompressed fields" );
  }
  else
  {
    ...
    ID3D_NOTICE ( "ID3_FrameImpl::to(): compressed fields, orig size = " <<
                  origSize );
  }

  size_t fldSize = flds.get_length();
  ID3D_NOTICE ( "ID3_FrameImpl::to(): field size = " << fldSize );
// No need to not write is_empty frames, why would we not? They can be used to fill up padding space
// which is even recommended in the id3 spec.
//  if (fldSize == 0)
//  {
//    ID3D_WARNING ( "ID3_FrameImpl::to(): no field data" );
//    return;
//  }
  ...
  if (fldSize != 0)
  {
    ...
    if (hdr.GetEncryption())
    {
      ...
      ID3D_NOTICE( "ID3_FrameImpl::to(): frame is compressed, encryption id = " << eID );
    }
    if (hdr.GetGrouping())
    {
      writer.writeChar(gID);
      ID3D_NOTICE( "ID3_FrameImpl::to(): frame is compressed, grouping id = " << gID );
    }

    // write the field data
    writer.writeChars(flds, fldSize);
  }
  m_bChanged = false;
}
