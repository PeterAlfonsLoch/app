...
namespace
{
  bool parseFields(ID3_Reader& rdr, ID3_FrameImpl& frame)
  {
    ...
    {
      ID3_Field* fp = frame.fields().ptr_at(i);
      ++iLoop;
      ...
      { 
        ...
        if (iLoop == iFields)
        {
          ...
          //an is_empty tag used for filling up padding, it's no problem
