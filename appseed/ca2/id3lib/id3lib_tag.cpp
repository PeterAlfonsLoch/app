 ...
 ** an is_empty tag and making an explicit call to Link().
 **
 ** \code
 **   ID3_Tag myTag;
 **   myTag.Link("song.mp3");
 ...
 **   while (NULL != (myFrame = iter->get_next()))
 **   {
 ...
 **   while (NULL != (myField = iter->get_next()))
 **   {
...
/** Indicates whether the tag has been altered since the last parse, to,
 ** or update.
 **
 ...
 ** and for applications that use the Parse() and to() methods.
 **
 ...
 **
 ** When using to() to to a binary tag to a
 ** primitive::memory buffer, first use the result of this call to allocate a buffer of
 ** unsigned chars.
 **
 ** \code
 **   if (myTag.HasChanged())
 ...
 **       if (NULL != buffer)
 **       {
 **         size_t actualSize = myTag.to(buffer);
 ...
 ** \endcode
 **
 ** \param bExt Whether to to an extended header
 **/
 ...
 ** tag is attached to a file and that file is not is_empty (aside from a
 ** pre-existing tag).
 **
 ...
 ** \endcode
 **
 ** \param bPad Whether or not to the tag with padding.
 **/
 ...
 ** releasing the frame's primitive::memory when tag goes out of scope.
 **
 ...
 ** releasing the frame's primitive::memory when tag goes out of scope.
 **
 ** This method accepts responsibility for the attached frame's primitive::memory, and
...
::primitive::memory_size ID3_Tag::Parse(const uchar* buffer, size_t bytes)
{
  ID3_MemoryReader mr((char *)buffer, bytes);
  ID3_Reader::pos_type beg = mr.getCur();
  id3::v2::parse(*_impl, mr);
  return (::primitive::memory_size) (mr.getEnd() - beg);
}
...
size_t ID3_Tag::Parse(const uchar header[ID3_TAGHEADERSIZE], const uchar *buffer)
{
  ...
  buf.allocate(ID3_TagHeader::SIZE + size);
  memcpy(buf.get_data(),reinterpret_cast<const char *>(header),
             ID3_TagHeader::SIZE);
  memcpy(&buf.get_data()[ID3_TagHeader::SIZE],reinterpret_cast<const char *>(buffer), size);
  return this->Parse(buf.get_data(), buf.get_size());
}
 ...
 **/
size_t ID3_Tag::to(uchar* buffer, ID3_TagType tt) const
{
  ID3_MemoryWriter mw(buffer, (size_t) -1);
  return (::primitive::memory_size) to(mw, tt);
}

file_size ID3_Tag::to(ID3_Writer& writer, ID3_TagType tt) const
{
  ID3_Writer::pos_type beg = writer.getCur();
  if (ID3TT_ID3V2 & tt)
  {
    id3::v2::to(writer, *this);
  }
  else if (ID3TT_ID3V1 & tt)
  {
    id3::v1::to(writer, *this);
  }
 ...
 ** do not need to use the size(), to(const uchar*, size_t), or
...
::primitive::memory_size ID3_Tag::GetPrependedBytes() const
{
  return _impl->GetPrependedBytes();
}

::primitive::memory_size ID3_Tag::GetAppendedBytes() const
{
  return _impl->GetAppendedBytes();
}

file_size ID3_Tag::GetFileSize() const
{
...
const char* ID3_Tag::GetFileName() const
{
  ...
  if (fn.get_length())
  {
    ...
    memmove((char *)_tmp_filename, fn, fn.get_length());
    return _tmp_filename; //_impl->GetFileName();
  }
  else
    return NULL;
}
...
/*ID3_Frame* ID3_Tag::Find(ID3_FrameID id, ID3_FieldID fld, const wchar_t* data) const
{
  string str = string(data, ucslen(data));
  return _impl->Find(id, fld, str);
}*/
 ...
 **/
count ID3_Tag::NumFrames() const
{
  return _impl->NumFrames();
}

ID3_Frames & ID3_Tag::frames()
{
  return _impl->frames();
}
...
{
  ID3_MemoryReader mr((char *) data, ID3_TagHeader::SIZE);
 ...
 ** for freeing the frames' primitive::memory when the tag goes out of scope.
 **
 ** \code
