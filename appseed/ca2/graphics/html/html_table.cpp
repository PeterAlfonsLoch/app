   ...
   namespace impl
   {
      ...
      void table::implement_phase1(data * pdata, ::html::elemental * pelemental)
      {
         ...
         if(ptag != NULL)
         {
            ...
            {
               m_iBorder         = (float) ::ca2::str::to_uint(ptag->get_attr_value("border"));
            }

            if(ptag->get_attr_value("cellspacing").has_char())
            {
               m_iCellSpacing    = (float) ::ca2::str::to_uint(ptag->get_attr_value("cellspacing"));
            }

            if(ptag->get_attr_value("cellpadding").has_char())
            {
               m_iCellPadding    = (float) ::ca2::str::to_uint(ptag->get_attr_value("cellpadding"));
            }
