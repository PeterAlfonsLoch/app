...
{
public:
   array_app_alloc(::ax::application * papp);
   array_app_alloc();
...
array_app_alloc < TYPE, ARG_TYPE >::array_app_alloc(::ax::application * papp) :
 ::ax::ax(papp)
{
}
...
{
   TYPE * pt = dynamic_cast < TYPE * > (System.alloc(this->get_app(), ::ax::get_type_info < TYPE > ()));
   this->ptra().add(pt);
   return pt;
}
...
void array_app_alloc<TYPE, ARG_TYPE>::set_at_grow(index iIndex, ARG_TYPE t)
{
   ...
   else
   {
      ...
      {
         this->ptra().element_at(i) = dynamic_cast < TYPE * > (System.alloc(this->get_app(), ::ax::get_type_info < TYPE > ()));
      }
      this->ptra().element_at(i) = dynamic_cast < TYPE * > (System.clone(dynamic_cast < ::ax::ax * > (const_cast < TYPE * > (&t))));
   }
}
