...
//         System.simple_message_box(NULL, "Failed to open desktop folder!");
   ...
   bool _017HasSubFolder(sp(::application) papp, LPITEMIDLIST lpiidl, const char * lpcszExtra)
   {
   ...
   EFolder GetFolderType(sp(::application) papp, const char * lpcsz)
   {
      return GetFolderType(papp, ::str::international::utf8_to_unicode(lpcsz));
   }

   EFolder GetFolderType(sp(::application) papp, const wchar_t * lpcszPath)
   {
...
*/
   ...
   ImageSet::ImageSet(sp(::application) papp) :
      element(papp),
      m_mutex(papp)
   {
