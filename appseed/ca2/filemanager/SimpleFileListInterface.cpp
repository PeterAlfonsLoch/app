  ...
  /*    stringa wstraItem;
   ...
   bool SimpleFileListInterface::_001CreateImageListStep()
   {
      ...
      if(pcolumn != NULL && pcolumn->m_iSubItem == m_iNameSubItem)
      {
         ...
         item.m_iImage = System.shellimageset().GetImage(
            (oswindow_) _GetWnd()->GetTopLevelParent()->_get_handle(),
            item.m_strPath,
            NULL,
            _shell::IconNormal,
   ...
   void SimpleFileListInterface::_001InsertColumns()
   {
      ...
      if(GetFileManager()->get_filemanager_data()->m_bListSelection)
      {
         ...
         {
            column.m_pil                  = System.shellimageset().GetImageList48();
         }
         else
         {
      ...
      if(GetFileManager()->get_filemanager_data()->m_iIconSize >= 48)
      {
         column.m_pilHover             = System.shellimageset().GetImageList48Hover();
         column.m_pil                  = System.shellimageset().GetImageList48();
      }
      else
      {
