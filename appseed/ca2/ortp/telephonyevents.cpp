...
int rtp_session_send_dtmf2(RtpSession *session, char dtmf, uint32_t userts, int duration)
{
   ...
   if (m1==NULL) return -1;
   rtp_session_add_telephone_event(session,m1,(uint8_t)tev_type,0,10,(uint16_t)durationtier);
   /* create a second packet */
   m2=rtp_session_create_telephone_event_packet(session,0);
   if (m2==NULL) return -1;
   rtp_session_add_telephone_event(session,m2,(uint8_t)tev_type,0,10,(uint16_t)(durationtier+durationtier));
      
   /* create a third and final packet */
   m3=rtp_session_create_telephone_event_packet(session,0);
   if (m3==NULL) return -1;
   rtp_session_add_telephone_event(session,m3,(uint8_t)tev_type,1,10,(uint16_t)duration);
