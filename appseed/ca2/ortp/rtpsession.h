...
namespace rtp
{
   ...
   public:
      ...
      void set_scheduling_mode(int yesno = 1);
      void set_blocking_mode(int yesno = 1);
      void set_profile(profile * pprofile);
      void set_send_profile(profile * pprofile);
      void set_recv_profile(profile * pprofile);


      int set_payload_type(int pt);

      int recv_with_ts(void * buffer, int len, uint32_t ts, int *have_more);
      int send_with_ts(const void * buffer, int len, uint32_t userts);
