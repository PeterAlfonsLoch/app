...
namespace mus
{
   ...
   namespace midi
   {
      ...
      */
      ...
      VMSRESULT midi::FillTypeCombo(simple_combo_box *lpcombo)
      {
      ...
      /*   lpcombo->ResetContent();
         ...
         lpcombo->SetItemData(iItem, midi::NoteOff);

         str.load_string(IDS_MIDI_NOTEON);
         iItem = lpcombo->AddString(str);
         lpcombo->SetItemData(iItem, midi::NoteOn);

      //    string str;
        //  str.load_string(IDS_MIDI_KEYPRESSURE);
          //lpcombo->AddString(str);
          //lpcombo->SetItemData(lpcombo->get_count() - 1, midi::K);
          str.load_string(IDS_MIDI_CONTROLCHANGE);
          iItem = lpcombo->AddString(str);
          lpcombo->SetItemData(iItem, midi::ControlChange);

          str.load_string(IDS_MIDI_PROGRAMCHANGE);
          iItem = lpcombo->AddString(str);
          lpcombo->SetItemData(iItem, midi::ProgramChange);*/

          return VMSR_SUCCESS;
      }

      VMSRESULT midi::FillPitchCombo(simple_combo_box *lpcombo)
      {
         UNREFERENCED_PARAMETER(lpcombo);
      /*    lpcombo->ResetContent();
      ...
      VMSRESULT midi::SelectTypeCombo(simple_combo_box *lpcombo, midi_event_base *lpevent)
      {
         ...
         UNREFERENCED_PARAMETER(lpevent);
          /*int iCount = lpcombo->get_count();
          for(int i = 0; i < iCount; i++)
          {
              DWORD dwData = lpcombo->GetItemData(i);
              if((lpevent->GetFullType() < midi::SysEx &&
                  dwData == lpevent->get_type()) ||
                  (lpevent->GetFullType() >= midi::SysEx &&
                   dwData == lpevent->GetFullType()))
              {
                  lpcombo->set_cur_sel(i);
                  break;
              }
          }*/
          return VMSR_SUCCESS;
      ...
      VMSRESULT midi::SelectPitchCombo(simple_combo_box *lpcombo, midi_event_base *lpevent)
      {
         ...
         UNREFERENCED_PARAMETER(lpevent);
          ...
          for(int i = 0; i < iCount; i++)
          {
              int iData = lpcombo->GetItemData(i);
              if(iData == lpevent->GetNotePitch())
              {
                  lpcombo->set_cur_sel(i);
                  break;
              }
      ...
      int midi::GetMessageLen(BYTE bEvent)
      {
         return grbChanMsgLen[(bEvent >> 4) & 0x0F];
      }

      // returns the midi stream var dword
      DWORD midi::GetVDWord(byte * &hpbMidiStream, DWORD dwLeft, DWORD &dwValueParam)
      {
      ...
      MMRESULT midi::TranslateSMFResult(
            e_file_result               smfrc)
      {
          switch(smfrc)
          {
