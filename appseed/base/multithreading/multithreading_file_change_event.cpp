...
file_change_event::file_change_event(sp(::base::application) papp, const char * path, bool watchsubtree, uint32_t filter) :
	event_base( ::FindFirstChangeNotificationW(::str::international::utf8_to_unicode(path), watchsubtree, filter) )
{
	if (item() == 0)
