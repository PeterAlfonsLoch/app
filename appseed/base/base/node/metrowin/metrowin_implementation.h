...
//CLASS_DECL_BASE bool __end_defer_register_class(LONG fToRegister, const char ** ppszClass);


// Temporary ::map management (locks temp ::map on current thread)
//CLASS_DECL_BASE void ::ca2::LockTempMaps(::axis::application * papp);
//CLASS_DECL_BASE bool ::ca2::UnlockTempMaps(::axis::application * papp, bool bDeleteTemps = TRUE);
...
CLASS_DECL_BASE void __cancel_modes(oswindow hWndRcvr);
CLASS_DECL_BASE bool __help_enabled();  // determine if ID_HELP handler exists
CLASS_DECL_BASE bool __custom_log_font(UINT nIDS, LOGFONT* pLogFont);
CLASS_DECL_BASE bool __get_prop_sheet_font(string & strFace, WORD& wSize, bool bWizard);

CLASS_DECL_BASE bool __is_combo_box_control(oswindow hWnd, UINT nStyle);
CLASS_DECL_BASE bool __check_center_dialog(const char * lpszResource);
CLASS_DECL_BASE bool __compare_class_name(oswindow hWnd, const char * lpszClassName);
