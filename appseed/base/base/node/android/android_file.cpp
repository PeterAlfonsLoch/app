...
CLASS_DECL_BASE string vfxStringFromCLSID(REFCLSID rclsid)
{
   CHAR szCLSID[256];
...
/*CLASS_DECL_BASE UINT vfxGetFileName(const char * lpszPathName, char * lpszTitle, UINT nMax)
{
   ASSERT(lpszTitle == NULL ||
      __is_valid_address(lpszTitle, _MAX_FNAME));
   ASSERT(__is_valid_string(lpszPathName));
...
void CLASS_DECL_BASE vfxThrowFileException(sp(::base::application) papp, int32_t cause, LONG lOsError, const char * lpszFileName /* == NULL */)
{
#ifdef DEBUG
   const char * lpsz;
