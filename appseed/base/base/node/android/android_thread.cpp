...
struct ___THREAD_STARTUP : ::ca2::thread_startup
{
   ...
   ::android::thread* pThread;    // thread for new thread
...
__STATIC void CLASS_DECL_BASE __pre_init_dialog(sp(::user::interaction )pWnd, LPRECT lpRectOld, DWORD* pdwStyleOld);
__STATIC void CLASS_DECL_BASE __post_init_dialog(sp(::user::interaction) pWnd, const RECT& rectOld, DWORD dwStyleOld);
...
UINT APIENTRY __thread_entry(void * pParam)
{
   ...
   ::android::thread* pThread = pStartup->pThread;
...
CLASS_DECL_BASE ::android::thread * __get_thread()
{
   // check for current thread in module thread state
   __MODULE_THREAD_STATE* pState = __get_module_thread_state();
   ::android::thread* pThread = pState->m_pCurrentWinThread;
   return pThread;
}


CLASS_DECL_BASE void __set_thread(::ca2::thread * pthread)
{
   // check for current thread in module thread state
   __MODULE_THREAD_STATE* pState = __get_module_thread_state();
   pState->m_pCurrentWinThread = dynamic_cast < ::android::thread * > (pthread->::ca2::thread::m_p.m_p);
}
...
{
   android::thread * pThread = __get_thread();
   if(pThread)
...
/*thread* CLASS_DECL_BASE AfxBeginThread(sp(::base::application) papp, __THREADPROC pfnThreadProc, LPVOID pParam,
...
return false;
   ...
   CLASS_DECL_BASE ::ca2::thread * get_thread()
   {
      ::android::thread * pwinthread = __get_thread();
      if(pwinthread == NULL)
         return NULL;
...
LRESULT CLASS_DECL_BASE AfxProcessWndProcException(base_exception* e, const MESSAGE* pMsg)
{
thread *pThread = System.GetThread();
if( pThread )
...
{
::ca2::scoped_ptr < android::message > spmessage(pmsg->lParam);
spmessage->send();
return TRUE;
}
/*   const __MSGMAP* pMessageMap; pMessageMap = GetMessageMap();
...
{
   ::ca2::thread* pthread;
   if (afxContextIsDLL || (code < 0 && code != MESSAGEF_DDEMGR) || (pthread = dynamic_cast < ::ca2::thread * > (::android::get_thread())) == NULL)
   {
...
static void initialize_navigationBarImages() {
  ::ca2::g_pfn_get_thread = &::android::get_thread;
