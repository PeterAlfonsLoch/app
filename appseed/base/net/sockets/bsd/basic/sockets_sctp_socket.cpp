...
namespace sockets
{
   ...
   int32_t SctpSocket::Bind(const ::net::address & ad)
   {
      if (!ad.IsValid())
      {
         log("SctpSocket", -1, "invalid address", ::axis::log::level_error);
         return -1;
      }
      ...
      if (GetSocket() != INVALID_SOCKET)
      {
         ...
         if (n == -1)
         {
            log("SctpSocket", -1, "bind() failed", ::axis::log::level_error);
   ...
   int32_t SctpSocket::AddAddress(const ::net::address & ad)
   {
      ...
      if (GetSocket() == INVALID_SOCKET)
      {
         log("SctpSocket", -1, "AddAddress called with invalid file descriptor", ::axis::log::level_error);
         return -1;
      }
      int32_t n = sctp_bindx(GetSocket(), ad, ad, SCTP_BINDX_ADD_ADDR);
      if (n == -1)
      {
         log("SctpSocket", -1, "sctp_bindx() failed", ::axis::log::level_error);
      }
      return n;
   }
   ...
   int32_t SctpSocket::RemoveAddress(const ::net::address & ad)
   {
      ...
      if (GetSocket() == INVALID_SOCKET)
      {
         log("SctpSocket", -1, "RemoveAddress called with invalid file descriptor", ::axis::log::level_error);
         return -1;
      }
   ...
   #endif
            {
               log("connect: connection pending", Errno, StrError(Errno), ::axis::log::level_info);
               SetConnecting( true ); // this flag will control fd_set's
            }
            else
            {
               log("SctpSocket", -1, "connect() failed", ::axis::log::level_error);
            }
         }
         return n;
      }
   ...
   int32_t SctpSocket::AddConnection(const ::net::address & ad)
   {
      ...
      if (GetSocket() == INVALID_SOCKET)
      {
         log("SctpSocket", -1, "AddConnection called with invalid file descriptor", ::axis::log::level_error);
         return -1;
      }
      int32_t n = sctp_connectx(GetSocket(), ad, ad);
      if (n == -1)
      {
         log("SctpSocket", -1, "sctp_connectx() failed", ::axis::log::level_error);
      }
      else
      {
         SetConnecting();
   ...
   int32_t SctpSocket::getpaddrs(sctp_assoc_t id,list<string>& vec)
   {
      ...
      if (!n || n == -1)
      {
         log("SctpSocket", -1, "sctp_getpaddrs failed", ::axis::log::level_warning);
         return n;
      }
   ...
   int32_t SctpSocket::getladdrs(sctp_assoc_t id,list<string>& vec)
   {
      ...
      if (!n || n == -1)
      {
         log("SctpSocket", -1, "sctp_getladdrs failed", ::axis::log::level_warning);
         return n;
      }
      for (int32_t i = 0; i < n; i++)
      {
   ...
   int32_t SctpSocket::PeelOff(sctp_assoc_t id)
   {
      ...
      if (n == -1)
      {
         log("SctpSocket", -1, "PeelOff failed", ::axis::log::level_warning);
         return -1;
      }
      socket *p = create();
      p -> attach(n);
   ...
   */
      ...
      if (n == -1)
      {
         log("SctpSocket", Errno, StrError(Errno), ::axis::log::level_fatal);
         SetCloseAndDelete();
      }
      else
      {
   ...
   void SctpSocket::OnWrite()
   {
      if (Connecting())
      {
         ...
         log("sctp: connect failed", err, StrError(err), ::axis::log::level_fatal);
