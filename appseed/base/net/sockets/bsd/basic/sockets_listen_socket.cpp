...
namespace sockets
{
   ...
   int32_t listen_socket_base::Bind(const string & intf,port_t port,int32_t depth)
   {
      ...
      log("Bind", 0, "name resolution of interface name failed", ::axis::log::level_fatal);
      return -1;
   }
   ...
   int32_t listen_socket_base::Bind(const ::net::address & ad,const string & protocol,int32_t depth)
   {
      ...
      {
         log("bind() failed for port " + ::str::from(ad.get_service_number()), Errno, StrError(Errno), ::axis::log::level_fatal);
         close_socket(s);
         return -1;
      }
      if (listen(s, depth) == -1)
      {
         log("listen", Errno, StrError(Errno), ::axis::log::level_fatal);
         close_socket(s);
   ...
   void listen_socket_base::OnRead()
   {
      ...
      {
         log("accept", Errno, StrError(Errno), ::axis::log::level_error);
         return;
      }
      if (!Handler().OkToAccept(this))
      {
         log("accept", -1, "Not OK to accept", ::axis::log::level_warning);
         close_socket(a_s);
         return;
      }
      if (Handler().get_count() >= FD_SETSIZE)
      {
         log("accept", (int32_t)Handler().get_count(), "base_socket_handler fd_set limit reached", ::axis::log::level_fatal);
         close_socket(a_s);
         return;
      }
