...
namespace file
{
   ...
   namespace dir
   {
      ...
      {
         m_pbaseapp->m_pbasesystem->m_spdir->matter_ls(get_app(), str, stra);
      }
      ...
      string application::matter_file(const char * lpcsz, const char * lpcsz2, bool bDir)
      {
         ...
         string strPath = m_pbaseapp->m_pbasesystem->m_spdir->matter(get_app(), lpcsz, lpcsz2, bDir);

         if(strPath.begins_ci("http://") || strPath.begins_ci("https://"))
         {
            string strFileExists = strPath;
            strFileExists.replace(":", "_");
            strFileExists = m_pbaseapp->m_pbasesystem->m_spdir->appdata("cache/" + strFileExists + ".exists_question");
      ...
      {
         return m_pbaseapp->m_pbasesystem->m_spdir->usersystemappdata(get_app(), lpcszPrefix, lpcsz, lpcsz2);
      }

      string application::userappdata(const char * lpcsz, const char * lpcsz2)
      {
         return m_pbaseapp->m_pbasesystem->m_spdir->userappdata(get_app(), lpcsz, lpcsz2);
      }

      string application::userdata(const char * lpcsz, const char * lpcsz2)
      {
         return m_pbaseapp->m_pbasesystem->m_spdir->userdata(get_app(), lpcsz, lpcsz2);
      }

      string application::userfolder(const char * lpcsz, const char * lpcsz2)
      {
         return m_pbaseapp->m_pbasesystem->m_spdir->userfolder(get_app(), lpcsz, lpcsz2);
      }

      string application::default_os_user_path_prefix()
      {
         return m_pbaseapp->m_pbasesystem->m_spdir->default_os_user_path_prefix(get_app());
      }

      string application::default_userappdata(const char * lpcszPrefix, const char * lpcszLogin , const char * pszRelativePath)
      {
         return m_pbaseapp->m_pbasesystem->m_spdir->default_userappdata(get_app(), lpcszPrefix, lpcszLogin, pszRelativePath);
      }

      string application::default_userdata(const char * lpcszPrefix, const char * lpcszLogin, const char * pszRelativePath)
      {
         return m_pbaseapp->m_pbasesystem->m_spdir->default_userdata(get_app(), lpcszPrefix, lpcszLogin, pszRelativePath);
      }

      string application::default_userfolder(const char * lpcszPrefix, const char * lpcszLogin, const char * pszRelativePath)
      {
         return m_pbaseapp->m_pbasesystem->m_spdir->default_userfolder(get_app(), lpcszPrefix, lpcszLogin, pszRelativePath);
      }

      string application::userquicklaunch(const char * pszRelativePath, const char * lpcsz2)
      {
         return m_pbaseapp->m_pbasesystem->m_spdir->userquicklaunch(get_app(), pszRelativePath, lpcsz2);
      }

      string application::userprograms(const char * pszRelativePath, const char * lpcsz2)
      {
         return m_pbaseapp->m_pbasesystem->m_spdir->userprograms(get_app(), pszRelativePath, lpcsz2);
      }
      ...
      {
         return m_pbaseapp->m_pbasesystem->m_spdir->pathfind(pszEnv, pszTopic, pszMode, m_pbaseapp);
      }
