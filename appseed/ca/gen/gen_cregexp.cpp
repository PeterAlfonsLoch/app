...
SRegInfo *next, *temp;
  ...
  retPos = 0;
  if (!gen::str::ilen(expr)) return EOK;
  retPos = -1;
...
//          delete br_name;
    ...
    if (expr[i] == '('){
      ...
      if (gen::str::ilen(expr) >= i+2 && expr[i+1] == '?' && expr[i+2] == ':'){
        next->op = ReNamedBrackets;
        next->param0 = -1;
        namedBracket = true;
        i += 3;
      } else if (gen::str::ilen(expr) > i+2 && expr[i+1] == '?' && expr[i+2] == '{'){
        // named bracket
        next->op = ReNamedBrackets;
        namedBracket = true;
        string br_name;
...
//      next->op = (exprn[i] == ReEnumS) ? ReEnum : ReNEnum;
  ...
  if (retPos == -1) retPos = gen::str::ilen(expr);
  return EOK;
};
...
const string &pattern = global_pattern;
  ...
  switch(symb){
    ...
    case ReEoL:
      ...
      return (end == toParse);
    case ReDigit:
      if (toParse >= end || !gen::ch::is_digit(&((const char *)pattern)[toParse])) return false;
      toParse++;
      return true;
    case ReNDigit:
      if (toParse >= end || gen::ch::is_digit(&((const char *)pattern)[toParse])) return false;
      ...
      return true;
    case ReWordSymb:
      if (toParse >= end || !(gen::ch::is_letter_or_digit(&((const char *)pattern)[toParse])
    ...
    case ReNWordSymb:
      ...
      return true;
    case ReWSpace:
      if (toParse >= end || !gen::ch::is_whitespace(&((const char *)pattern)[toParse])) return false;
      toParse++;
      return true;
    case ReNWSpace:
      if (toParse >= end || gen::ch::is_whitespace(&((const char *)pattern)[toParse])) return false;
      ...
      return true;
    case ReUCase:
      if (toParse >= end || !gen::ch::is_upper_case(&((const char *)pattern)[toParse])) return false;
      toParse++;
      return true;
    case ReNUCase:
      if (toParse >= end || !gen::ch::is_lower_case(&((const char *)pattern)[toParse])) return false;
      toParse++;
      return true;
    case ReWBound:
      return isWordBoundary(toParse);
    ...
    case RePreNW:
      ...
      return toParse == 0 || !gen::ch::is_letter(&((const char *)pattern)[toParse-1]);
#ifdef COLORERMODE
    case ReSoScheme:
  ...
  while(re){
    switch(re->op){
      ...
      case ReSymb:
        if (toParse >= end) return false;
        if (ignoreCase){
          if (gen::ch::to_lower_case(&((const char *)pattern)[toParse]) != gen::ch::to_lower_case(*re->un.symbol) &&
             gen::ch::to_upper_case(&((const char *)pattern)[toParse]) != gen::ch::to_upper_case(*re->un.symbol))
            return false;
        }else if (gen::str::utf8_char(&((const char *)pattern)[toParse]) != *re->un.symbol) return false;
        toParse++;
        break;
      case ReMetaSymb:
...
inline bool cregexp::quickCheck(strsize toParse)
{
  if (firstChar != -1){
    if (toParse >= end) return false;
    if (ignoreCase){
      if (gen::ch::to_lower_case(&((const char *)global_pattern)[toParse]) != gen::ch::to_lower_case(firstChar)) return false;
    }else
