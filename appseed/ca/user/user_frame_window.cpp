...
void frame_window::ExitHelpMode()
{
   ...
   sp(frame_window) pFrameWnd = GetTopLevelFrame();
...
int32_t frame_window::OnCreateHelper(LPCREATESTRUCT lpcs, sp(::ca::create_context) pContext)
{
   ...
   {
      TRACE(::ca::trace::category_AppMsg, 0, "Failed to create client pane/::view for frame.\n");
      return -1;
   }
...
/*         if (wParam == __IDS_SCCLOSE && m_lpfnCloseProc != ::null())
      ...
      sp(frame_window) pFrameWnd = pMessageBar->GetParentFrame();
      if (pFrameWnd != ::null())
      {
...
sp(frame_window) frame_window::GetActiveFrame()
{
