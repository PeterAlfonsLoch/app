...
Fixed memoryleak (unref arrayClass at deconstructor of JS tinyjs)
...
/*string &gen::str::itoa(int intData, const string &inString=string());
string &gen::str::itoa(int intData, const string &inString) {
...
CScriptException::CScriptException(const string &exceptionText, strsize Pos)
{
   text = exceptionText;
   pos = Pos;
}
...
void CScriptLex::reset(strsize toPos)
{
...
void CScriptLex::match(int expected_tk)
{
   ...
   if (tk!=expected_tk) {
      _template_std_ostringstream errorString;
...
string CScriptLex::getTokenStr(int token)
{
   ...
   _template_std_ostringstream msg;
   msg << "?[" << token << "]";
   return msg.str();
}
...
string CScriptLex::getSubString(strsize lastPosition) {
   strsize lastCharIdx = tokenLastEnd+1;
...
CScriptLex *CScriptLex::getSubLex(strsize lastPosition)
{
   strsize lastCharIdx = tokenLastEnd+1;
   if (lastCharIdx < dataEnd)
...
CScriptVar *CScriptVar::getParameter(index Idx)
{
   CScriptVar *arguments = findChildOrCreate(TINYJS_ARGUMENTS_VAR)->var;
   return arguments->findChildOrCreate(gen::str::itoa(Idx))->var;
}

int64_t CScriptVar::getParameterLength()
{
   CScriptVar *arguments = findChildOrCreate(TINYJS_ARGUMENTS_VAR)->var;
...
{
   SCRIPTVAR_CHILDS::pair * ppair = Childs.PGetFirstAssoc();
   while(ppair != NULL)
   {
      delete ppair->m_value;
      ppair = Childs.PGetNextAssoc(ppair);
   }
   Childs.remove_all();
}

CScriptVar *CScriptVar::getArrayIndex(index idx)
{
   CScriptVarLink *link = findChild(gen::str::itoa(idx));
   if (link) return link->var;
   else return new CScriptVar(TINYJS_BLANK_DATA, SCRIPTVAR_NULL); // undefined
}

void CScriptVar::setArrayIndex(index idx, CScriptVar *value)
{
   string sIdx;
   CScriptVarLink *link = findChild(gen::str::itoa(sIdx, idx));
...
count CScriptVar::getArrayLength()
{
   int highest = -1;
   if (!isArray()) return 0;
   ...
   while(ppair != NULL)
   {
      if (isNumber(ppair->m_key))
      {
         int val = atoi(ppair->m_key);
         if(val > highest)
            highest = val;
      }
      ppair = Childs.PGetNextAssoc(ppair);
...
int64_t CScriptVar::getInt() {
   ...
   if (isDouble()) return (int64_t)doubleData;
   if (isString()) return gen::str::atoi64(data.c_str());
   return 0;
}
...
double CScriptVar::getDouble() {
   ...
   if (isInt() || isBool()) return (double) intData;
...
const string &CScriptVar::getString() {
   ...
   * I should really just use char* :) */
   ...
   if (isInt()) {
      return gen::str::itoa(data, intData);
   }
   if (isDouble()) {
      char buffer[32];
...
{
   int64_t dai = a->getInt();// use int when needed
   int64_t dbi = b->getInt();
...
void CScriptVar::copyValue(CScriptVar *val) {
   if (val) {
      ...
      while(ppair != NULL)
      {
         ...
         if (ppair->m_key != TINYJS_PROTOTYPE_CLASS)
            copied = ppair->m_value->var->deepCopy();
         else
            copied = ppair->m_value->var;

         addChild(ppair->m_key, copied);
         ppair = Childs.PGetNextAssoc(ppair);
      }
   } else {
      setUndefined();
...
void CScriptVar::trace(const char * name)
{
   string str;
   trace(str, name);
}
void CScriptVar::trace(string & indentStr, const char * name) {
   string indent = "  ";
   if(recursionSet) {
      if(recursionFlag) {
   ...
   while(ppair != NULL)
   {
      ppair->m_value->var->trace(indentStr, ppair->m_key);
      ppair = Childs.PGetNextAssoc(ppair);
   }
   if(recursionSet) {
...
string CScriptVar::getParsableString()
{
   ...
   if (isNumeric())
      return getString();
   if (isFunction()) {
      _template_std_ostringstream funcStr;
      funcStr << "function (";
      // get list of parameters
      count count = Childs.size();
      SCRIPTVAR_CHILDS::pair * ppair = Childs.PGetFirstAssoc();
      while(ppair != NULL)
      {
         funcStr << ppair->m_key;
...
void CScriptVar::getJSON(_template_std_ostringstream &destination, const string linePrefix)
{
   if (isObject()) {
      ...
      while(ppair != NULL)
      {
         ...
         if (isAlphaNum(ppair->m_key))
            destination  << ppair->m_key;
         else
            destination  << getJSString(ppair->m_key);
         destination  << " : ";
         ppair->m_value->var->getJSON(destination, indentedLinePrefix);
   ...
   if(internalRefs)
   {
      ...
      if(internalRefs==0) {
         ...
         if(recursionSet->recursionSet.size())
         {
            ...
            while(ppair != NULL)
            {
               if(ppair->m_value->var->recursionSet == recursionSet)
                  ppair->m_value->var->unrefInternal();
               ppair = Childs.PGetNextAssoc(ppair);
            }
            ret = recursionSet;
         } else
   ...
   if(recursionSet) { // this Var is in a Set
      ...
      if(Owner && Owner->recursionSet == recursionSet) { // internal Ref
         if(internalRefs==1) {
            if(refs) {
               ...
               if(old_set) { // we have breaked a recursion bu the recursion Set is not destroyed
                  ...
                  for(int i = 0; i < old_set->recursionSet.size(); ++i)
                  {
                     old_set->recursionSet[i]->internalRefs = 0;
                     old_set->recursionSet[i]->recursionSet = 0;
                  }
                  // now we can redetect recursions
                  for(int i = 0; i < old_set->recursionSet.size(); ++i)
                  {
                     if(old_set->recursionSet[i]->recursionSet == 0)
                        old_set->recursionSet[i]->recoursionCheck();
                  }
                  delete old_set;
               }
         ...
         if(recursionSet->sumRefs==recursionSet->sumInternalRefs) {
            ...
            Childs.clear();
            SCRIPTVAR_CHILDS::pair * ppair = copyOfChilds.PGetFirstAssoc();
            while(ppair != NULL)
            {
               delete ppair->m_value;
               ppair = copyOfChilds.PGetNextAssoc(ppair);
            }
            copyOfChilds.clear();
         } // otherwise nothing to do
      }
...
index find_last(RECURSION_VECT &Vector, const CScriptVar * Key)
{
   for(index i = Vector.get_upper_bound(); i>= 0; i--)
   {
      if(Vector[i]==Key)
         return ++i;
   }
...
void CScriptVar::recoursionCheck(RECURSION_VECT &recursionPath)
{
   if(recursionFlag || (recursionSet && recursionSet->recursionPathBase)) { // recursion found - create a new set
      ...
      for(; it < new_set->recursionSet.get_size(); it++) {
         if(new_set->recursionSet[it]->recursionSet) {											// insert an existing Set
            RECURSION_SET_VAR *old_set = new_set->recursionSet[it]->recursionSet;
            ...
            new_set->recursionSet[it]->internalRefs++;											// over an internal reference
            // insert all Vars of the old Set in the new Set
            for(int set_it = 0; set_it < old_set->recursionSet.size(); ++set_it) {
               new_set->recursionSet.insert(old_set->recursionSet[set_it]);	// insert this Var to the Set
               old_set->recursionSet[set_it]->recursionSet = new_set;		// the Var is now in a Set
            }
            delete old_set;
         } else {											// insert this Var in the new set
            new_set->sumInternalRefs++;			// a new Var in a Set is linked
            new_set->recursionSet[it]->internalRefs++;					// over an internal reference
            new_set->sumRefs += new_set->recursionSet[it]->refs;		// adds the Var refs to the Set refs
            new_set->recursionSet.insert(new_set->recursionSet[it]);	// insert this Var to the Set
            new_set->recursionSet[it]->recursionSet = new_set;		// the Var is now in a Set
            new_set->recursionSet[it]->recursionFlag = 0;				// clear the Var-recursionFlag, because the Var in now in a Set
         }
      }
      ...
      for(int i = 0; i < Set.get_size(); i++) {
         SCRIPTVAR_CHILDS::pair * ppair = Set[i]->Childs.PGetFirstAssoc();
         while(ppair != NULL)
         {
            if(ppair->m_value->var->recursionSet != recursionSet)
               ppair->m_value->var->recoursionCheck(recursionPath);
            ppair = Set[i]->Childs.PGetNextAssoc(ppair);
         }
      }
...
tinyjs::tinyjs(::ca::application * papp, bool TwoPass) :
   ca(papp)
{
   twoPass=TwoPass;
   funcOffset = 0;
   ...
   exceptionVar = NULL;
   addNative("function eval(jsCode)", this, &tinyjs::scEval); // execute the given string and return the result
}
...
void tinyjs::throwError(bool &execute, const string &message, strsize pos /*=-1*/)
{
...
void tinyjs::parseFunctionArguments(CScriptVar *funcVar) {
   ...
   while (l->tk!=')')
   {
      arguments->addChild(gen::str::itoa(idx++), new CScriptVar(l->tkStr));
      l->match(LEX_ID);
      if (l->tk!=')') l->match(',');
   }
   l->match(')');
...
void tinyjs::addNative(const string &funcDesc, NativeFncBase *ptr, void *userdata) {
   CScriptVar *funcVar = addNative(funcDesc);
   funcVar->setCallback(ptr, userdata);
}
CScriptVar *tinyjs::addNative(const string &funcDesc) {
   CScriptLex *oldLex = l;
...
CScriptVarSmartLink tinyjs::parseFunctionDefinition() {
   // actually parse a function...
   strsize oldFuncOffset = funcOffset;
   ...
   if(IS_RUNTIME_PASS_TWO_2) {
      ...
      strsize funcBegin = l->tokenStart;
...
CScriptVarSmartLink tinyjs::factor(bool &execute) {
   if (l->tk==LEX_R_TRUE) {
      l->match(LEX_R_TRUE);
      ...
      while (l->tk=='(' || l->tk=='.' || l->tk=='[') {
         ...
         if (l->tk=='(') { // ------------------------------------- Function Call
            if (execute) {
               ...
               if(__locale__) {
                  SCRIPTVAR_CHILDS::pair * ppair = __locale__->var->Childs.PGetFirstAssoc();
                  while(ppair != NULL)
                  {
                     functionRoot->var->addChild(ppair->m_key , ppair->m_value->var->deepCopy());
                     ppair = __locale__->var->Childs.PGetNextAssoc(ppair);
                  }
               }
               // grab in all parameters
               CScriptVarLink *arguments_proto = a->var->findChild("arguments");
               count length_proto = arguments_proto ? arguments_proto->var->getChildren() : 0;
               CScriptVar *arguments = new CScriptVar(TINYJS_BLANK_DATA, SCRIPTVAR_OBJECT);
               index arguments_idx = 0;
               for(; l->tk!=')' || arguments_idx<length_proto; ++arguments_idx) {
                  ...
                  if (execute) {
                     string arguments_idx_str; gen::str::itoa(arguments_idx_str, arguments_idx);
   ...
   if (l->tk=='[') {
      ...
      while (l->tk != ']') {
         CScriptVarSmartLink a = assignment(execute);
         if (execute) {
            contents->addChild(gen::str::itoa(idx), a->var);
         }
         // no need to clean here, as it will definitely be used
         if (l->tk != ']') l->match(',');
         idx++;
...
//      CScriptVarLink *anonymous = scopes.back()->findChild(TINYJS_ANONYMOUS_VAR);
      string anonymous_name = gen::str::itoa(l->getDataPos()-this->funcOffset);
      CScriptVarSmartLink funcVar = parseFunctionDefinition();
      this->runtimeFlags;
      //				funcName = anonymous ? gen::str::itoa(l->tokenStart) : l->tkStr;
...
CScriptVarSmartLink tinyjs::unary(bool &execute) {
   CScriptVarSmartLink a;
   if (l->tk=='-') {
      l->match('-');
      a = unary(execute);
...
// L->R: Precedence 5
CScriptVarSmartLink tinyjs::term(bool &execute) {
...
// L->R: Precedence 6
CScriptVarSmartLink tinyjs::expression(bool &execute) {
...
CScriptVarSmartLink tinyjs::binary_shift(bool &execute) {
...
CScriptVarSmartLink tinyjs::relation(bool &execute, int set, int set_n) {
...
CScriptVarSmartLink tinyjs::logic_binary(bool &execute, int op, int op_n1, int op_n2) {
...
CScriptVarSmartLink tinyjs::logic(bool &execute, int op, int op_n) {
...
// R->L: Precedence 15
CScriptVarSmartLink tinyjs::condition(bool &execute)
{
...
// R->L: Precedence 16
CScriptVarSmartLink tinyjs::assignment(bool &execute) {
...
// L->R: Precedence 17
CScriptVarSmartLink tinyjs::base(bool &execute) {
   CScriptVarSmartLink a;
   for(;;)
   {
...
CScriptVarSmartLink tinyjs::statement(bool &execute) {
   ...
   if (l->tk=='{') {
      ...
      * set and then we parse as if we're doing a normal equals.*/
      ...
      if(IS_RUNTIME_PASS_TWO_1)
      {
         ...
         strsize whileBodyStart = l->tokenStart;
         strsize whileCondStart = 0;
      ...
      if(IS_RUNTIME_PASS_TWO_1) {
         ...
         strsize whileCondStart = l->tokenStart;
         bool noexecute = false;
         CScriptVarSmartLink cond = base(execute);
      ...
      if(for_each_in && !for_in)
         ...
         if(IS_RUNTIME_PASS_TWO_1)
            ...
            strsize forBodyStart = l->tokenStart;
            bool old_execute = execute;
            int old_loop_runtimeFlags = runtimeFlags & RUNTIME_LOOP_MASK;
            runtimeFlags = (runtimeFlags & ~RUNTIME_LOOP_MASK) | RUNTIME_CANBREAK | RUNTIME_CANCONTINUE;
            SCRIPTVAR_CHILDS::pair * ppair = for_in_var->var->Childs.PGetFirstAssoc();
            while(ppair != NULL)
            {
               if (for_each_in)
                  for_var->replaceWith(ppair->m_value->var);
               else
                  for_var->replaceWith(new CScriptVar(ppair->m_key));
               ...
               ppair = for_in_var->var->Childs.PGetNextAssoc(ppair);
            }
            runtimeFlags = (runtimeFlags & ~RUNTIME_LOOP_MASK) | old_loop_runtimeFlags;
         }
      } else {
         ...
         //l->match(';');
         bool noexecute = false;
         strsize forCondStart = l->tokenStart;
         ...
         {
            iter_empty = false;
            strsize forIterStart = l->tokenStart;
      ...
      strsize tokenStart = l->tokenStart;
      l->match(LEX_R_THROW);
...
void tinyjs::scEval(CScriptVar *c, void *data)
{
