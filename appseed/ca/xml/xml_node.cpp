...
namespace xml
{
   ...
   node::base_array::base_array(::ca::application * papp) :
      ca(papp)
   {
   }
   ...
   */
      ...
      {
         return m_nodea.m_ptra.find_first(pnode);
      }
   ...
   char * node::LoadDocType( const char * pszXml, parse_info * pparseinfo)
   {
      ...
      {
         gen::str::consume_spaces(pszXml);
         gen::str::consume_quoted_value(pszXml);
      }
      else if(gen::str::begins_consume(pszXml, "PUBLIC"))
      {
         gen::str::consume_spaces(pszXml);
         gen::str::consume_quoted_value(pszXml);
         gen::str::consume_spaces(pszXml);
      ...
      if(*pszXml == '[')
      {
         ...
         while(*pszXml != ']')
         {
            if(gen::str::begins_consume(pszXml, "<!ENTITY"))
            {
               gen::str::consume_spaces(pszXml);
               string entity_name = gen::str::consume_nc_name(pszXml);
               gen::str::consume_spaces(pszXml);
               string entity_value;
               ...
               {
                  gen::str::consume_spaces(pszXml);
                  ext_entity_value = gen::str::consume_quoted_value(pszXml);
               }
               else if (gen::str::begins_consume(pszXml, "PUBLIC"))
               {
                  gen::str::consume_spaces(pszXml);
               ...
               {
                  entity_value = gen::str::consume_quoted_value(pszXml);
               }
            ...
            else if(gen::str::xml_is_comment(pszXml))
            {
               gen::str::xml_consume_comment(pszXml);
            }
            pszXml++;
         }
         gen::str::consume(pszXml, "]");
   ...
   char * node::LoadOtherNodes(bool* pbRet, const char * pszXml, parse_info * pparseinfo)
   {
      ...
      *pbRet = false;
      ...
      while( xml && do_other_type )
      {
         ...
         {
            // is DOCTYPE
            if(gen::str::begins(xml, szXMLDOCTYPEOpen))
            {
               // processing instrunction parse
               // return pointer is next node of pparseinfo
         ...
         //                        ^- current pointer
         {
            ...
            if(gen::str::trimmed_is_empty(m_strValue))
            {
               ...
               while(*pEnd != '<' && *pEnd != '\0')
               {
                  ...
                  {
                     pEnd = (char *) gen::str::utf8_inc(pEnd);
                  }
               }
