...
namespace webserver
{
   ...
   {
      m_iLastServer = 0;
      m_pmusicdatabasecache = new music::database_cache();
   }
   ...
   bool fontopus_database::initialize(int iServer)
   {
      ...
      if(iServer == 0)
      {
            ...
            "fontopus"))
         {
            if(Application.m_strDatabaseServerHost == "localhost"
               ...
               "fontopus"))
            {
               if(Application.m_strDatabaseServerHost == "ca2.cc"
                  || !connect(
                  "ca2.cc",
                  "fontopus_web",
                  "5dCAzq43",
      ...
      else if(iServer == 1)
      {
         if(!connect(
            ...
            "fontopus"))
         {
            if(Application.m_strDatabaseServerHost == "fontopus.com"
               || !connect(
               "fontopus.com",
               "fontopus_web",
               "5dCAzq43",
      ...
      else if(iServer == 2)
      {
            ...
            "fontopus"))
         {
            if(Application.m_strDatabaseServerHost == "localhost"
               ...
               "gustavocc"))
            {
               if(Application.m_strDatabaseServerHost == "carlosgustavocecynlundgrenamacamilosasuketsumanuma.us.ca2.se"
                  || !connect(
   ...
   bool fontopus_database::get_session(const char * pszId, gen::property_set & set)
   {
      ...
      if(var.is_new())
         return false;
      System.base64().unserialize(set, var.get_string());
      return true;
   }
   ...
   bool fontopus_database::set_session(const char * pszId, gen::property_set & set)
   {
      ...
      until_here();
      bool bOk = webserver::database::query(strSql) != NULL;
   ...
   bool fontopus_database::secure_email_available(const char * pszEmail)
   {
      ...
      string strSql;
      strSql.Format("SELECT COUNT(*) FROM fun_user WHERE email = '%s' AND register = 1", pszEmail);
      var var = query_item(strSql);
      if(!var.is_set())
         return false;
      ...
      const char * pszRuri)
   {
      ...
      string strOneNessie(pszOneNessie);
      strOneNessie.make_lower();
      string strPass = System.crypt().v5_get_password_hash(strOneNessie);
      ...
      strSql.Format("REPLACE INTO fun_user (`firstname`, `lastname`, `login`, `email`, `password`, `hash`, `requestregisterdatetime`, `register`, `userlevel`, `registerdatetime`) VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')",
      ...
      var flaga)
   {
      ...
      */
      ...
      if(defaultsql)
      {
         ...
         {
            sql = sql + addwhereclause;
            sqlcount = sqlcount + addwhereclause;
         }
   ...
   music::song * fontopus_database::get_song(var id)
   {
      ...
      string sql = eval("SELECT song.id, song.title, SUM(tmp_song.lyricscount), SUM(tmp_song.chordcount), song_info.releaseyear, SUM(tmp_song.karaokecount) FROM song INNER JOIN tmp_song ON tmp_song.song = song.id LEFT OUTER JOIN song_info ON song_info.songid = song.id WHERE song.id = $id GROUP BY song.id");
      ...
      psong->m_propertyset["title"]          = row.at(1);
      //psong->m_propertyset["click"]          = row.at(2);
      psong->m_propertyset["lyricscount"]      = row.at(2);
      psong->m_propertyset["chordcount"]      = row.at(3);
      psong->m_propertyset["releaseyear"]      = row.at(4);
      psong->m_propertyset["karaokecount"]   = row.at(5);
   ...
   mysql::result * fontopus_database::query(const char * pszSql)
   {
      ...
      mysql::result * presult = webserver::database::query(pszSql);
   ...
   var fontopus_database::query_item(const char * pszSql, var varDefault)
   {
      ...
      var var = webserver::database::query_item(pszSql, varDefault);
   ...
   var fontopus_database::query_items(const char * pszSql)
   {
      ...
      var var = webserver::database::query_items(pszSql);
   ...
   var fontopus_database::query_row(const char * pszSql)
   {
      ...
      var var = webserver::database::query_row(pszSql);
   ...
   var fontopus_database::query_rows(const char * pszSql)
   {
      ...
      var var = webserver::database::query_rows(pszSql);
   ...
   ::webserver::music::song_source * fontopus_database::get_song_source(var sourceid)
   {
      string sql = "SELECT name, url, email FROM song_source WHERE id = " + sourceid;
      var row = query_row(sql);
      if(row.array_get_count() == 3)
      {
         ::webserver::music::song_source * psource = new music::song_source;
   ...
   */
   ...
   var fontopus_database::get_song_classes_for_performers(var song)
   {
      ...
      var rows = query_items(sql);
      var classa;
      for(index i = 0; i < rows.array_get_count(); i++)
      {
         string strClass = rows.at(i);
         if(strClass.has_char())
         {
            ::webserver::music::category * pcategory = get_class_for_performers(1, "", strClass);
            if(pcategory != NULL)
            {
               classa.vara().add(var(pcategory));
            }
         }
      }
      return classa;
   ...
   music::category * fontopus_database::get_class_for_performers(int level, string order, string classa)
   {
      ...
      string sql = eval("SELECT DISTINCT class.id, class.namestringid, COUNT(DISTINCT song_performer.performerid) AS performercount, COUNT(song.id) as songcount, parentid FROM song INNER JOIN topic_class ON topic_class.topic = song.id INNER JOIN class ON topic_class.class = class.id INNER JOIN song_performer ON song_performer.songid = song.id LEFT OUTER JOIN class_language ON (class.id = class_language.classid AND class_language.languageid = $g_languageid) GROUP BY class.id, class.namestringid HAVING class.id = "+classid+" ORDER BY ISNULL(class_language.listorder), class_language.listorder, class.id");

      var row = query_row(sql);

      ::webserver::music::category * pclass = NULL;
   ...
   var fontopus_database::get_classes_for_performers(int level,  string order, string parentclass, int topictype)
   {
      ...
      for(index i = 0; i < rows.array_get_count(); i++)
      {
         ...
         ::webserver::music::category * pclass                   = new music::category();
         pclass->m_id               = classid;
   ...
   music::database_cache & fontopus_database::cache()
   {
   ...
   bool fontopus_database::is_karfile_licensed(__int64 iKarFileId)
   {
      ...
      if(row.is_empty())
         return false;
      if(row.at(1).get_string().get_length() < 16) // heuristical, nessie is not valid, song path information is incomplete
      {
         ...
         string strHash = System.file36().nessie(varFile);
         __int64 len = System.file().length(varFile);
         string strMimeType = psong->calc_content_type();
         string strExtension = System.file().extension(varFile);

         strSql.Format("UPDATE karfile SET nessie = '%s', `key` = '%d', `size` = '%I64d', mimetype = '%s', extension = '%s' WHERE id = %I64d LIMIT 1", strHash, 1, len, strMimeType, strExtension, iKarFileId);

         if(query(strSql) == NULL)
            return false;
         strSql.Format("SELECT filename, nessie, `key`, `size`, mimetype, extension FROM karfile WHERE id = %I64d", iKarFileId);
         row = query_row(strSql);
         if(row.is_empty())
            return false;
      }
