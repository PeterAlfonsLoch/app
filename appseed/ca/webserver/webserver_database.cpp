...
namespace webserver
{
   ...
   mysql::result * database::query(const char * pszSql)
   {
      ...
      dprint("<h4>webserver::database::query</h4>");
      dprint(pszSql);
      dprint("<br/>");
      dprint("<h5>query start</h5>");
      until_here();
      prof_enter("webserver::database::query");
      ...
      prof_leave("webserver::database::query");
      until_here();
   ...
   {
      dprint("webserver::database::query_item");
      prof_enter("webserver::database::query_item");
      var var = mysql::database::query_item(pszSql, varDefault);
      prof_leave("webserver::database::query_item");
      return var;
   }

   var database::query_items(const char * pszSql)
   {
      dprint("webserver::database::query_items");
      prof_enter("webserver::database::query_items");
      var var = mysql::database::query_items(pszSql);
      prof_leave("webserver::database::query_items");
      return var;
   }

   var database::query_row(const char * pszSql)
   {
      dprint("webserver::database::query_row");
      prof_enter("webserver::database::query_row");
      var var = mysql::database::query_row(pszSql);
      prof_leave("webserver::database::query_row");
      return var;
   }
