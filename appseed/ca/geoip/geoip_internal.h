...
GEOIP_API uint32_t _GeoIP_seek_record (GeoIP *gi, uint32_t long ipnum);
GEOIP_API uint32_t long _GeoIP_addr_to_num (const char *addr);
...
GEOIP_API uint32_t long _GeoIP_lookupaddress (const char *host);
