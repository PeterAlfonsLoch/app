 ...
 *      time, but has a larger average cost, uses more primitive::memory and is patented.
...
typedef block_state (*compress_func) OF((deflate_state *s, int32_t flush));
/* Compression function. Returns the block state after the call. */

zlib_local void fill_window    OF((deflate_state *s));
zlib_local block_state deflate_stored OF((deflate_state *s, int32_t flush));
zlib_local block_state deflate_fast   OF((deflate_state *s, int32_t flush));
#ifndef FASTEST
zlib_local block_state deflate_slow   OF((deflate_state *s, int32_t flush));
#endif
zlib_local void lm_init        OF((deflate_state *s));
zlib_local void putShortMSB    OF((deflate_state *s, uint32_t b));
zlib_local void flush_pending  OF((z_streamp strm));
zlib_local int32_t read_buf        OF((z_streamp strm, byte *buf, uint32_t size));
#ifndef FASTEST
#ifdef ASMV
      void match_init OF(()); /* asm code initialization */
      uint32_t longest_match  OF((deflate_state *s, IPos cur_match));
#else
zlib_local uint32_t longest_match  OF((deflate_state *s, IPos cur_match));
#endif
#endif
zlib_local uint32_t longest_match_fast OF((deflate_state *s, IPos cur_match));

#ifdef DEBUG
zlib_local  void check_match OF((deflate_state *s, IPos start, IPos match,
                            int32_t length));
#endif
...
/* Minimum amount of lookahead, except at the end of the input spfile->
...
#ifdef FASTEST
zlib_local const config configuration_table[2] = {
/*      good lazy nice chain */
/* 0 */ {0,    0,  0,    0, deflate_stored},  /* store only */
/* 1 */ {4,    4,  8,    4, deflate_fast}}; /* max speed, no lazy matches */
#else
zlib_local const config configuration_table[10] = {
/*      good lazy nice chain */
/* 0 */ {0,    0,  0,    0, deflate_stored},  /* store only */
/* 1 */ {4,    4,  8,    4, deflate_fast}, /* max speed, no lazy matches */
...
struct static_tree_desc_s {int32_t dummy;}; /* for buggy compilers */
#endif
...
/* ========================================================================= */
int32_t ZEXPORT deflateSetDictionary (
    z_streamp strm,
...
/* ========================================================================= */
int32_t ZEXPORT deflateSetHeader (
    z_streamp strm,
    gz_headerp head)
{
...
zlib_local void flush_pending(
    z_streamp strm)
{
    uint_ptr len = strm->state->pending;
    ...
    int32_t flush)
{
    ...
    if (strm->avail_in != 0 || s->lookahead != 0 ||
        ...
        if (bstate == need_more || bstate == finish_started) {
             ...
             * is_empty block here, this will be done at next call. This also
        ...
        if (bstate == block_done) {
            if (flush == Z_PARTIAL_FLUSH) {
                ...
                /* For a full flush, this is_empty block will be recognized
 ...
 * doesn't have enough primitive::memory anyway to duplicate compression states).
 */
int32_t ZEXPORT deflateCopy (
    z_streamp dest,
    z_streamp source)
{
#ifdef MAXSEG_64K
 ...
 */
zlib_local uint32_t longest_match(
    deflate_state *s,
    IPos cur_match)                           /* current match */
{
    ...
    int32_t best_len = s->prev_length;              /* best match length so far */
    int32_t nice_match = s->nice_match;             /* stop if match long enough */
    ...
    do {
         ...
         * reasons.  Therefore uninitialized primitive::memory will be accessed, and
 ...
 */
zlib_local uint32_t longest_match_fast(
    deflate_state *s,
    IPos cur_match)                           /* current match */
{
    register byte *scan = s->window + s->strstart; /* current string */
    ...
    do {
        ...
        if (s->strstart >= wsize+MAX_DIST(s)) {
            ...
            /* Slide the hash table (could be avoided with 32 bit values
               at the expense of primitive::memory usage). We slide even when level == 0
 ...
 */
zlib_local block_state deflate_stored(
    deflate_state *s,
    int32_t flush)
{
    ...
    int32_t flush)
{
    ...
    int32_t bflush;           /* set if current block must be flushed */

    for (;;) {
        /* Make sure that we always have enough lookahead, except
         * at the end of the input spfile-> We need MAX_MATCH bytes
 ...
 */
zlib_local block_state deflate_slow(
    deflate_state *s,
    int32_t flush)
{
 ...
 */
zlib_local block_state deflate_rle(
    deflate_state *s,
    int32_t flush)
{
    int32_t bflush;         /* set if current block must be flushed */
    uint32_t run;           /* length of run */
    uint32_t max;           /* maximum length of run */
