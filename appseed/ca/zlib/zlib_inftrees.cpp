...
/*
   ...
   whose indices are 0..2^bits-1.  work is a writable base_array of at least
...
uint16_t FAR *work)
{
    ...
    code codeThis;                  /* table entry for duplication */
    ...
    /*
       ...
       symbols by length from int16_t to long, and retaining the symbol order
    ...
    if (max == 0) {                     /* no symbols to code at all */
        codeThis.op = (uchar)64;    /* invalid code marker */
        codeThis.bits = (uchar)1;
        codeThis.val = (uint16_t)0;
        *(*table)++ = codeThis;             /* make a table to force an error */
        *(*table)++ = codeThis;
        *bits = 1;
    ...
    while (huff != 0) {
        ...
        if (drop != 0 && (huff & mask) != low) {
            ...
            len = root;
            next = *table;
            codeThis.bits = (uchar)len;
        }

        /* put invalid code marker in table */
        next[huff >> drop] = codeThis;
