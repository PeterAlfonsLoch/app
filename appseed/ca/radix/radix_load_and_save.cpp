...
namespace _template
{
   ...
   static bool WINAPI SetSettings(trace_settings *pTraceSettings, UINT nLevel, UINT nStatus)
   {
      ...
      {
      case 0:
         pTraceSettings->m_eStatus = trace_settings::Inherit;
         break;
      case 1:
         pTraceSettings->m_eStatus = trace_settings::Enabled;
         break;
      case 2:
      default:
         pTraceSettings->m_eStatus = trace_settings::Disabled;
         break;
      }
      return true;
   }

   static bool WINAPI GetSettings(const trace_settings &rTraceSettings, UINT *pnStatus)
   {
      ATLASSERT(pnStatus);
      if(!pnStatus)
         return false;
   ...
   BOOL __stdcall AtlTraceLoadSettingsA(const CHAR *pszFileName, DWORD_PTR dwProcess /* = 0 */)
   {
      ...
      if(pszFileName)
      {
         if(-1 != GetFileAttributesA(pszFileName))
         {
            ...
            UINT nModule;
            trace_process *pProcess;
            trace_module *pModule;
            trace_category *pCategory;
            CHAR *pszProcess = "Process";
            CHAR cEnabled, cFuncAndCategoryNames, cFileNameAndLineInfo;
            allocator *pAllocator = &g_Allocator;

            if (dwProcess)
               pAllocator = reinterpret_cast<allocator*>(dwProcess);
            ...
            for(UINT i = 0; i < nModules; i++)
            {
               ...
               szName[MAX_PATH -1] = 0;
               if(!pAllocator->FindModule(gen::international::utf8_to_unicode(szName), &nModule))
                  continue;
               ...
               for(UINT j = 0; j < nCategories; j++)
               {
                  ...
                  while( iCategory != UINT( -1 ) )
                  {
                     ...
                     if( lstrcmpA(pCategory->Name(), szName) == 0 )
                     {
...
/*      ATLASSERT(pszFileName);
      ...
      */
      ...
      for(int i = 0; i <  info.nModules; i++)
      {
         ...
         sprintf(szKey, "Module%d", i+1);
   #endif
         if(::WritePrivateProfileStringA(pszProcess, szKey, gen::international::unicode_to_utf8(pModule->Name()), pszFileName) == 0)
            return FALSE;
   ...
   #endif
         if(::WritePrivateProfileStringA(gen::international::unicode_to_utf8(pModule->Name()), "Settings", szValue, pszFileName) == 0)
            return FALSE;
         ...
         while( nCategory != UINT( -1 ) )
         {
            ...
            if(::WritePrivateProfileStringA(gen::international::unicode_to_utf8(pModule->Name()), szKey, szValue, pszFileName) == 0)
               return FALSE;
   ...
   BOOL __stdcall AtlTraceLoadSettingsU(const CHAR *pszFileName, DWORD_PTR dwProcess /* = 0 */)
   {
      ...
      if(pszFileName)
      {
         if(-1 != GetFileAttributesW(pszFileName))
         {
            ...
            trace_module *pModule;
            trace_category *pCategory;
            const wchar_t * pszProcess = L"Process";
