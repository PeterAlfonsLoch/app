...
namespace sockets
{
   ...
   int32_t SctpSocket::Bind(::sockets::address & ad)
   {
      if (!ad.IsValid())
      {
         Handler().LogError(this, "SctpSocket", -1, "invalid address", ::ca::log::level::error);
         return -1;
      }
      ...
      if (GetSocket() != INVALID_SOCKET)
      {
         ...
         if (n == -1)
         {
            Handler().LogError(this, "SctpSocket", -1, "bind() failed", ::ca::log::level::error);
   ...
   int32_t SctpSocket::AddAddress(::sockets::address & ad)
   {
      ...
      if (GetSocket() == INVALID_SOCKET)
      {
         Handler().LogError(this, "SctpSocket", -1, "AddAddress called with invalid file descriptor", ::ca::log::level::error);
         return -1;
      }
      int32_t n = sctp_bindx(GetSocket(), ad, ad, SCTP_BINDX_ADD_ADDR);
      if (n == -1)
      {
         Handler().LogError(this, "SctpSocket", -1, "sctp_bindx() failed", ::ca::log::level::error);
      }
      return n;
   }
   ...
   int32_t SctpSocket::RemoveAddress(::sockets::address & ad)
   {
      ...
      if (GetSocket() == INVALID_SOCKET)
      {
         Handler().LogError(this, "SctpSocket", -1, "RemoveAddress called with invalid file descriptor", ::ca::log::level::error);
         return -1;
      }
   ...
   #endif
            {
               Handler().LogError(this, "connect: connection pending", Errno, StrError(Errno), ::ca::log::level::info);
               SetConnecting( true ); // this flag will control fd_set's
            }
            else
            {
               Handler().LogError(this, "SctpSocket", -1, "connect() failed", ::ca::log::level::error);
            }
         }
         return n;
      }
   ...
   int32_t SctpSocket::AddConnection(::sockets::address & ad)
   {
      ...
      if (GetSocket() == INVALID_SOCKET)
      {
         Handler().LogError(this, "SctpSocket", -1, "AddConnection called with invalid file descriptor", ::ca::log::level::error);
         return -1;
      }
      int32_t n = sctp_connectx(GetSocket(), ad, ad);
      if (n == -1)
      {
         Handler().LogError(this, "SctpSocket", -1, "sctp_connectx() failed", ::ca::log::level::error);
      }
      else
      {
         SetConnecting();
   ...
   int32_t SctpSocket::getpaddrs(sctp_assoc_t id,list<string>& vec)
   {
      ...
      if (!n || n == -1)
      {
         Handler().LogError(this, "SctpSocket", -1, "sctp_getpaddrs failed", ::ca::log::level::warning);
         return n;
      }
   ...
   int32_t SctpSocket::getladdrs(sctp_assoc_t id,list<string>& vec)
   {
      ...
      if (!n || n == -1)
      {
         Handler().LogError(this, "SctpSocket", -1, "sctp_getladdrs failed", ::ca::log::level::warning);
         return n;
      }
      for (int32_t i = 0; i < n; i++)
      {
   ...
   int32_t SctpSocket::PeelOff(sctp_assoc_t id)
   {
      ...
      if (n == -1)
      {
         Handler().LogError(this, "SctpSocket", -1, "PeelOff failed", ::ca::log::level::warning);
         return -1;
      }
      socket *p = create();
      p -> attach(n);
   ...
   */
      ...
      if (n == -1)
      {
         Handler().LogError(this, "SctpSocket", Errno, StrError(Errno), ::ca::log::level::fatal);
         SetCloseAndDelete();
      }
      else
      {
   ...
   void SctpSocket::OnWrite()
   {
      if (Connecting())
      {
         ...
         Handler().LogError(this, "sctp: connect failed", err, StrError(err), ::ca::log::level::fatal);
