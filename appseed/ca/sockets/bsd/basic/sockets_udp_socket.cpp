...
namespace sockets
{
   ...
   int32_t udp_socket::Bind(::sockets::address & ad, int32_t range)
   {
      ...
      if (GetSocket() != INVALID_SOCKET)
      {
         ...
         if (n == -1)
         {
            Handler().LogError(this, "bind", Errno, StrError(Errno), ::ca::log::level::fatal);
            SetCloseAndDelete();
            throw simple_exception(get_app(), "bind() failed for udp_socket, port:range: " + ::ca::str::from(ad.get_service_number()) + ":" + ::ca::str::from(range));
            return -1;
         }
         m_bind_ok = true;
   ...
   bool udp_socket::open(::sockets::address & ad)
   {
      ...
      if (GetSocket() != INVALID_SOCKET)
      {
         ...
         {
            Handler().LogError(this, "connect", Errno, StrError(Errno), ::ca::log::level::fatal);
            SetCloseAndDelete();
            return false;
         }
         SetConnected();
   ...
   void udp_socket::SendToBuf(const ::sockets::address & ad, const char *data, int32_t len, int32_t flags)
   {
      ...
      if (GetSocket() != INVALID_SOCKET)
      {
         ...
         {
            Handler().LogError(this, "sendto", Errno, StrError(Errno), ::ca::log::level::error);
         }
      }
   }
   ...
   void udp_socket::SendBuf(const char *data, int32_t len, int32_t flags)
   {
      if (!IsConnected())
      {
         Handler().LogError(this, "SendBuf", 0, "not connected", ::ca::log::level::error);
         return;
      }
      if ((m_last_size_written = send(GetSocket(), data, (int32_t)len, flags)) == -1)
      {
         Handler().LogError(this, "send", Errno, StrError(Errno), ::ca::log::level::error);
      }
   }
   ...
   void udp_socket::OnRead()
   {
      if (IsIpv6())
      {
   ...
   #endif
                  Handler().LogError(this, "recvfrom", Errno, StrError(Errno), ::ca::log::level::error);
            }
            return;
         }
         size_t n = recvfrom(GetSocket(), m_ibuf, m_ibufsz, 0, (struct sockaddr *)&sa, &sa_len);
         ...
         while (n > 0)
         {
            if (sa_len != sizeof(sa))
            {
               Handler().LogError(this, "recvfrom", 0, "unexpected address struct size", ::ca::log::level::warning);
            }
   ...
   void udp_socket::SetBroadcast(bool b)
   {
      ...
      if (b)
      {
         ...
         {
            Handler().LogError(this, "SetBroadcast", Errno, StrError(Errno), ::ca::log::level::warning);
         }
      }
      else
      {
   ...
   bool udp_socket::IsBroadcast()
   {
      ...
      {
         Handler().LogError(this, "IsBroadcast", Errno, StrError(Errno), ::ca::log::level::warning);
      }
   ...
   void udp_socket::SetMulticastTTL(int32_t ttl)
   {
      ...
      {
         Handler().LogError(this, "SetMulticastTTL", Errno, StrError(Errno), ::ca::log::level::warning);
      }
   }
   ...
   int32_t udp_socket::GetMulticastTTL()
   {
      ...
      {
         Handler().LogError(this, "GetMulticastTTL", Errno, StrError(Errno), ::ca::log::level::warning);
      }
      return ttl;
   }
   ...
   void udp_socket::SetMulticastLoop(bool x)
   {
      ...
      if (IsIpv6())
      {
         ...
         {
            Handler().LogError(this, "SetMulticastLoop", Errno, StrError(Errno), ::ca::log::level::warning);
         }
         return;
      }
      int32_t val = x ? 1 : 0;
   ...
   bool udp_socket::IsMulticastLoop()
   {
      ...
      if (IsIpv6())
      {
         ...
         {
            Handler().LogError(this, "IsMulticastLoop", Errno, StrError(Errno), ::ca::log::level::warning);
         }
   ...
   void udp_socket::AddMulticastMembership(const string & group, const string & local_if, int32_t if_index)
   {
      ...
      if (IsIpv6())
      {
         ...
         if (System.net().convert( addr, group ))
         {
            ...
            {
               Handler().LogError(this, "AddMulticastMembership", Errno, StrError(Errno), ::ca::log::level::warning);
            }
         }
         return;
      }
   ...
   void udp_socket::DropMulticastMembership(const string & group, const string & local_if, int32_t if_index)
   {
      ...
      if (IsIpv6())
      {
         ...
         if (System.net().convert(addr, group))
         {
            ...
            {
               Handler().LogError(this, "DropMulticastMembership", Errno, StrError(Errno), ::ca::log::level::warning);
            }
         }
         return;
      }
   ...
   void udp_socket::SetMulticastHops(int32_t hops)
   {
      ...
      if (!IsIpv6())
      {
         Handler().LogError(this, "SetMulticastHops", 0, "Ipv6 only", ::ca::log::level::error);
         return;
      }
      if (setsockopt(GetSocket(), IPPROTO_IPV6, IPV6_MULTICAST_HOPS, (char *)&hops, sizeof(int32_t)) == -1)
      {
         Handler().LogError(this, "SetMulticastHops", Errno, StrError(Errno), ::ca::log::level::warning);
      }
   }
   ...
   int32_t udp_socket::GetMulticastHops()
   {
      ...
      {
         Handler().LogError(this, "GetMulticastHops", Errno, StrError(Errno), ::ca::log::level::warning);
      }
      return hops;
   }
