...
namespace crypto
{
   ...
   gen::HRes aes_cbc_coder::SetKey(const byte *data, uint32_t size)
   {
   ...
   gen::HRes aes_cbc_coder::SetInitVector(const byte *data, uint32_t size)
   {
      if (size != AES_BLOCK_SIZE)
         return E_INVALIDARG;
