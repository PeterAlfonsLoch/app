 ...
 */
int tiny_http::t_read_buffer (int fd, char * buffer, int length, void (*callback)(void *, int, uint_ptr), void * callback_param)
{
  ssize_t n,r;
...
tiny_http::http_retcode tiny_http::t_put(const char * data, int length, int overwrite, void (*callback)(void *, int, uint_ptr), void * callback_param)
{
  char header[MAXBUF];
...
tiny_http::http_retcode tiny_http::t_get(char ** pdata, int * plength, void (*callback)(void *, int, uint_ptr), void * callback_param)
{
  http_retcode ret;
  ...
  if (ret==200) {
	...
	return ERRRDHD;
      ...
      sscanf(header,"content-type: %s", (char *) (const char*) m_strContentType);
    }
    if (length<=0) {
      close(fd);
      return ERRNOLG;
