...
namespace zip
{
   ...
   bool File::unzip_open(const char * lpcwsz)
   {
      ...
      try
      {
         if(!spfile->open(lpcwsz, ::gen::file::mode_read | ::gen::file::type_binary))
         {
            return false;
         }
      }
   ...
   bool File::unzip_open(gen::filesp pfile)
   {
      m_pbuffile1(new ::gen::buffered_file(get_app(), pfile, 1024 * 256));
      m_pbuffile2(new ::gen::buffered_file(get_app(), m_pbuffile1, 1024 * 256));
   ...
   bool File::zip_open(const char * lpcwsz)
   {
      ...
      try
      {
         if(!spfile->open(lpcwsz, ::gen::file::mode_read_write | ::gen::file::type_binary | ::gen::file::mode_create | ::gen::file::defer_create_directory))
         {
            return false;
         }
      }
   ...
   bool File::zip_open(gen::filesp pfile)
   {
      m_pbuffile1(new ::gen::buffered_file(get_app(), pfile, 1024 * 256));
      m_pbuffile2(new ::gen::buffered_file(get_app(), m_pbuffile1, 1024 * 256));
      m_pbuffile2->seek_to_begin();
   ...
   void  File::write_to_file(gen::file * pfileOut, const wchar_t * lpcsz)
   {
      string str;
   ...
   voidpf File::open_file_func (voidpf opaque, const char* filename, int32_t mode)
   {
      ...
      gen::file * pfile = pzipfile->m_pfile;
      return (voidpf) pfile;
   }
