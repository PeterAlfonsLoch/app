...
namespace libcompress
{
   ...
   #define CA2_COMPRESS_INTERFACE_archive_ExtractCallback(specifier) \
     ...
     virtual ca::HRes GetStream(uint32_t index, ca::writer **outStream,  int32_t askExtractMode) specifier; \
...
//virtual ca::HRes GetStream(const char * name, ca::byte_input_stream **inStream) specifier; \ // input stream should be seekable, so by the time using ca::file interface
   ...
   */
     ...
     //virtual ca::HRes Open(::ca::byte_input_stream * stream, const uint64_t *maxCheckStartPosition, ::libcompress::archive_open_callback_interface *openArchiveCallback) specifier; \ // input stream should be seekable, so by the time using ca::file interface
   #define CA2_COMPRESS_INTERFACE_input_archive(specifier) \
     virtual ca::HRes Open(::ca::byte_input_stream * stream, const file_position *maxCheckStartPosition, ::libcompress::archive_open_callback_interface *openArchiveCallback) specifier; \
     virtual ca::HRes Close() specifier; \
     virtual ca::HRes GetNumberOfItems(uint32_t *numItems) specifier; \
     virtual ca::HRes GetProperty(uint32_t index, int32_t propID, var *value) specifier; \
     virtual ca::HRes Extract(const uint32_t* indices, uint32_t numItems, int32_t testMode, ::libcompress::archive_extract_callback_interface *extractCallback) specifier; \
     virtual ca::HRes GetArchiveProperty(int32_t propID, var *value) specifier; \
     virtual ca::HRes GetNumberOfProperties(uint32_t *numProperties) specifier; \
     virtual ca::HRes GetPropertyInfo(uint32_t index, string & name, int32_t *propID, var::e_type *varType) specifier; \
   ...
   #define CA2_COMPRESS_INTERFACE_archive_update_callback(specifier) \
     CA2_EX1_progress_interface(specifier); \
     virtual ca::HRes GetUpdateItemInfo(uint32_t index,  \
   ...
   #define IMP_IInArchive_Props_WITH_NAME \
     ca::HRes CHandler::GetNumberOfProperties(uint32_t *numProperties) \
       { *numProperties = sizeof(kProps) / sizeof(kProps[0]); return S_OK; } \
     ca::HRes CHandler::GetPropertyInfo IMP_IInArchive_GetProp_WITH_NAME(kProps)


   #define IMP_IInArchive_ArcProps \
     ca::HRes handler::GetNumberOfArchiveProperties(uint32_t *numProperties) \
   ...
   #define IMP_IInArchive_ArcProps_WITH_NAME \
     ca::HRes CHandler::GetNumberOfArchiveProperties(uint32_t *numProperties) \
       { *numProperties = sizeof(kArcProps) / sizeof(kArcProps[0]); return S_OK; } \
   ...
   #define IMP_IInArchive_ArcProps_NO_Table \
     ...
     ca::HRes CHandler::GetArchivePropertyInfo(uint32_t, string &, int32_t *, var::e_type *) \
       { return E_NOTIMPL; } \
