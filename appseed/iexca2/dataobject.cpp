...
namespace iexca2
{
   ...
   struct format_etc :
      public FORMATETC
   {
      typedef FORMATETC BASE;

      format_etc()
      {
      }

      format_etc(const FORMATETC & etc)
      {
         operator = (etc);
      }

      format_etc & operator = (const FORMATETC & etc)
      {
         if(this != &etc)
         {
            memcpy_dup(this, &etc, sizeof(*this));
         }
         return *this;
      }
   };

   typedef simple_enum < IID_IEnumFORMATETC, IEnumFORMATETC, format_etc > VLCEnumFORMATETC;

   ////////////////////////////////////////////////////////////////////////////////////////////////

   data_object::data_object(host * p_instance) :
      _p_instance(p_instance)
   {
       _v_formatEtc.add(_enhMetaFileFormatEtc);
       _v_formatEtc.add(_metaFileFormatEtc);
   ...
   STDMETHODIMP data_object::DAdvise(LPFORMATETC pFormatEtc, DWORD padvf, LPADVISESINK pAdviseSink, LPDWORD pdwConnection)
   {
       return _p_adviseHolder->Advise(this,
               pFormatEtc, padvf,pAdviseSink, pdwConnection);
   }

   STDMETHODIMP data_object::DUnadvise(DWORD dwConnection)
   {
       return _p_adviseHolder->Unadvise(dwConnection);
   }

   STDMETHODIMP data_object::EnumDAdvise(IEnumSTATDATA **ppenumAdvise)
   {
       return _p_adviseHolder->EnumAdvise(ppenumAdvise);
   }

   STDMETHODIMP data_object::EnumFormatEtc(DWORD dwDirection, IEnumFORMATETC **ppEnum)
   {
       if( NULL == ppEnum )
           return E_POINTER;
   ...
   STDMETHODIMP data_object::GetCanonicalFormatEtc(LPFORMATETC pFormatEtcIn, LPFORMATETC pFormatEtcOut)
   {
   ...
   STDMETHODIMP data_object::GetData(LPFORMATETC pFormatEtc, LPSTGMEDIUM pMedium)
   {
       if( NULL == pMedium )
           return E_POINTER;
   ...
   STDMETHODIMP data_object::GetDataHere(LPFORMATETC pFormatEtc, LPSTGMEDIUM pMedium)
   {
       if( NULL == pMedium )
           return E_POINTER;
   ...
   HRESULT data_object::getMetaFileData(LPFORMATETC pFormatEtc, LPSTGMEDIUM pMedium)
   {
   ...
   HRESULT data_object::getEnhMetaFileData(LPFORMATETC pFormatEtc, LPSTGMEDIUM pMedium)
   {
      return E_FAIL;
       /*HDC hicTargetDev = CreateDevDC(pFormatEtc->ptd);
       if( NULL == hicTargetDev )
   ...
   STDMETHODIMP data_object::QueryGetData(LPFORMATETC pFormatEtc)
   {
       if( NULL == pFormatEtc )
           return E_POINTER;
   ...
   STDMETHODIMP data_object::SetData(LPFORMATETC pFormatEtc, LPSTGMEDIUM pMedium, BOOL fRelease)
   {
       return E_NOTIMPL;
   }

   void data_object::onClose(void)
   {
