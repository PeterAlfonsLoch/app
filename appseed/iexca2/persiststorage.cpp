...
namespace iexca2
{
   ...
   STDMETHODIMP persist_storage::GetClassID(LPCLSID pClsID)
   {
       if( NULL == pClsID )
           return E_POINTER;
   ...
   STDMETHODIMP persist_storage::IsDirty(void)
   {
       return _p_instance->isDirty() ? S_OK : S_FALSE;
   }

   STDMETHODIMP persist_storage::InitNew(LPSTORAGE pStg)
   {
       return _p_instance->onInit();
   }

   STDMETHODIMP persist_storage::Load(LPSTORAGE pStg)
   {
       if( NULL == pStg )
           return E_INVALIDARG;
   ...
   STDMETHODIMP persist_storage::Save(LPSTORAGE pStg, BOOL fSameAsLoad)
   {
       if( NULL == pStg )
           return E_INVALIDARG;
   ...
   STDMETHODIMP persist_storage::SaveCompleted(IStorage *pStg)
   {
       return S_OK;
   }

   STDMETHODIMP persist_storage::HandsOffStorage(void)
   {
       return S_OK;
   }
