...
{
   return keySetDef(hKey, s, sizeof(WCHAR)*(wcslen_dup(s)+1), REG_SZ);
}
...
static void UnregisterProgID(REFCLSID rclsid, unsigned int version)
{
   ...
   swnprintf_dup(progId, sizeof(PROGID_STR)+16,  W("%s.%u"), W(PROGID_STR), version);
...
static HRESULT RegisterClassID(HKEY hParent, REFCLSID rclsid, unsigned int version, BOOL isDefault, LPCWSTR path,const size_t pathLen)
{
   ...
   swnprintf_dup(description, sizeof(DESCRIPTION)+16, W("%s v%u"), W(DESCRIPTION), version);
   ...
   if( NULL != hClassKey )
   {
      ...
      // ToolboxBitmap32 key value
      {
         ...
         memcpy_dup(iconPath, path, sizeof(WCHAR)*pathLen);
         wcscpy_dup(iconPath+pathLen, W(",1"));
...
STDAPI DllRegisterServer()
{
   ...
   HANDLE hinstanceParent = g_hinstance;

   DllUnregisterServer();

   wstring DllPath;
   
   DllPath.alloc(MAX_PATH * 8);

   ::GetModuleFileNameW((HMODULE) hinstanceParent, (wchar_t*)(const wchar_t *) DllPath, MAX_PATH * 8);

   UINT DllPathLen = DllPath.length();
