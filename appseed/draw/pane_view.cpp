...
pane_view::ViewData * pane_view::CreateView(EPaneView eview)
{
   ...
   html_application * papp = dynamic_cast < html_application * > (((BaseFrameWnd *) GetParentFrame())->m_papp);;
   switch(eview)
   {
   case PaneViewContextMenu:
      /*{
         devedgeFrontDoc * pdoc = (devedgeFrontDoc *) (dynamic_cast < html_application * > (((BaseFrameWnd *) GetParentFrame())->m_papp))->m_ptemplateFront->OpenDocumentFile("http://localhost:10011/");
         if(pdoc != NULL)
         {
            POSITION pos = pdoc->GetFirstViewPosition();
            BaseView * pview = pdoc->GetNextView(pos);
