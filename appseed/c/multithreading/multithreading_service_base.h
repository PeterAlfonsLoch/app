...
class CLASS_DECL_c service_base :
...
public:
   ...
   service_base(sp(base_application) papp, uint32_t controlsAccepted = SERVICE_ACCEPT_PAUSE_CONTINUE | SERVICE_ACCEPT_STOP | SERVICE_ACCEPT_SHUTDOWN);

#else

   service_base(sp(base_application) papp, uint32_t controlsAccepted = 0);
