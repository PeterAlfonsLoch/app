...
public:
   ...
   strid_map(sp(::application) papp = NULL, ::count nBlockSize = 256);
...
strid_map < VALUE, ARG_VALUE, HASH, EQUALS >::strid_map(sp(::application) papp, ::count nBlockSize) :
   element(papp), 
...
template < class T >
class CLASS_DECL_c string_to_pointer :
