...
namespace sockets
{
   ...
   int32_t udp_socket::Bind(::net::address & ad, int32_t range)
   {
      ...
      if (GetSocket() != INVALID_SOCKET)
      {
         ...
         if (n == -1)
         {
            log("bind", Errno, StrError(Errno), ::base::log::level_fatal);
            SetCloseAndDelete();
   ...
   bool udp_socket::open(const ::net::address & ad)
   {
      ...
      if (GetSocket() != INVALID_SOCKET)
      {
         ...
         {
            log("connect", Errno, StrError(Errno), ::base::log::level_fatal);
            SetCloseAndDelete();
            return false;
         }
         SetConnected();
   ...
   void udp_socket::SendToBuf(const ::net::address & ad, const char *data, int32_t len, int32_t flags)
   {
      ...
      if (GetSocket() != INVALID_SOCKET)
      {
         ...
         {
            log("sendto", Errno, StrError(Errno), ::base::log::level_error);
         }
      }
   }
   ...
   void udp_socket::write(const void *data, ::primitive::memory_size len)
   {
      ...
      if (!IsConnected())
      {
         ...
         log("write", 0, "not connected", ::base::log::level_error);
      ...
      if ((m_last_size_written = send(GetSocket(), (const char *) data, (int32_t)len, m_iWriteFlags)) == -1)
      {
         ...
         log("write", Errno, StrError(Errno), ::base::log::level_error);
   ...
   void udp_socket::OnRead()
   {
      if (IsIpv6())
      {
   ...
   #endif
                  log("recvfrom", Errno, StrError(Errno), ::base::log::level_error);
            }
            return;
         }
         size_t n = recvfrom(GetSocket(), m_ibuf, m_ibufsz, 0, (struct sockaddr *)&sa, &sa_len);
         ...
         while (n > 0)
         {
            if (sa_len != sizeof(sa))
            {
               log("recvfrom", 0, "unexpected address struct size", ::base::log::level_warning);
            }
   ...
   void udp_socket::SetBroadcast(bool b)
   {
      ...
      if (b)
      {
         ...
         {
            log("SetBroadcast", Errno, StrError(Errno), ::base::log::level_warning);
         }
      }
      else
      {
   ...
   bool udp_socket::IsBroadcast()
   {
      ...
      {
         log("IsBroadcast", Errno, StrError(Errno), ::base::log::level_warning);
      }
   ...
   void udp_socket::SetMulticastTTL(int32_t ttl)
   {
      ...
      {
         log("SetMulticastTTL", Errno, StrError(Errno), ::base::log::level_warning);
      }
   }
   ...
   int32_t udp_socket::GetMulticastTTL()
   {
      ...
      {
         log("GetMulticastTTL", Errno, StrError(Errno), ::base::log::level_warning);
      }
      return ttl;
   }
   ...
   void udp_socket::SetMulticastLoop(bool x)
   {
      ...
      if (IsIpv6())
      {
         ...
         {
            log("SetMulticastLoop", Errno, StrError(Errno), ::base::log::level_warning);
         }
         return;
      }
      int32_t val = x ? 1 : 0;
   ...
   bool udp_socket::IsMulticastLoop()
   {
      ...
      if (IsIpv6())
      {
         ...
         {
            log("IsMulticastLoop", Errno, StrError(Errno), ::base::log::level_warning);
         }
   ...
   void udp_socket::AddMulticastMembership(const string & group, const string & local_if, int32_t if_index)
   {
      ...
      if (IsIpv6())
      {
         ...
         if (System.net().convert( addr, group ))
         {
            ...
            {
               log("AddMulticastMembership", Errno, StrError(Errno), ::base::log::level_warning);
            }
         }
         return;
      }
   ...
   void udp_socket::DropMulticastMembership(const string & group, const string & local_if, int32_t if_index)
   {
      ...
      if (IsIpv6())
      {
         ...
         if (System.net().convert(addr, group))
         {
            ...
            {
               log("DropMulticastMembership", Errno, StrError(Errno), ::base::log::level_warning);
            }
         }
         return;
      }
   ...
   void udp_socket::SetMulticastHops(int32_t hops)
   {
      ...
      if (!IsIpv6())
      {
         log("SetMulticastHops", 0, "Ipv6 only", ::base::log::level_error);
         return;
      }
      if (setsockopt(GetSocket(), IPPROTO_IPV6, IPV6_MULTICAST_HOPS, (char *)&hops, sizeof(int32_t)) == -1)
      {
         log("SetMulticastHops", Errno, StrError(Errno), ::base::log::level_warning);
      }
   }
   ...
   int32_t udp_socket::GetMulticastHops()
   {
      ...
      {
         log("GetMulticastHops", Errno, StrError(Errno), ::base::log::level_warning);
      }
      return hops;
   }
