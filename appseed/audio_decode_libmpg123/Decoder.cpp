...
namespace audio_decode_libmpg123
{
   ...
   bool decoder::_DecoderInitialize(ex1::file *pfile)
   {
      ...
      try
      {
         ...
         m_readframe.m_lSamplesPerSecond = 44100;
         m_readframe.m_iChannels = 2;
   ...
   bool decoder::DecoderSetExpectedParams(UINT uiSamplesPerSec, UINT uiChannelCount, UINT uiBitCount, UINT uiEncodeAvgBitRate)
   {
      m_uiEncodedAvgBitRate            = uiEncodeAvgBitRate;
      m_readframe.m_lSamplesPerSecond  = uiSamplesPerSec;
      m_readframe.m_iChannels          = uiChannelCount;
      ASSERT(uiBitCount == 16);
      return false;
   }
...
//      DWORD dw = m_readframe.m_pfileIn->get_position();
   ...
   bool decoder::ReadFrame::_seekable_read_frame()
   {
      ...
      if(!m_bFileInEof)
      {
         while(m_pfileOut->get_length() < m_pdecoder->DecoderGetRawSize(m_memoryIn.get_size()))
         {
            done = 0;
            iRet = MPG123_ERR;
            try
   ...
   bool decoder::ReadFrame::_non_seekable_read_frame()
   {
      ...
      if(!m_bFileInEof)
      {
         while(iTotalRead < iMaxCount)
         {
            ...
            {
               if(m_pfileOut->get_length() >= m_pdecoder->DecoderGetRawSize(m_memoryIn.get_size()))
                  break;
   ...
   int decoder::DecoderSetReadBlockSize(int iSize)
   {
      ...
      if(iSize > 0)
      {
         ...
         m_iBufferSize = iSize;
         ::audio_decode::decoder::DecoderSetReadBlockSize(iSize);
      }
