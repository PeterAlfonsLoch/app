...
{
   m_pmediaviewdata = new mediaplay::MediaViewData();
   m_pmediaviewdata->m_pdocument = this;

   //SetKaraokeData(new CKaraokeData());
   //if(GetMediaViewData()->GetKaraokeData().IsNull())
      //throw new CMemoryException;

   GetMediaViewData()->GetKaraokeData().SetInterface(&GetMediaViewData()->HelperGetMidiSequence());
...
void MusicalPlayerLightDoc::Serialize(CArchive& ar)
{
   ...
   if (ar.IsStoring())
   {
      GetMediaViewData()->HelperGetMidiSequence().SaveFile(ar);
   }
   else
   {
      
      mmr = GetMediaViewData()->HelperGetMidiSequence().OpenFile(ar, MidiFile::OpenForPlaying);
...
BOOL MusicalPlayerLightDoc::OnOpenDocument(LPCTSTR lpszPathName) 
{
   ...
   if(!GetMediaViewData()->OnOpenDocument(lpszPathName))
   {
      DeleteContents();
      return FALSE;
   }
...
{
	GetMediaViewData()->DeleteContents();
	
	OleServerDoc::DeleteContents();
}

void MusicalPlayerLightDoc::AttachPlaylist(PlaylistDoc *pdoc)
{
   CVmpLightViewUpdateHint uh;
   uh.SetType(CVmpLightViewUpdateHint::TypeAttachPlaylist);
