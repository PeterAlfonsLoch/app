...
bool DocumentManager::CreateTemplates()
{
   MusicalPlayerLightApp * papp = (MusicalPlayerLightApp *) AfxGetApp();
	...
	playercentral.m_pruntimeclassDocument  = RUNTIME_CLASS(MusicalPlayerLightDoc);
	playercentral.m_pruntimeclassFrameWnd  = RUNTIME_CLASS(MainFrame);
	playercentral.m_pruntimeclassView      = RUNTIME_CLASS(PaneView);
	playercentral.m_pruntimeclassIPFrame   = RUNTIME_CLASS(InPlaceFrame);
	playercentral.m_uiResourceId           = IDR_MAINFRAME;
	playercentral.m_uiResourceIdEmbedding  = IDR_SRVR_EMBEDDED;
	playercentral.m_uiResourceIdInPlace    = IDR_SRVR_INPLACE;

	musctrl::SingleDocTemplate* pPlayerSingleDocTemplate;
