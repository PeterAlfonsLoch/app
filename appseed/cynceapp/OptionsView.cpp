...
CWnd* OptionsView::CreateView(CreateContext* pContext, UINT nID, CWnd * pwndParent)
{
...
void OptionsView::OnUpdate(BaseView* pSender, LPARAM lHint, CObject* pHint) 
{
...
void OptionsView::Show(ViewData * pviewdata)
{
   ...
   OptionsChildFrame * pframe = dynamic_cast < OptionsChildFrame * > ((CWnd *) GetParentFrame());
   ...
   case OptionsViewFolders:
      {
         CreateContext cc;
         cc.m_pCurrentDoc = GetDocument();
         cc.m_pNewViewClass =  RUNTIME_CLASS(OptionsFoldersView);

         BaseView * pview = dynamic_cast < OptionsFoldersView * > (CreateView(&cc, OptionsViewFolders, this));
         if(pview != NULL)
         {
            pviewdata = new ViewData();
            pviewdata->m_eview = eview;
...
/*CWnd * OptionsView::GetKaraokeWindow()
{
   ViewData * pview;
...
//   MusicalPlayerLightView * pview = (MusicalPlayerLightView *) GetKaraokeWindow();
