...
public:
   ...
   void PostFillTask(CString & strFile, UINT uiTimer);

	virtual void _001OnSort();
	bool GetSongPath(CString & str, int iItem);
	MediaLibraryDoc * GetDocument();

// Overrides
	virtual int _001GetItemImage(int iItem, int iSubItem, int iListItem);
	virtual void _001InsertColumns();
	virtual bool _001GetItemText(CString &str, int iItem, int iSubItem, int iListItem);
   virtual int _001GetItemCount();
   virtual bool _001SearchGetItemText(CString &str, int iItem, int iSubItem, int iListItem);
