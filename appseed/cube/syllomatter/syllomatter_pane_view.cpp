...
void syllomatter_pane_view::on_create_view(view_data * pviewdata)
{
   ...
   case syllomatter::PaneViewForm:
      {
         m_pdoctemplateExtractChanges = new ::userbase::single_document_template(
          get_app(),
          "syllomatter/frame",
          &typeid(syllomatter_document),
          &typeid(syllomatter_frame),
          &typeid(syllomatter_view));

         /*
       syllomatter_document * pdoc = dynamic_cast < syllomatter_document * > (
          m_pdoctemplateExtractChanges->open_document_file(
          System.dir().matter("syllomatter_extract_changes_form.xhtml")));*/
         ...
         {
            pviewdata->m_pdoc = get_document();
            pviewdata->m_pwnd = pview->GetParentFrame();
         }
      }
      break;
   case syllomatter::PaneViewSVN:
      {
         ...
         cc.m_typeinfoNewView = &typeid(svn_view);
         svn_view * pview = dynamic_cast < svn_view * > (create_view(typeid(svn_view), get_document(), this, 1000));
         m_psvnview = pview;
...
void syllomatter_pane_view::_001InstallMessageHandling(::user::win::message::dispatch * pinterface)
{
