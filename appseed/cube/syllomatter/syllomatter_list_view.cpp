...
void syllomatter_list_view::_001InstallMessageHandling(::user::win::message::dispatch * pinterface)
{
   ::userbase::form_list::_001InstallMessageHandling(pinterface);
...
int syllomatter_list_view::_001GetItemImage(int iItem, int iSubItem, int iListItem)
{
   if(iSubItem == 2)
   {
...
bool syllomatter_list_view::_001GetItemText(
                  string &str,
                  INT_PTR iItem,
                  INT_PTR iSubItem, 
                  INT_PTR iListItem)
...
int syllomatter_list_view::commit_thread::run()
{
   ...
   System.file().put_contents(strTime, strFile);
   strCmd += "--file " + strTime;
   strCmd += " ";
...
COLORREF syllomatter_list_view::_001GetItemColor(
                  INT_PTR iItem, 
                  INT_PTR iSubItem,
                  INT_PTR iListItem,
                  int iState)
{
   if(m_straStatus[iItem] == "M")
   {
      return RGB(100, 150, 255);
   }
   else if(m_straStatus[iItem] == "A")
   {
      return RGB(235, 150, 255);
   }
   else if(m_straStatus[iItem] == "D")
   {
      return RGB(255, 100, 55);
   }
   else
   {
      return ::userbase::form_list::_001GetItemColor(iItem,
         iSubItem, iListItem, iState);
   }
}
