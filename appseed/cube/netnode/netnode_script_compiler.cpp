#include "StdAfx.h"
#include "netnodeScriptCompiler.h"
#include "netnodeScriptInstance.h"
#include "netnodeScriptCache.h"



netnodeScriptCompiler::netnodeScriptCompiler(::ca::application * papp) :
   ca(papp),
   m_memfileLibError(papp),
   m_folderwatch(papp),
   m_folderwatchFribox(papp)
{
   m_hmoduleLib = NULL;
}

netnodeScriptCompiler::~netnodeScriptCompiler(void)
{
}

void netnodeScriptCompiler::initialize(void)
{
   prepare_compile_and_link_environment();
   folder_watch();
   compile_library();
   run_persistent();
}

void netnodeScriptCompiler::prepare_compile_and_link_environment(void)
{
   string strVars = getenv("VS100COMNTOOLS");
   System.file().path().eat_end_level(strVars, 2, "\\");
   //m_strEnv = strVars;
   //m_strEnv += "vc\\vcvarsall.bat";
   m_strEnv = ".\\vc10vars64.bat";


   m_strTime = System.dir().ca2("time");

   //m_strEnv = "C:\\Program Files\\Microsoft SDKs\\Windows\\v7.1\\Bin\\SetEnv.cmd";

   m_strSdk1 = "windows7.1sdk";
   string strPlat2;
#ifdef _M_X64
   m_strPlat1     = "64";
   strPlat2 = " /x64";
   m_strPlatform = "x64";
   m_strLibPlatform = "x64/";
#else
   m_strPlat1     = "";
   strPlat2 = " /x86";
   m_strPlatform = "x86";
   m_strLibPlatform = "";
#endif

   System.file().lines(m_straSync, "C:\\ca2\\database\\text\\netnode\\syncer.txt", get_app());

   prepare1("netnodecld" + m_strPlat1 + ".bat", "netnodecld" + m_strPlat1 + ".bat");
   prepare1("netnodelibcd" + m_strPlat1 + ".bat", "netnodelibcd" + m_strPlat1 + ".bat");
   prepare1("netnodelibld" + m_strPlat1 + ".bat", "netnodelibld" + m_strPlat1 + ".bat");

   string vars1batSrc;
   string vars2batSrc;
   string vars1batDst;
   string vars2batDst;
   vars1batSrc = System.dir().ca2("app/stage/app/matter/vc10vars64.bat");
   vars2batSrc = System.dir().ca2("app/stage/app/matter/vc10vars_query_registry.bat");
   vars1batDst = System.dir().votagus("stage/front", "vc10vars64.bat");
   vars2batDst = System.dir().votagus("stage/front", "vc10vars_query_registry.bat");
   System.file().copy(vars1batDst, vars1batSrc);
   System.file().copy(vars2batDst, vars2batSrc);


   var var = System.process().get_output("\"" + m_strEnv  + "\" "+ strPlat2);
   TRACE0(var.get_string());

   string str;
   string strItem;

   strItem = System.dir().ca2("stage\\" + m_strPlatform);
   str = str + strItem + ";";

   strItem = System.dir().ca2("stage\\" + m_strPlatform + "\\netnode\\library");
   str = str + strItem + ";";

   DWORD dwSize = GetEnvironmentVariable("PATH", NULL, 0);
   LPTSTR lpsz = new char[dwSize + 1];
   dwSize = GetEnvironmentVariable("PATH", lpsz, dwSize + 1);
   str += lpsz;
   BOOL bResult = SetEnvironmentVariable("PATH", str);
   TRACE("netnodeScriptCompiler::prepare_compile_and_link_environment SetEnvironmentVariable return BOOL %d", bResult);
   delete lpsz;

}

void netnodeScriptCompiler::compile(netnodeScript * pscript)
{
   CSingleLock slScript(&pscript->m_mutex, TRUE);
   TRACE("Compiling script \"%s\"\n", pscript->m_strName);
   string strName(pscript->m_strName);
   pscript->on_start_build();
   
   //strName.replace("/", "\\");
   //string strVotagusFolder;
   //strVotagusFolder = System.dir().votagus();
   string str;
   string strB;
   string strO;
   string strP;
   string strL;
   string strE;
   string strSVI;
   string strSVP;
   string strSPCH;
   string strDVI;
   string strDVP;
   string strDPCH;

   /*string strScript(strName);
   strScript.replace("\\", ",");
   strScript.replace("/", ",");
   strScript = "ca2netnodescript." + strScript;*/
   string strScript;
   strScript = System.file().title_(strName);
   string strTransformName = strName;
   if(System.file().exists(strName))
   {
      pscript->m_strSourcePath = strName;
      strTransformName.replace(":", "");
   }
   else
   {
      pscript->m_strSourcePath.Format(System.dir().ca2("net\\netseed\\ds\\ca2\\%s"), strName);
   }
   
   if(!System.file().exists(pscript->m_strSourcePath))
   {
      pscript->m_memfileError<< "<pre>";
      str.Format("Source File : \"%s\" does not exist", pscript->m_strSourcePath);
      pscript->m_memfileError << str;
      pscript->m_memfileError<< "</pre>";
      return;
   }

   



   pscript->m_strCppPath.Format(System.dir().path(m_strTime, "netnode\\%s.cpp"), strTransformName);
   pscript->m_strBuildBat.Format(System.dir().stage("front\\netnode\\BuildBat\\%s.bat"), strTransformName);
#ifdef _DEBUG
   strO.Format(System.dir().path(m_strTime, "intermediate\\" + m_strPlatform + "\\netnodescript\\%s\\%s.obj"), strTransformName, System.file().name_(strTransformName));
   strP.Format(System.dir().stage(m_strPlatform+"\\netnode\\%s.pdb"),        
         System.dir().path(System.dir().name(strTransformName),
         strScript));
   strL.Format(System.dir().stage(m_strPlatform+"\\netnode\\%s.lib"), 
         System.dir().path(System.dir().name(strTransformName),
         strScript));
   strE.Format(System.dir().stage(m_strPlatform+"\\netnode\\%s.exp"), 
         System.dir().path(System.dir().name(strTransformName),
         strScript));
   strDVI.Format(System.dir().path(m_strTime, "intermediate\\" + m_strPlatform + "\\netnodescript\\%s\\vc" + m_strSdk1 + ".idb"), strTransformName);
   strDVP.Format(System.dir().path(m_strTime, "intermediate\\" + m_strPlatform + "\\netnodescript\\%s\\vc" + m_strSdk1 + ".pdb"), strTransformName);
   strDPCH.Format(System.dir().path(m_strTime, "intermediate\\" + m_strPlatform + "\\netnodescript\\%s\\netnodescript.pch"), strTransformName);
   strSVI.Format(System.dir().path(m_strTime, "intermediate\\" + m_strPlatform + "\\netnodescript\\vc" + m_strSdk1 + ".idb"), strTransformName);
   strSVP.Format(System.dir().path(m_strTime, "intermediate\\" + m_strPlatform + "\\netnodescript\\vc" + m_strSdk1 + ".pdb"), strTransformName);
   strSPCH.Format(System.dir().path(m_strTime, "intermediate\\" + m_strPlatform + "\\netnodescript\\netnodescript.pch"), strTransformName);
   pscript->m_strScriptPath = System.dir().path(
      System.dir().stage(m_strPlatform+"\\netnode"), 
         System.dir().path(System.dir().name(strTransformName),
         strScript + ".dll"));
#else
   pscript->m_strLibraryPath.Format(System.dir().stage("Release\\%s.dll"), strName);
#endif

   ::DeleteFile(strP);
   ::DeleteFile(strO);
   ::DeleteFile(strL);
   ::DeleteFile(strE);
   //::DeleteFile(pscript->m_strBuildBat);
   try
   {
      if(System.file().exists(pscript->m_strScriptPath + ".old"))
      {
         System.file().del(pscript->m_strScriptPath + ".old");
      }
   }
   catch(string strError)
   {
      TRACE0(strError + "\n");
   }
   try
   {
      if(System.file().exists(pscript->m_strScriptPath))
      {
         System.file().move(pscript->m_strScriptPath + ".old", pscript->m_strScriptPath);
      }
   }
   catch(string strError)
   {
      TRACE0(strError + "\n");
   }
   try
   {
      if(System.file().exists(pscript->m_strScriptPath + ".old"))
      {
         System.file().del(pscript->m_strScriptPath + ".old");
      }
   }
   catch(string strError)
   {
      TRACE0(strError + "\n");
   }
   
   System.dir().mk(System.dir().name(strDVI));
   System.dir().mk(System.dir().name(pscript->m_strBuildBat));
   ::CopyFile(strSVI, strDVI, FALSE);
   ::CopyFile(strSVP, strDVP, FALSE);
   ::CopyFile(strSPCH, strDPCH, FALSE);




   string vars1batSrc;
   string vars2batSrc;
   string vars1batDst;
   string vars2batDst;
   vars1batSrc = System.dir().ca2("app/stage/app/matter/vc10vars64.bat");
   vars2batSrc = System.dir().ca2("app/stage/app/matter/vc10vars_query_registry.bat");
   vars1batDst = System.dir().path(System.dir().name(pscript->m_strBuildBat), "vc10vars64.bat");
   vars2batDst = System.dir().path(System.dir().name(pscript->m_strBuildBat), "vc10vars_query_registry.bat");
   System.file().copy(vars1batDst, vars1batSrc);
   System.file().copy(vars2batDst, vars2batSrc);

   System.dir().mk(System.dir().name(pscript->m_strScriptPath));
   System.dir().mk(System.dir().name(strL));
   System.dir().mk(System.dir().path(System.dir().path(m_strTime, "intermediate\\" + m_strPlatform + "\\netnodescript"), System.dir().name(strTransformName)));
   
   cppize(pscript);

   string strV(System.dir().votagus());
   strV.replace("\\", "/");
   

   string strBuildCmd;
#ifdef _DEBUG
   strBuildCmd.Format(System.dir().votagus("app\\stage\\app\\matter\\netnodecld" + m_strPlat1 + ".bat"));
#else
   strBuildCmd.Format(System.dir().stage("front\\netnodecl.bat"));
#endif
   str = Application.file().as_string(strBuildCmd);
   str.replace("%ITEM_NAME%", strTransformName);
   str.replace("%ITEM_TITLE%", System.file().name_(strTransformName));
   str.replace("%ITEM_DIR%", System.dir().name(strTransformName));
   str.replace("%LIBS_LIBS%", m_strLibsLibs);
   str.replace("%VS_VARS%", m_strEnv);
   str.replace("%VOTAGUS_ROOT%", strV);
   str.replace("%PLATFORM%", m_strPlatform);
   str.replace("%LIBPLATFORM%", m_strLibPlatform);
   str.replace("%SDK1%", m_strSdk1);
   string strTargetPath = pscript->m_strScriptPath;
   gen::str::ends_eat_ci(strTargetPath, ".dll");
   str.replace("%TARGET_PATH%", strTargetPath);
   strBuildCmd = pscript->m_strBuildBat;
   System.file().put_contents(strBuildCmd, str);

   //System.file().put_contents(strLinkCmd, str);

   STARTUPINFO si;
   PROCESS_INFORMATION pi; 
   memset(&si, 0, sizeof(si));
   memset(&pi, 0, sizeof(pi));
   si.cb = sizeof(si);
   si.dwFlags = STARTF_USESHOWWINDOW;
   si.wShowWindow = SW_HIDE; 
   ::CreateProcess(NULL, (LPTSTR) (const char *) strBuildCmd, NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, System.dir().name(pscript->m_strBuildBat), &si, &pi);

   bool bTimeout = false;
   DWORD dwStart = ::GetTickCount();
   DWORD dwExitCode;
   while(true)
   {
      if(!GetExitCodeProcess(pi.hProcess, &dwExitCode))
         break;
      if(dwExitCode != STILL_ACTIVE)
         break;
      Sleep(284);
      if(::GetTickCount() - dwStart > 5 * 60 * 1000) // 5 minutes
      {
         bTimeout = true;
         break;
      }
   }
   //system(strCmd);

//   TRACE0(strBuildCmd);
  // var varBuildOutput;
   //varBuildOutput = System.process().get_output(strBuildCmd);

   /*TRACE0(strLinkCmd);
   var varLinkOutput;
   varLinkOutput = System.process().get_output(strLinkCmd);*/


   pscript->m_memfileError.Truncate(0);
   pscript->m_memfileError << "<pre>";

   pscript->m_memfileError << "Compiling...\n";
   pscript->m_memfileError << pscript->m_strCppPath;
   pscript->m_memfileError << "\n";
   if(bTimeout)
   {
      pscript->m_memfileError << "error: Timeout during compilation (If there are the compilation or link errors about the file \"" + pscript->m_strCppPath +"\" following this message, they may be out-of-date)";
   }
   str.Format(System.dir().path(m_strTime, "netnode\\%s-compile-log.txt"), strTransformName);
   str = Application.file().as_string(str);
   str.replace("\r\n", "\n");
   pscript->m_memfileError << str;
   
   pscript->m_memfileError<< "Linking...\n";
   str.Format(System.dir().path(m_strTime, "netnode\\%s-link-log.txt"), strTransformName);
   str = Application.file().as_string(str);
   str.replace("\r\n", "\n");
   pscript->m_memfileError << str;
   pscript->m_memfileError << "</pre>";

   ::DeleteFile(strDVI);
   ::DeleteFile(strDVP);
   ::DeleteFile(strDPCH);


   pscript->m_dwLastBuildTime = ::GetTickCount();
   
   // Wait for finalization of build
   // or delay in case of error to avoid run conditions due extreme overload.
   //Sleep(pscript->m_pmanager->m_dwBuildTimeWindow +
     // System.math().RandRange(0, pscript->m_pmanager->m_dwBuildTimeRandomWindow));
   pscript->m_bShouldBuild =false;

   HANDLE h = ::CreateFile(pscript->m_strSourcePath, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
   memset(&pscript->m_ftCreation, 0, sizeof(FILETIME));
   memset(&pscript->m_ftAccess, 0, sizeof(FILETIME));
   memset(&pscript->m_ftModified, 0, sizeof(FILETIME));
   GetFileTime(
      h , 
      &pscript->m_ftCreation, 
      &pscript->m_ftAccess, 
      &pscript->m_ftModified);
   ::CloseHandle(h);

}

string base_name(const char * lpcsz)
{
   string str(lpcsz);
   int iPos1 = str.reverse_find('\\');
   int iPos2 = str.reverse_find('/');
   if(iPos1 > iPos2)
   {
      return str.Left(iPos1);
   }
   else if(iPos2 >= 0)
   {
      return str.Left(iPos2);
   }
   else
   {
      return str;
   }
}


bool ensure_dir(const char * lpcsz)
{
   stringa stra;
   string str = lpcsz;
   while((::GetFileAttributes(str) & FILE_ATTRIBUTE_DIRECTORY) == 0
      || (::GetFileAttributes(str) == INVALID_FILE_ATTRIBUTES))
   {
      stra.add(str);
      str = base_name(str);
   }
   for(int i = stra.get_size() - 1; i >= 0; i-- )
   {
      if(!::CreateDirectory(stra[i], NULL))
         return false;
   }
   return true;
}


void netnodeScriptCompiler::cppize(netnodeScript * pscript)
{
   ensure_dir(base_name(pscript->m_strCppPath));
   cppize1(pscript);
}

string escape(const char * lpcsz)
{
   string str(lpcsz);
   str.replace("\\", "\\\\");
   str.replace("\"", "\\\"");
   str.replace("\r\n", "\\r\\n");
   str.replace("\r", "\\r\\n");
   str.replace("\n", "\\r\\n");
   str.replace("\t", "\\t");
   return str;
}

void netnodeScriptCompiler::cppize1(netnodeScript * pscript)
{
   /*ex1::filesp spfile(get_app());
   if(!spfile->open(pscript->m_strSourcePath, ::ex1::file::type_binary | ::ex1::file::mode_read | ::ex1::file::shareDenyNone))
      return;

   primitive::memory memstorage;
   memstorage.allocate(spfile->get_length() + 1);
   memstorage.GetAllocation()[memstorage.GetStorageSize() - 1] = '\0';
   memstorage.read(file);*/
   string strSource = Application.file().as_string(pscript->m_strSourcePath);

   string strDest;
   strDest = "";
   strDest += "#include \"StdAfx.h\"\r\n";
   strDest += "#include \"11ca2_fontopus.h\"\r\n";
   for(int i = 0; i < m_straLibIncludePath.get_count(); i++)
   {
      string str;
      str = m_straLibIncludeRelPath[i];
      gen::str::ends_eat_ci(str, ".ds");
      strDest += "#include \""+str+".h\"\r\n";
   }
   int iStart = 0;
   int iPos = 0;
   int iLastEnd = 0;
   if(strSource.Mid(0, 4) == "<?ss")
   {
      iLastEnd = strSource.find("?>", iPos);
      if(iLastEnd > 0)
      {
         iLastEnd += 2;
         strDest += cppize2(strSource.Mid(iPos + 4, iLastEnd - iPos - 6), false);
         iStart = iLastEnd;
      }
   }
   else if(strSource.Mid(0, 7) == "<? //ss")
   {
      iLastEnd = strSource.find("?>", iPos);
      if(iLastEnd > 0)
      {
         iLastEnd += 2;
         strDest += cppize2(strSource.Mid(iPos + 7, iLastEnd - iPos - 7 - 2), false);
         iStart = iLastEnd;
      }
   }
   strDest += "\r\n";
   strDest += "class ds_script : public netnodeScriptInstance\r\n";
   strDest += "{\r\n";
   strDest += "public:\r\n";
   strDest += "   ds_script(netnodeScript * pscript) : netnodeScriptInstance(pscript) {};  \r\n";
   strDest += "   virtual void run();\r\n";
   strDest += "};\r\n";
   strDest += "\r\n";
   strDest += "extern \"C\" netnodeScriptInstance * __cdecl create_netnode_script_instance (netnodeScript * pscript)\r\n";
   strDest += "{\r\n";
   strDest += "   return dynamic_cast < netnodeScriptInstance * > (new ds_script(pscript));\r\n";
   strDest += "}\r\n";
   strDest += "\r\n";
   strDest += "void ds_script::run()\r\n";
   strDest += "{\r\n";
   strDest += "//Start parsed user script\r\n";
   while((iPos = strSource.find("<?", iStart)) >= 0)
   {
      if(iPos > iLastEnd)
      {
         strDest += get_ds_print(strSource.Mid(iLastEnd, iPos - iLastEnd));
      }
      iLastEnd = strSource.find("?>", iPos);
      if(iLastEnd < 0)
         break;
      iLastEnd += 2;
      int iShift = 0;
      if(strSource.Mid(iPos, 5).CompareNoCase("<?php") == 0)
         iShift = 3;
      strDest += cppize2(strSource.Mid(iPos + 2 + iShift, iLastEnd - iPos - 4 - iShift), true);
      iStart = iLastEnd;
   }
   strDest += get_ds_print(strSource.Mid(iStart));
   strDest += "//End parsed user script\r\n";
   strDest += "}\r\n";

   System.file().put_contents_utf8(pscript->m_strCppPath, strDest);

}

void netnodeScriptCompiler::prepare1(const char * lpcszSource, const char * lpcszDest)
{
   string strVotagusFolder;
   strVotagusFolder = System.dir().votagus();
   string strTemplate;
   string strSource = "app\\stage\\app\\matter\\";
   strSource += lpcszSource;

#ifdef _DEBUG
   strTemplate = System.dir().path(strVotagusFolder, strSource );
#else
   strTemplate = System.dir().path(strVotagusFolder, "app\\stage\\ca2\\fontopus\\app\\main\\matter\\netnodecl.bat");
#endif
   string str;
   str = Application.file().as_string(strTemplate);
   /*string strVars = getenv("VS100COMNTOOLS");
   System.file().path().eat_end_level(strVars, 2, "\\");
   strVars += "vc\\bin\\vcvars32.bat";*/
   str.replace("%VS_VARS%", m_strEnv);
   string strV(strVotagusFolder);
   strV.replace("\\", "/");
   str.replace("%VOTAGUS_ROOT%", strV);
   string strDest = "stage\\front\\";
   strDest += lpcszDest;
   string strCmd;
#ifdef _DEBUG
   strCmd = System.dir().path(strVotagusFolder, strDest);
#else
   strCmd = System.dir().path(strVotagusFolder, "app\\stage\\ca2\\fontopus\\app\\main\\front\\netnodecl.bat");
#endif
   System.dir().mk(System.dir().name(strCmd));
   System.file().put_contents(strCmd, str);
   System.dir().mk(System.dir().path(m_strTime, "netnode\\"));
}



void netnodeScriptCompiler::on_file_action(::ex2::folder_watch::e_action eaction, const char * pszFolder, const char * psz)
{
   UNREFERENCED_PARAMETER(eaction);
   string str = System.dir().path(pszFolder, psz);
   string strTransfer = str;

   // does not transfer
   if(gen::str::find_ci("netnode_persistent_ui_str", strTransfer) >= 0)
      return;

   if(m_straSync.get_count() > 1)
   {
      for(int i = 1; i < m_straSync.get_count(); i++)
      {
         Application.http().get("http://" + m_straSync[i] + "/sync?src=" +m_straSync[0] + "&url=" + System.url().url_encode(strTransfer) + "&pwd=sym123&authnone=1");
      }
   }
   int i = 0;
   while(i < str.get_length() && str[i] != '\\' && str[i] != '/')
   {
      i++;
   }
   if(gen::str::begins(str, System.dir().netseed("ds\\ca2\\library\\include")))
   {
      compile_library();
      m_pmanager->m_pcache->set_all_out_of_date();
   }
   else if(gen::str::begins(str, System.dir().netseed("ds\\ca2\\library\\source")))
   {
      compile_library();
   }
   else if(defer_run_persistent(str))
   {
   }

}

void netnodeScriptCompiler::folder_watch()
{

   m_folderwatch->m_pcallback = this;
   m_folderwatch->watch(System.dir().netseed("ds\\ca2\\"));
   m_folderwatchFribox->m_pcallback = this;
   m_folderwatchFribox->watch("V:\\fribox\\ds\\");

}

void netnodeScriptCompiler::compile_library()
{
   CSingleLock slLibrary(&m_mutexLibrary, TRUE);

   unload_library();
   string strName = "library";

   m_strLibsLibs = System.dir().ca2("time/library/" + m_strPlatform + "/library/library.lib");

   m_memfileLibError.Truncate(0);
   string strVotagusFolder;
   strVotagusFolder = System.dir().votagus();
   m_straLibSourcePath.remove_all();
   m_straLibSourceRelPath.remove_all();
   System.dir().rls(System.dir().netseed("ds\\ca2\\library\\source"),  &m_straLibSourcePath, NULL, &m_straLibSourceRelPath);
   for(int i = 0; i < m_straLibSourcePath.get_size(); )
   {
      if(System.file().extension(m_straLibSourcePath[i]) != "ds")
      {
         m_straLibSourcePath.remove_at(i);
         m_straLibSourceRelPath.remove_at(i);
      }
      else
      {
         i++;
      }
   }
   m_straLibCppPath.remove_all();
   for(int i = 0; i < m_straLibSourcePath.get_size(); i++ )
   {
      string str = m_straLibSourceRelPath[i];
      gen::str::ends_eat_ci(str, ".ds");
      str.replace(":", "");
      m_straLibCppPath.add(System.dir().path(
         System.dir().path(m_strTime, "netnode\\library\\source"),
         str + ".cpp"));
   }
   m_straLibIncludePath.remove_all();
   m_straLibIncludeRelPath.remove_all();
   System.dir().rls(System.dir().netseed("ds\\ca2\\library\\include"),  &m_straLibIncludePath, NULL, &m_straLibIncludeRelPath);
   for(int i = 0; i < m_straLibIncludePath.get_size(); )
   {
      if(System.file().extension(m_straLibIncludePath[i]) != "ds"
         || gen::str::find_ci(m_straLibIncludePath[i], "\\.svn\\") >= 0
         || System.dir().is(m_straLibIncludePath[i]))
      {
         m_straLibIncludePath.remove_at(i);
         m_straLibIncludeRelPath.remove_at(i);
      }
      else
      {
         i++;
      }
   }
   m_straLibHppPath.remove_all();
   for(int i = 0; i < m_straLibIncludePath.get_size(); i++ )
   {
      string str = m_straLibIncludeRelPath[i];
      gen::str::ends_eat_ci(str, ".ds");
      str.replace(":", "");
      m_straLibHppPath.add(System.dir().path(
         System.dir().path(m_strTime, "netnode\\library\\include"),
         str + ".h"));
   }

   string strLib(System.file().name_(strName));

   
#ifdef _DEBUG
   m_strLibraryPath.Format(
      System.dir().stage(m_strPlatform + "\\netnode\\library\\%s.dll"), 
         System.dir().path(System.dir().name(strName),
         strLib));
#else
   plib->m_strLibraryPath.Format(System.dir().path(strVotagusFolder, "app\\stage\\ca2\\fontopus\\app\\main\\front\\Release\\%s.dll"), strName);
#endif

   System.dir().mk(System.dir().name(m_strLibraryPath));
   System.dir().mk(System.dir().path(System.dir().path(m_strTime, "intermediate\\" + m_strPlatform + "\\netnodelibrary\\library"), System.dir().name(strName)));

   for(int i = 0; i < m_straLibIncludePath.get_size(); i++)
   {
      cppize(m_straLibIncludePath[i], m_straLibHppPath[i], cpptype_include);
   }
   
   for(int i = 0; i < m_straLibSourcePath.get_size(); i++)
   {
      cppize(m_straLibSourcePath[i], m_straLibCppPath[i], cpptype_source);
      string strRel = m_straLibSourceRelPath[i];
      gen::str::ends_eat_ci(strRel, ".ds");
      strRel.replace("\\", "/");
      string str1;
      str1 = "library/source/" + strRel;
      string strCmd;
#ifdef _DEBUG
   strCmd.Format(System.dir().ca2("stage\\front\\netnodelibcd" + m_strPlat1 + ".bat"));
#else
   strCmd.Format(System.dir().path(strVotagusFolder, "app\\stage\\ca2\\fontopus\\app\\main\\front\\netnodecl.bat"));
#endif
   string str = Application.file().as_string(strCmd);
   str.replace("%ITEM_NAME%", str1);
   str.replace("%ITEM_DIR%", System.dir().name(str1));
   str.replace("%PLATFORM%", m_strPlatform);
   str.replace("%LIBPLATFORM%", m_strLibPlatform);
   str.replace("%SDK1%", m_strSdk1);
   System.dir().mk(System.dir().path(m_strTime, "intermediate\\" + m_strPlatform + "\\netnodelibrary\\" + System.dir().name(str1)));
   System.dir().mk(System.dir().path(m_strTime, "library\\" + m_strPlatform + "\\" + System.dir().name(str1)));
   strCmd = System.dir().ca2("stage\\front\\netnodelibc1.bat");

   string vars1batSrc;
   string vars2batSrc;
   string vars1batDst;
   string vars2batDst;
   vars1batSrc = System.dir().ca2("app/stage/app/matter/vc10vars64.bat");
   vars2batSrc = System.dir().ca2("app/stage/app/matter/vc10vars_query_registry.bat");
   vars1batDst = System.dir().stage("stage\\front", "vc10vars64.bat");
   vars2batDst = System.dir().stage("stage\\front", "vc10vars_query_registry.bat");
   ::CopyFile(vars1batSrc, vars1batDst, FALSE);
   ::CopyFile(vars2batSrc, vars2batDst, FALSE);

   System.file().put_contents(strCmd, str);
   STARTUPINFO si;
   PROCESS_INFORMATION pi; 
   memset(&si, 0, sizeof(si));
   memset(&pi, 0, sizeof(pi));
   si.cb = sizeof(si);
   si.dwFlags = STARTF_USESHOWWINDOW;
   si.wShowWindow = SW_HIDE; 
   ::CreateProcess(NULL, (LPTSTR) (const char *) strCmd, NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, System.dir().ca2("stage\\front"), &si, &pi);

   DWORD dwExitCode;
   while(true)
   {
      if(!GetExitCodeProcess(pi.hProcess, &dwExitCode))
         break;
      if(dwExitCode != STILL_ACTIVE)
         break;
      Sleep(200);
   }
   m_memfileLibError<< "<html><head></head><body><pre>";
   str.Format(System.dir().path(m_strTime, "netnode\\%s-compile-log.txt"), str1);
   str = Application.file().as_string(str);
   str.replace("\r\n", "</pre><pre>");
   m_memfileLibError << str;
   //system(strCmd);
   }

   string strObjs;
   for(int i = 0; i < m_straLibSourcePath.get_size(); i++)
   {
      strObjs += " ";
      string strRel = m_straLibSourceRelPath[i];
      gen::str::ends_eat_ci(strRel, ".ds");
      strObjs += System.dir().path(
         System.dir().path(m_strTime, "intermediate\\" + m_strPlatform + "\\netnodelibrary\\library\\source"),
         strRel + ".obj");
      strObjs += " ";
   }
   string strCmd;
#ifdef _DEBUG
   strCmd.Format(System.dir().stage("front\\netnodelibld" + m_strPlat1 + ".bat"));
#else
   strCmd.Format(System.dir().path(strVotagusFolder, "app\\stage\\ca2\\fontopus\\app\\main\\front\\netnodecl.bat"));
#endif
   string str = Application.file().as_string(strCmd);
   str.replace("%ITEM_NAME%", "library\\" + strName);
   str.replace("%ITEM_DIR%", "library");
   str.replace("%OBJS%", strObjs);
   str.replace("%PLATFORM%", m_strPlatform);
   str.replace("%LIBPLATFORM%", m_strLibPlatform);
   str.replace("%SDK1%", m_strSdk1);
   string strTargetName = m_strLibraryPath;
   gen::str::ends_eat_ci(strTargetName, ".dll");
   str.replace("%TARGET_NAME%", strTargetName);
   System.dir().mk(System.dir().ca2("stage\\" + m_strPlatform + "\\library"));
   strCmd = System.dir().ca2("stage\\front\\netnodelibl1.bat");
   System.file().put_contents(strCmd, str);
   STARTUPINFO si;
   PROCESS_INFORMATION pi; 
   memset(&si, 0, sizeof(si));
   memset(&pi, 0, sizeof(pi));
   si.cb = sizeof(si);
   si.dwFlags = STARTF_USESHOWWINDOW;
   si.wShowWindow = SW_HIDE; 
   ::CreateProcess(NULL, (LPTSTR) (const char *) strCmd, NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, System.dir().ca2("stage\\front"), &si, &pi);

   DWORD dwExitCode;
   while(true)
   {
      if(!GetExitCodeProcess(pi.hProcess, &dwExitCode))
         break;
      if(dwExitCode != STILL_ACTIVE)
         break;
      Sleep(200);
   }
   
   str.Format(System.dir().path(m_strTime, "netnode\\library\\%s-link-log.txt"), strName);
   //str.Format("V:\\time\\ca2\\fontopus\\net\\netnode\\%s-build-log.txt", lpcszName);
   str = Application.file().as_string(str);
   str.replace("\r\n", "</pre><pre>");
   m_memfileLibError << str;
   m_memfileLibError << "</pre></body></html>";
   load_library();
}



void netnodeScriptCompiler::cppize(const char * lpcszSource, const char * lpcszDest, ecpptype e_type)
{
   ensure_dir(base_name(lpcszDest));
   cppize1(lpcszSource, lpcszDest, e_type);
}

void netnodeScriptCompiler::cppize1(const char * lpcszSource, const char * lpcszDest, ecpptype e_type)
{

   string strSource = Application.file().as_string(lpcszSource);

   bool bCode = false;

   string strDest;
   strDest = "";
   strDest = "";
   if(e_type == cpptype_source || e_type == cpptype_ds)
   {
      strDest += "#include \"StdAfx.h\"\r\n";
      strDest += "#include \"11ca2_fontopus.h\"\r\n";
      for(int i = 0; i < m_straLibIncludePath.get_count(); i++)
      {
         string str;
         str = m_straLibIncludeRelPath[i];
         gen::str::ends_eat_ci(str, ".ds");
         strDest += "#include \""+str+".h\"\r\n";
      }
   }
   int iStart = 0;
   int iPos = 0;
   int iLastEnd = 0;
   while((iPos = strSource.find("<?", iStart)) >= 0)
   {
      if(iPos > iLastEnd && bCode)
      {
         strDest += get_ds_print(strSource.Mid(iLastEnd, iPos - iLastEnd));
      }
      iLastEnd = strSource.find("?>", iPos);
      if(iLastEnd < 0)
         break;
      iLastEnd += 2;
      int iShift = 0;
      if(strSource.Mid(iPos, 5).CompareNoCase("<?php") == 0)
         iShift = 3;
      bCode = true;
      strDest += cppize2(strSource.Mid(iPos + 2 + iShift, iLastEnd - iPos - 4 - iShift), false);
      iStart = iLastEnd;
   }
/*   strDest += "   print(\"" ;
   strDest += escape(strSource.Mid(iStart));
   strDest += "\");\r\n";
   strDest += "//End parsed user script\r\n";
   strDest += "}\r\n";*/

   System.file().put_contents_utf8(lpcszDest, strDest);
   

}



string netnodeScriptCompiler::cppize2(const char * psz, bool bScript)
{
   string str(psz);
   str.trim();
   bool bInSimpleQuote = false;
   bool bInDoubleQuote = false;
   bool bInVar = false;
   bool bInSlash = false;
   bool bInRet = false;
   bool bInSpec1 = false;
   bool bInSpec1Close = false;
   bool bInSpec2 = false;
   bool bInSpec2Close = false;
   int iBracket = 0;
   bool bInBrace = false;
   int iIdLen;
   char ch;
   char chNext;
   string strResult;
   int iLen = str.get_length();
   int i = 0;
   bool bInitial = true;
   string strSpec1;
   int iOpenParen = 1; // open Parenthesis Count
   stringa straFunction;
   int_array iaFunctionParen; // index of the parenthesis of the function
   bool bInserted = false;
   while(i < iLen)
   {
      bInserted = false;
      ch = str[i];
      chNext = str[i + 1];
      if(gen::str::begins(str.Mid(i), "bk_filter_active1"))
      {
         //AfxDebugBreak();
      }
      if(bInSpec1)
      {
         if(isspace(ch))
         {
            i++;
            continue;
         }
         else if(ch == '(')
         {
            strResult += strSpec1 + "(";
            bInSpec1 = false;
            i++;
            continue;
         }
         else
         {
            strResult += strSpec1 + "(";
            bInSpec1Close = true;
            bInSpec1 = false;
         }
      }
      if(bInSpec2)
      {
         if(isspace(ch))
         {
            i++;
            continue;
         }
         else if(ch == '=')
         {
            strResult += ".propset().add(NULL, ";
            bInSpec2 = false;
            bInSpec2Close = true;
            i++;
            continue;
         }
         else
         {
            ///erro
         }
      }
      if(iBracket > 0 && ch == ']')
      {
         strResult += ch;
         iBracket--;
      }
      else if(bInSimpleQuote)
      {
         if(bInSlash)
         {
            bInSlash = false;
            strResult += ch;
         }
         else
         {
            if(ch == '\\')
            {
               bInSlash = true;
               if(chNext != '{'
               && chNext != '}'
               && chNext != '$')
               {
                  strResult += ch;
               }
            }
            else if(ch == '\'')
            {
               bInSimpleQuote = false;
               strResult += "\")";
            }
            else
            {
               bInSlash = false;
               strResult += ch;
            }
         }
      }
      else if(bInVar)
      {
         if(isdigit(ch) || isalpha(ch) || ch == '_')
         {
            strResult += ch;
         }
         else
         {
            bInVar = false;
            if(bInDoubleQuote)
            {
               if(ch == '$')
               {
                  bInVar = true;
                  strResult += "\") + gstr(\"";
                  i++;
                  continue;
               }
               else
               {
                  strResult += "\") + unitext(\"";
                  if(ch == '\"')
                  {
                     bInDoubleQuote = false;
                     strResult += "\")";
                     i++;
                     continue;
                  }
                  else if(ch == '\\')
                  {
                     bInSlash = true;
                  }
               }
            }
            else
            {
               while(isspace(str[i]) && i < iLen)
               {
                  i++;
               }
               ch = str[i];
               if(ch == '[' && chNext != ']')
               {
                  iBracket++;
                  strResult += "\").propset()";
               }
               else if(ch == '-' && chNext == '>')
               {
                  i+=2;
                  while(isspace(str[i]) && i < iLen)
                  {
                     i++;
                  }
                  ch = str[i];
                  string strToken;
                  if(isalpha(ch) || ch == '_')
                  {
                     strToken += ch;
                     i++;
                  }
                  else
                  {
                     strResult = "-> error";
                     return strResult;
                  }
                  ch = str[i];
                  while(isdigit(ch) || isalpha(ch) || ch == '_')
                  {
                     strToken += ch;
                     i++;
                     ch = str[i];
                  }
                  ch = str[i];
                  while(isspace(ch))
                  {
                     i++;
                     ch = str[i];
                  }
                  if(ch == '(')
                  {
                     i++;
                     ch = str[i];
                     iaFunctionParen.add(iOpenParen);
                     iOpenParen++;
                     straFunction.add(strToken);
                     while(isspace(ch))
                     {
                        i++;
                        ch = str[i];
                     }
                     if(ch == ')')
                     {
                        strResult += "\").ca2 < ::dso_base >()->call(\""+ strToken + "\" ";
                     }
                     else
                     {
                        strResult += "\").ca2 < ::dso_base >()->call(\""+ strToken + "\", ";
                        continue;
                     }
                  }
                  else
                  {
                     strResult += "\").ca2 < ::dso_base >()->m_propertyset[\""+ strToken + "\"]";
                  }
               }
               else
               {
                  strResult += "\")";
               }
            }
            if(ch == ';')
               goto ch_comma;
            else
               goto ch_else;
            //strResult += ch;
         }
      }
      else if(bInDoubleQuote)
      {
         if(bInSlash)
         {
            bInSlash = false;
            strResult += ch;
         }
         else
         {
            if(ch == '$' && (isalpha(chNext) || chNext == '_'))
            {
               bInVar = true;
               strResult += "\") + gstr(\"";
            }
            else if(ch == '{')
            {
               bInBrace = true;
               bInDoubleQuote = false;
               strResult += "\") + ";
            }
            else if(ch == '\\')
            {
               bInSlash = true;
               if(chNext != '{'
               && chNext != '}'
               && chNext != '$')
               {
                  strResult += ch;
               }
            }
            else if(ch == '\"')
            {
               bInDoubleQuote = false;
               strResult += "\")";
            }
            else
            {
               bInSlash = false;
               strResult += ch;
            }
         }
      }
      else if(ch == '\"')
      {
         bInDoubleQuote = true;
         strResult += "unitext(\"";
      }
      else if(ch == '\'')
      {
         bInSimpleQuote = true;
         strResult += "unitext(\""; // overloads should cope with the possibility of conversion between string to character
      }
      else if(ch == '$' && (isalpha(chNext) || chNext == '_'))
      {
         if(bInVar)
         {
            if(bInDoubleQuote)
            {
               strResult += "\") + gstr(\"";
            }
            else
            {
               strResult += "\")"; // probably will generate compile error, leave to c++ compiler
            }
         }
         else
         {
            bInVar = true;
            if(bInDoubleQuote)
            {
               strResult += "\") + gstr(\"";
            }
            else
            {
               if(is_id(str.Mid(i + 1), "_GET", iIdLen))
               {
                  strResult += "geta()";
                  bInVar = false;
                  i += iIdLen;
               }
               else if(is_id(str.Mid(i + 1), "_POST", iIdLen))
               {
                  strResult += "posta()";
                  bInVar = false;
                  i += iIdLen;
               }
               else if(is_id(str.Mid(i + 1), "_REQUEST", iIdLen))
               {
                  strResult += "requesta()";
                  bInVar = false;
                  i += iIdLen;
               }
               else if(is_id(str.Mid(i + 1), "_SERVER", iIdLen))
               {
                  strResult += "inattra()";
                  bInVar = false;
                  i += iIdLen;
               }
               else if(is_id(str.Mid(i + 1), "_SESSION", iIdLen))
               {
                  strResult += "sessiona()";
                  bInVar = false;
                  i += iIdLen;
               }
               else if(is_id(str.Mid(i + 1), "_COOKIE", iIdLen))
               {
                  strResult += "cookies()";
                  bInVar = false;
                  i += iIdLen;
               }
               else
               {
                  strResult += "gprop(\"";
               }
            }
         }
      }
      else if(ch == ';')
      {
ch_comma:
         if(bInRet)
         {
            bInRet = false;
            strResult += ch;
            strResult += "\r\nreturn;\r\n}\r\n";
         }
         else if(bInSpec1Close)
         {
            bInSpec1Close = false;
            strResult += ")";
            strResult += ch;
         }
         else if(bInSpec2Close)
         {
            bInSpec2Close = false;
            strResult += ")";
            strResult += ch;
         }
         else
         {
            strResult += ch;
         }
         bInitial = true;
      }
      else if(ch == '(')
      {
         strResult += ch;
         iOpenParen++;
      }
      else if(ch == ')')
      {
         iOpenParen--;
         //if(iaFunctionParen.pop_match('');
         strResult += ch;
      }
      else
      {
ch_else:
         if(bScript && is_id(str.Mid(i), "return", iIdLen) && next_nonspace(str.Mid(i + iIdLen))[0] != ';')
         {
            bInRet = true;
            strResult += "\r\n{\r\nm_varRet = ";
            i += iIdLen - 1;
         }
         else if(is_id(str.Mid(i), "include", iIdLen))
         {
            bInSpec1 = true;
            strSpec1 = "include";
            i += iIdLen - 1;
         }
         else if(is_id(str.Mid(i), "print", iIdLen))
         {
            bInSpec1 = true;
            strSpec1 = "print";
            i += iIdLen - 1;
         }
         else if(is_id(str.Mid(i), "echo", iIdLen))
         {
            bInSpec1 = true;
            strSpec1 = "echo";
            i += iIdLen - 1;
         }
         else if(str.Mid(i, 2) == "[]")
         {
            bInSpec2 = true;
            i += 2 - 1;
         }
         else if(bInBrace && ch == '}')
         {
            bInDoubleQuote = true;
            bInBrace = false;
            strResult += " + unitext(\"";
         }
         else
         {
            strResult += ch;
         }
      }
      i++;
   }
   return strResult;

}


bool netnodeScriptCompiler::is_id(const char * psz, const char * pszId, int & iIdLenRet)
{
   int iLen = strlen(psz);
   int iIdLen = strlen(pszId); 
   if(iLen < iIdLen)
      return false;
   if(iIdLen == iLen)
   {
      if(!strcmp(psz, pszId))
      {
         iIdLenRet = iIdLen;
         return true;
      }
   }
   if(gen::str::begins(psz, pszId)
      && !isdigit(psz[iIdLen]) && !isalpha(psz[iIdLen]) && psz[iIdLen] != '_')
   {
      iIdLenRet = iIdLen;   
      return true;
   }
   return false;
}

const char * netnodeScriptCompiler::next_nonspace(const char * psz)
{
   while(*psz != '\0' && isspace(*psz))
   {
      psz++;
   }
   return psz;
}

void netnodeScriptCompiler::run_persistent()
{
   string strPath = System.dir().netseed("ds\\ca2\\ca2\\persistent");
   stringa stra;
   System.dir().rls(strPath, &stra);

   string strCat;
   strCat = System.dir().netseed("ds\\ca2\\ca2\\netnode_persistent_ui_str.ds");
   string strBody;
   strBody = "<?\r\n";
   strBody += "// ATTENTION!\r\n";
   strBody += "// Do not edit this file\r\n";
   strBody += "// This file is automatically generated as the\r\n";
   strBody += "// concatenation of all .ds files in thisr\r\n";
   strBody += "// folder and subfolders!\r\n";
   strBody += "\r\n";
   strBody += "?>\r\n\r\n\r\n";
   for(int i = 0; i < stra.get_size(); i++)
   {
      string str = stra[i];
      if(gen::str::begins_ci(str, System.dir().netseed("ds\\ca2\\ca2\\persistent"))
         && gen::str::ends_ci(str, ".ds"))
      {
         strBody += Application.file().as_string(str);
      }
   }
   System.file().put_contents_utf8(strCat, strBody);
   string strInclude = strCat;
//   defer_run_persistent(str);
   gen::str::begins_eat_ci(strInclude, System.dir().netseed("ds\\ca2\\"));
   gen::str::ends_eat_ci(strInclude, ".ds");
   netnodeScriptInstance * pinstance = m_pmanager->get(strInclude);
   if(pinstance != NULL)
   {
      string strError;
      pinstance->m_pmanager = m_pmanager;
      pinstance->m_pscript->m_memfileError.seek_to_begin();
      pinstance->m_pscript->m_memfileError.to_string(strError);
      m_pmanager->m_strPersistentError += strError;
      pinstance->run();
      pinstance->destroy();
   }
}


bool netnodeScriptCompiler::defer_run_persistent(const char *psz)
{
   string str(psz);
   
   if(gen::str::find_ci("pstr_set", psz) && gen::str::ends_ci(psz, ".txt"))
   {
      string strCat;
      strCat = System.dir().netseed("ds\\ca2\\ca2\\netnode_persistent_ui_str.ds");
      string strInclude = strCat;
      gen::str::begins_eat_ci(strInclude, System.dir().netseed("ds\\ca2\\"));
      gen::str::ends_eat_ci(strInclude, ".ds");
      netnodeScriptInstance * pinstance = m_pmanager->get(strInclude);
      if(pinstance != NULL)
      {
         string strError;
         pinstance->m_pmanager = m_pmanager;
         pinstance->m_pscript->m_memfileError.seek_to_begin();
         pinstance->m_pscript->m_memfileError.to_string(strError);
         m_pmanager->m_strPersistentError += strError;
         pinstance->run();
         pinstance->destroy();
      }
   }
   else if(gen::str::begins_eat_ci(str, System.dir().netseed("ds\\ca2\\ca2\\persistent"))
      && gen::str::ends_eat_ci(str, ".ds")
      && str.CompareNoCase("netnode_persistent_ui_str") != 0)
   {
      run_persistent();
      return true;
   }
   return false;
}


bool netnodeScriptCompiler::library_DoesMatchVersion()
{
   CSingleLock slLibrary(&m_mutexLibrary, TRUE);
   for(int i = 0; i < m_straLibSourcePath.get_size(); i++)
   {
      FILETIME ftCreation;
      FILETIME ftAccess;
      FILETIME ftModified;
      memset(&ftCreation, 0, sizeof(FILETIME));
      memset(&ftAccess, 0, sizeof(FILETIME));
      memset(&ftModified, 0, sizeof(FILETIME));
      HANDLE h = ::CreateFile(m_straLibSourcePath[i], GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
      GetFileTime(h, &ftCreation, &ftAccess, &ftModified);
      ::CloseHandle(h);
      if(memcmp(&ftCreation, &m_ftaLibCreation[i], sizeof(FILETIME)) != 0
      || memcmp(&m_ftaLibModified[i], &ftModified, sizeof(FILETIME)) != 0)
      {
         return false;
      }
   }
   return true;
}
void netnodeScriptCompiler::load_library()
{
   CSingleLock slLibrary(&m_mutexLibrary, TRUE);
   m_hmoduleLib = ::LoadLibraryW(
      gen::international::utf8_to_unicode("\\\\?\\" + m_strLibraryPath));
   m_ftaLibCreation.set_size(m_straLibSourcePath.get_size());
   m_ftaLibAccess.set_size(m_straLibSourcePath.get_size());
   m_ftaLibModified.set_size(m_straLibSourcePath.get_size());
   for(int i = 0; i < m_straLibSourcePath.get_size(); i++)
   {
      HANDLE h = ::CreateFile(m_straLibSourcePath[i], GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
      memset(&m_ftaLibCreation[i], 0, sizeof(FILETIME));
      memset(&m_ftaLibAccess[i], 0, sizeof(FILETIME));
      memset(&m_ftaLibModified[i], 0, sizeof(FILETIME));
      GetFileTime(h , &m_ftaLibCreation[i], &m_ftaLibAccess[i], &m_ftaLibModified[i]);
      ::CloseHandle(h);
   }
}


void netnodeScriptCompiler::unload_library()
{
   CSingleLock slLibrary(&m_mutexLibrary, TRUE);
   ::FreeLibrary(m_hmoduleLib);
}

string netnodeScriptCompiler::get_ds_print(const char *psz)
{
   string strSource(psz);
   string strDest;
   while(strSource.get_length() > 0)
   {
      int iLen = min(4 * 1024, strSource.get_length());
      if(strSource.get_length() > iLen)
      {
         if(strSource[iLen] == '\n')
            iLen++;
      }
      string str = strSource.Left(iLen);
      strDest += "   ds_print(unitext(\"";
      strDest += escape(str);
      strDest += "\"));\r\n";
      strSource = strSource.Mid(iLen);
   }
   return strDest;
}
