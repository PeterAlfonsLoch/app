...
namespace music
{
	...
	in2_list_category::in2_list_category(var order, var parentclass, var target)
	{
      ...
      nava = explode(",", gprop("param_nav"));
		...
		{
			music::category * pcategory = musicdb().get_class_for_performers(1, order, parentclass);
...
/*	function getTitle()
	...
	void in2_list_category::print_rows()
	{
		...
		{
			print_row(m_classa.at(i).ca2 < music::category >());
			print("\n");
			m_cur_line++;
			gprop("g_listline")++;
		}
...
//			m_tr_mdwn_parama[m_cur_line] = $link;
	...
	void in2_list_category::print_row(music::category * pcategory)
	{
		gprop("r1path") = "/";
