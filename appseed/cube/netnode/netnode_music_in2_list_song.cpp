...
namespace music
{
   ...
   in2_list(target, order, gen::g_newconst, params)
   {
      ...
      m_partista = new music::performer_ptr_array;
   ...
   void in2_list_song::print_slist_preamble()
   {
      ...
      if(m_querystyle == SQS_LAST_ACCESSES)
      {
         ...
         {
            music::song * psong = m_psonga->element_at(i);
      ...
      if(has_param("cid"))
      {
         music::song_ptr_array * psonga = m_psonga;
         int num_rows             = psonga->get_count();
         music::song_ptr_array * psonga2                = m_psonga2;
         int num_rows2             = psonga2->get_count();
         music::performer * pperformer             = m_pperformer;
...
//      int pos = 0;
      ...
      if(has_param("aid"))
      {
            ...
            i++)
         {
            ...
            if(performeridsa.add_unique_ci(psong->m_propertyset["performerids"]))
            {
               ...
               {
                  music::person * pperformer = psong->m_propertyset["pperformera"].at(j).ca2 < music::person >();
   ...
   void in2_list_song::print_row(music::song * psong, var line)
   {
      /*      global $g_languageid;
      global $param_nav;
