...
namespace netnode
{
   ...
   string music::music_performer_get_reg_exp(var code)
   {
      /*if($code == "*")
      {
         return ".*";
      ...
      else if(preg_match("/^kp-(.+)$/", $code, $matches))
      {
         return "kp-^".$matches[1];
      }
      else
...
string music::karaoke_getFwdParamStyle()
   ...
   var music::file_get_file(var filename, ::music::song ** ppsong)
   {
      var karfileid = file_get_kar_file_id(filename);
      return file_get_file_by_kar_file_id(karfileid, ppsong);
   }
   
   var music::file_get_file_by_kar_file_id(var karfileid, ::music::song ** ppsong)
   {
      dprint("karfileid=" + karfileid);
      ::music::song * psong = musicdb().get_kar_file_song(karfileid);
      if(ppsong != NULL)
      {
         *ppsong = psong;
      }
   ...
   var music::file_test_file(var filename, ::music::song ** ppsong)
   {
