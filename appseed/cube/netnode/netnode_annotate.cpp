...
ca2::note * annotate::get(var user, var objectid)
{
   string strSql;
...
var annotate::insert(var user, var title, var text)
   ...
   void annotate::get_user_summary_list(var user, base_array < ca2::note_summary * > & summary_list)
   {
      string strSql;
      ...
      {
         var row = rowa.at(i);
         ca2::note_summary * psummary = new ca2::note_summary();
