...
namespace paint
{
      ...
      case PaneViewDevEdge:
         {
            create_context cc;
            cc.m_pCurrentDoc = get_document();
            cc.m_typeinfoNewView =  &typeid(html_view);

            ::userbase::view * pview = dynamic_cast < ::userbase::view * > (create_view(typeid(html_view), get_document(), this, 101));
            if(pview != NULL)
            {
               pviewdata->m_pdoc = get_document();
               pviewdata->m_pwnd = pview;
   ...
   {
      view_data * pview = get(PaneViewDevEdge);
      if(pview != NULL)
      {
         return pview->m_pwnd;
      }
