...
namespace mplite
{
   ...
   MediaLibraryListView::MediaLibraryListView(::ca::application * papp) :
      ca(papp),
      ::userbase::view(papp),
   ...
   void MediaLibraryListView::_001InstallMessageHandling(::user::win::message::dispatch * pinterface)
   {
      simple_list_view::_001InstallMessageHandling(pinterface);
      InstallBuffering(pinterface);
      IGUI_WIN_MSG_LINK(WM_LBUTTONDBLCLK, pinterface, this, &MediaLibraryListView::_001OnLButtonDblClk);
      IGUI_WIN_MSG_LINK(WM_TIMER, pinterface, this, &MediaLibraryListView::_001OnTimer);
      IGUI_WIN_MSG_LINK(WM_SIZE, pinterface, this, &MediaLibraryListView::_001OnSize);
      IGUI_WIN_MSG_LINK(WM_CONTEXTMENU, pinterface, this, &MediaLibraryListView::_001OnContextMenu);
      connect_command("ID_ALBUM_EXECUTE_PLAY",  &MediaLibraryListView::_001OnAlbumExecutePlay);
      connect_update_cmd_ui("ID_ALBUM_EXECUTE_PLAY",  &MediaLibraryListView::_001OnUpdateAlbumExecutePlay);
      connect_command("ID_EXECUTE_PREVIOUS",  &MediaLibraryListView::_001OnExecutePrevious);
      connect_update_cmd_ui("ID_EXECUTE_PREVIOUS", &MediaLibraryListView::_001OnUpdateExecutePrevious);
      connect_command("ID_EXECUTE_NEXT", &MediaLibraryListView::_001OnExecuteNext);
      connect_update_cmd_ui("ID_EXECUTE_NEXT",  &MediaLibraryListView::_001OnUpdateExecuteNext);
      IGUI_WIN_MSG_LINK(WM_USER + 1217   ,      pinterface, this ,  &MediaLibraryListView::_001OnFillTaskResponse);
   }



   #ifdef _DEBUG
   void MediaLibraryListView::assert_valid() const
   {
      simple_list_view::assert_valid();
   }

   void MediaLibraryListView::dump(dump_context & dumpcontext) const
   {
   ...
   index MediaLibraryListView::_001GetItemImage(index iItem, index iSubItem, index iListItem)
   {
   ...
   bool MediaLibraryListView::_001GetItemText(string &str, index iItem, index iSubItem, index iListItem)
   {
      UNREFERENCED_PARAMETER(iListItem);
      /*if(simple_list_view::_001GetItemText(
         str,
         iItem, 
         iSubItem, 
         iListItem))
   ...
   //   if(pdoc->m_pdsAlbumRecord->num_rows() <= 0)
     // {
      ...
      //else*/
      {
         ...
         {
            str.Format("%05d", pds->result.records[iItem][(int) eindex].get_integer());
            return true;
         }
         else
         {
   ...
   bool MediaLibraryListView::_001SearchGetItemText(string &str, index iItem, index iSubItem, index iListItem)
   {
      UNREFERENCED_PARAMETER(iListItem);
      MediaLibraryDoc * pdoc = get_document();
      CSingleLock sl(pdoc->m_pcsAlbum1, TRUE);
   ...
   MediaLibraryDoc * MediaLibraryListView::get_document()
   {
      return dynamic_cast < MediaLibraryDoc * > (::view::get_document());
   }

   void MediaLibraryListView::on_update(::view * pSender, LPARAM lHint, ::radix::object* phint) 
   {
   ...
   bool MediaLibraryListView::GetSongPath(string &str, int iItem)
   {
   ...
   void MediaLibraryListView::PostFillTask(string & wstrFile, UINT uiTimer)
   {
      ...
      else if(pbase->m_wparam == 1)
      {
         ...
         if(iId >= 0)
         {
               ...
               iDisplayItem++)
            {
               ...
               if(iId == pds->result.records[iItem][FieldIndexId].get_integer())
               {
                  iFound = iItem;
                  break;
               }
...
//      int iItem;
   ...
   count MediaLibraryListView::_001GetItemCount() 
   {
