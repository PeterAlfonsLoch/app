...
#define TIMEOUT_DEFER_USER_UPDATE_VERIFICATION_CODE_CHECK "+10 minute"
	music_database_cache::music_database_cache()
	{
	}
	

	// $onlycheck - it does only check, but does not update the last user update time
	bool music_database_cache::can_bypass_user_update_verification_code_check(var userid)
	{
...
/*		string lastupdate = musicdb().get_prop_bigint("last_user_update_verification_code_check(var userid)");
	...
	// $onlycheck - it does only check, but does not update the last user update time
void music_database_cache::set_bypass_user_update_verification_code_check(var userid)
	{
/*		DWORD time = ::GetTickCount();
	...
	void music_database_cache::defer_updatecycle1()
	{
/*		DWORD time = ::GetTickCount();
...
//		$strtimeout = "+1 minute";
	...
	void music_database_cache::create_proc()
	{
...
SQL;
	...
	var music_database_cache::get_useragent_id_ex(var useragent)
	{
	...
	var music_database_cache::getClassIdArray2()
	{
...
//      return var;
	...
	void music_database_cache::pulse_access_stats_1()
	{
	...
	void music_database_cache::registerSongClick(var topicid, var address, var http_referer, var lang , var style , var referer ,var type)
	{
...
/*	var music_database_cache::getstring($id)
	{
		return musicdb().get_string($id);
	}

	var music_database_cache::getlangstring($id, $languageid)
	{
		return db_read->getlangstring($id, $languageid);
	}
   */
	void music_database_cache::tmp_song_insert(bool bVerbose)
	{
	...
	void music_database_cache::tmp_song_update_classida(bool bVerbose)
	{
	...
	void music_database_cache::tmp_song_update_performerida(bool bVerbose)
	{
	...
	var music_database_cache::gethostbyaddr_ex(var ip)
	{
      /*
...
//			print("reverse found last gethost: ".datetime_formatDateTime($lastgethost_ts)."<br>");
	...
	bool music_database_cache::access_stats_1_gethostbyaddr(var ip, string &hostname, string &lastvisit, string &lastgethost)
	{
...
/*				print("access_stats_1_gethostbyaddr<br>");
	...
	var music_database_cache::access_stats_1_updatehost(var ip)
	{
	...
	void music_database_cache::update_tmp_song_clickdistinct1_item(var song_click_id, var topicid, var langid, var styleid, bool bVerbose)
	{
		karaoke_update_tmp_song_clickdistinct1_item(song_click_id, topicid, langid, styleid, bVerbose);
	}

	void music_database_cache::song_cache_initialize(bool bVervose)
	{
		string strSql = "UPDATE song_click, lang SET song_click.langid = lang.id WHERE lang.lang = song_click.lang ";
		musicdb().query(strSql);
	
		strSql = "UPDATE song_click, style SET song_click.styleid = style.id WHERE style.style = song_click.style ";
		musicdb().query(strSql);
...
/*		strSql = "DELETE FROM tmp_topic_lang_style";
	...
	var music_database_cache::update_song_click_stats(bool bVerbose, bool bPartial)
	{
	...
	void music_database_cache::karaoke_update_tmp_song_clickdistinct1(bool bVerbose)
	{
		if(bVerbose)
		{
...
/*		if(bVerbose)
      ...
      string delta_time1;
		...
		string strSql = "DELETE FROM topic_click_compiled_delta";
		musicdb().query(strSql);
...
//		$delta_time2 = datetime_formatDateTime(strtotime("-2 month",time()));
	...
	//	strSql = "INSERT INTO song_click_delta SELECT *, '0000-00-00 00:00:00', 0, 0, 0, 0, 0, 0, 0 FROM song_click";
		...
		strSql = "DELETE FROM tmp_topic_lang_style_click2";
		musicdb().query(strSql);
		if(bVerbose)
		{
			DWORD time2 = ::GetTickCount();
		...
		strSql = "DELETE FROM tmp_topic_lang_style_click";
		musicdb().query(strSql);
		if(bVerbose)
		{
			DWORD time2 = ::GetTickCount();
      ...
      int min_month = row.at(1);
		...
		else	
		{
			...
			strWhere = "WHERE topic_click_compiled.year > '$min_year' OR (topic_click_compiled.year = '$min_year' AND topic_click_compiled.month >= '$min_month')";
		}
      ...
      int min_week = row.at(1);
		...
		else	
		{
			...
			strWhere = "WHERE topic_click_compiled.year >= '$min_year' OR (topic_click_compiled.year = '$min_year' AND topic_click_compiled.week >= '$min_week')";
		}
	...
	void music_database_cache::karaoke_update_tmp_song_clickdistinct1_item(var song_click_id, var topicid, var langid, var styleid, bool bVerbose)
	{
...
void music_database_cache::tmp_performer_update(bool bVerbose)
{
...
/*	strSql = "DROP TABLE temp_table1";
	...
	void music_database_cache::tmp_performer_item_update_classida(var performerid, var classida)
	{
	...
	void music_database_cache::tmp_performer_item_update_chord_classida(var performerid, var classida)
	{
		var classida_text = get_classida_text(classida);
		print (performerid + " --- " + classida_text + "<br>");
	...
	void music_database_cache::karaoke_update_tmp_class(bool bVerbose)
	{
		if(bVerbose)
		{
	...
	var music_database_cache::karaoke_getCodeArray(var classid, var topic, bool bVerbose)
	{
/*		global $musicdb;
		
		$codea = array();
		
		$as = new AlphaSelectPerformer($topic);
		$as->classid = $classid;
		$as->db = &$musicdb;
		
		$fulla = $as->code_getFullArray();
		foreach($fulla as $code)
		{
			if($as->code_getCount($code, bool bVerbose))
			{
				$codea[] = $code;
			}
		}
	...
	var music_database_cache::karaoke_getCodeArrayField(var classid, var topic, bool bVerbose)
	{
		/*$codea = karaoke_getCodeArray($classid, $topic, bVerbose);
		return implode(';', $codea);*/
      return gen::g_emptyconst;
	}

	
	void music_database_cache::tmp_class_update_classida_item(var topictype, var classid, var classidaParam, bool bVerbose)
	{
...
/*		for(int i = 0;each($classida as $id)
	...
	void music_database_cache::tmp_class_update_classida(bool bVerbose)
	{
		if(bVerbose)
		{
			print("<pre>");
		...
		string strSql = "INSERT IGNORE INTO tmp_song (songid) SELECT song.id FROM song";
...
//							tmp_class_update_classida_item($oldtopictype, $class, $parent_class, bVerbose);
	...
	void music_database_cache::tmp_class_2_update_classida(bool bVerbose)
	{
		...
		for(int i = 0; i < rows.array_get_count(); i++)
		{
			var & row = rows.at(i);
			classid		=row.at(0);
			parentclass  = row.at(2);
			parentclassida = x1_decode_ida($parentclass);			
			$classida = array_merge(array($classid), $parentclassida);
			if(bVerbose)
			{
				print("<h4>";
				print("Insert<*2*> class=$classid, parentclass=$parentclass: ";
				print("perfomer({$row[2]}) song({$row[3]}) chord({$row2[4]})";
				print("</h4>";
			}
			
			
			$sql_insert = "INSERT tmp_class_2 (id, classid, topictype, parentclassida, performercount, songcount, codea) VALUES ('$class2id', '$classid', '{$row[1]}', '$parentclass', '{$row[3]}', '{$row[4]}', '{$row[5]}')";
			musicdb().query($sql_insert);
			
			foreach($classida as $_classid)
			{
				strSql = "INSERT IGNORE INTO tmp_class_2_class(class2id, classid) VALUES('$class2id','$_classid') ";
				musicdb().query(strSql);
			}
			
			$class2id++;
			
			$a = array_permute($classida);
			foreach($a as $ida)
			{
				$classid = $ida[0];
				$xa = array_slice($ida, 1);
				$x1_ida = x1_encode_ida($xa);
				if(bVerbose)
				{
					print("<strong>";
				...
				$sql_check = "SELECT COUNT(classid) FROM tmp_class_2 WHERE classid = '$classid' AND parentclassida = '$x1_ida' AND topictype = '{$row[1]}'";
				$result2 = musicdb().query($sql_check);
				$row2 = fonto_fetch_row($result2);
				if(bVerbose)
				{
					print("\n";
					print("COUNTCOUNT Count Count count = ".$row2[0];
					print("\n";
				}
				if($row2[0] == 0)
				{
			...
			$sql_insert = "INSERT tmp_class_2 (id, classid, topictype, parentclassida, performercount, songcount, codea) VALUES ('$class2id', '$classid', '{$row[1]}', '$x1_ida', '{$row[3]}', '{$row[4]}', '{$row[5]}')";
					if(bVerbose)
					{
						print $sql_insert;
					}
	...
	void music_database_cache::karaoke_update_tmp_song_lyricscount(bool bVerbose)
	{
		karaoke_update_tmp_song_topictype_count("lyricscount", 1, bVerbose);
	}
	void music_database_cache::karaoke_update_tmp_song_chordcount(bool bVerbose)
	{
		karaoke_update_tmp_song_topictype_count("chordcount", 2, bVerbose);
	}
	void music_database_cache::karaoke_update_tmp_song_karaokecount(bool bVerbose)
	{
		karaoke_update_tmp_song_topictype_count("karaokecount", 0, bVerbose);
	}

	void music_database_cache::karaoke_update_tmp_song_topictype_count(var tmp_song_field, var topictype, bool bVerbose)
	{
      string strSql;
		DWORD time1 = ::GetTickCount();
		if(bVerbose)
	...
	void music_database_cache::update_lyrics(bool bVerbose)
	{
	...
	void music_database_cache::full_song_update(bool bVerbose)
	{	
	...
	void music_database_cache::on_song_insertion(bool bVerbose)
	{	
	...
	void music_database_cache::on_song_insertion_1(bool bVerbose)
	{
	/*
	...
	void music_database_cache::update_tmp_song_lang_style_rank(bool bVerbose)
	{
      /*string lastdatetime;
      string strWhere;
...
void music_database_cache::tmp_topic_lang_style_update_top( var top_field, var lastdatetime, bool bVerbose)
{
   string strWhere;
		if(strlen(lastdatetime) > 0)
		{
...
void music_database_cache::tmp_topic_lang_style_update_top_item(var topicid, var langid, var styleid, var top_field, var lastdatetime, bool bVerbose)
{
		if(bVerbose)
		{
			print("<table>");
...
var music_database_cache::get_min_datetime_hour_round_song_click()
{
...
var music_database_cache::get_max_datetime_hour_round_song_click()
{
	string strSql = "SELECT CONCAT(SUBSTRING(datetime, 1, 13), ':00:00') FROM topic_click ORDER BY datetime DESC LIMIT 1";
	return musicdb().query_item(strSql);
}

var music_database_cache::get_min_datetime_topic_click_compiled()
{
	string strSql = "SELECT MIN(datetime) FROM topic_click_compiled";
	return musicdb().query_item(strSql);
}

var music_database_cache::get_max_datetime_topic_click_compiled()
{
	string strSql = "SELECT MAX(datetime) FROM topic_click_compiled";
	return musicdb().query_item(strSql);
}

void music_database_cache::update_tmp_topic(bool bVerbose)
{
   DWORD time1;
		if(bVerbose)
		{
...
void music_database_cache::update_tmp_topic_click(bool bVerbose)
{
		if(bVerbose)
		{
			DWORD time1 = ::GetTickCount();
...
var music_database_cache::get_classida_text(var ida)
{
   stringa stra;
   stra =  ida.stra();
	stra.QuickSort();
...
var music_database_cache::debug_delta(var time1, var time2)
{
