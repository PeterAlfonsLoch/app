...
//      ::userbase::edit_plain_text_view::_001OnAfterChangeText();
   ...
   void im_stream_view::_001InstallMessageHandling(::user::win::message::dispatch * pinterface)
   {
   ...
   void im_stream_view::update()
   {
      ...
      {
         m_recStart = System.db().im_post().last(ApplicationUser.m_strFunUserId, m_pimview->m_strRecipient);
      }
      var varList = System.db().im_post().get_since(m_recStart);
      string strHtml;
   ...
   {
      string strPath;
      var rec = System.db().im_post().last(ApplicationUser.m_strFunUserId, m_pimview->m_strRecipient);
      rec.propset(get_app())["user"] = m_pimview->m_strRecipient;
      strPath = "http://i2com.api.veriterse.net/get?"
         + string("sessid=")
         + ApplicationUser.m_strSessid + "&" + rec.propset().get_http_post();
...
//               int iLength = strSender.get_length();
               ...
               if(pnodeBody != NULL)
               {
                  ...
                  if(pnodeMessage->m_strName == "message")
                  {
                     ...
                     System.db().im_post().write(pnodeBody->attrs());
...
//                     post("trying to accept...");
                     int iPort = Application.open_rtprx();
                     if(iPort < 0)
                     {
                     ...
                     strPath = "http://i2com.api.veriterse.net/accept_invitation?"
                       + string("sessid=")
                       + ApplicationUser.m_strSessid 
                       + "&rtprx_port=" + gen::str::itoa(iPort);
                       + "&::fontopus::user=" + strSender;
                     string str = Application.http().get(strPath);
                     //post("failed to open receive channel");
                  }
               }
