...
namespace whiteboard
{
   ...
   void view::URLOpenNew(const char * lpcsz, bool bMakeVisible)
   {
      if(m_phtmlview == NULL)
      {
         m_phtmlview = dynamic_cast < html_view * > (ensure_tab_by_id(ViewHtml)->m_pwnd);
      }
   ...
   void view::MrtsUrlOpenNew(const char * lpcsz, bool bMakeVisible)
   {
      ...
      {
         m_phtmlviewMrts = dynamic_cast < html_view * > (ensure_tab_by_id(ViewMrtsHtml)->m_pwnd);
         if(m_phtmlviewMrts != NULL)
         {
   ...
   void view::MrtsQuerylistUrlOpenNew(const char * lpcsz, bool bMakeVisible)
   {
      ...
      {
         m_phtmlviewMrtsQuerylist = dynamic_cast < html_view * > (ensure_tab_by_id(ViewMrtsQuerylistHtml)->m_pwnd);
         if(m_phtmlviewMrtsQuerylist != NULL)
         {
...
*/
   ...
   void view::on_create_view(view_data * pviewdata)
   {
      ...
      case ViewMrtsQuerylistHtml:
      case ViewUserHour:
         pwnd = create_view(typeid(html_view), get_document(), this, pviewdata->m_id);
         break;
      case ViewOptions:
         pwnd = create_view(typeid(options_view), get_document(), this, pviewdata->m_id);
         break;
      }
      if(pwnd == NULL)
         return;
   ...
   {
      CSingleLock sl(&Application.m_csUserHour, TRUE);
      int iRetry = 10;
   ...
   HICON view::GetIcon(const char * pszMatter)
   {
      ...
      if(!m_mapIcon.Lookup(pszMatter, hicon))
      {
         hicon = (HICON) ::LoadImage(
            NULL, 
            System.dir_matter("whiteboard/notification_area", string(pszMatter) + ".ico"), 
            IMAGE_ICON,
            16, 16,
            LR_LOADFROMFILE);
         ASSERT(hicon != NULL);
