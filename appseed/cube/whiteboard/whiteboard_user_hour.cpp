...
namespace user_hour
{
   ...
   void interval::PulseEvent(whiteboard::view * pview, whiteboard::time & time, e_event eevent)
   {
         ...
         )
      {
       ...
       ::ca::get_thread()->m_papp->m_psystem->log().trace("interval::PulseEvent::Reason Closed: %s", m_varReasonClosed.get_string());
         m_bClosed = true;
         m_timeEnd = time;
      }
      else
...
/*         interval * pinterval = FindInterval(time);
  ...
  //       pinterval->PulseEvent(time, eevent);
   ...
   bool user_hour_set::WriteFile(const char * lpcszFileName)
   {
      ...
      System.file().put_contents(lpcszFileName, doc.get_xml());
