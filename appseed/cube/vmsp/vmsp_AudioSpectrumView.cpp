...
AudioSpectrumView::AudioSpectrumView(::ca::application * papp) :
   ca(papp),
   ::userbase::view(papp)
{
...
// // BEGIN_MESSAGE_MAP(AudioSpectrumView, ::userbase::view)
   //{{AFX_MSG_MAP(AudioSpectrumView)
/*   ON_WM_CREATE()
   ON_WM_SIZE()
   ON_WM_DESTROY()
   ON_WM_CONTEXTMENU()
...
//   ::ca::bitmap *pOldBitmap = graphicsMem->SelectObject(m_pBmpMono);
   ...
   double dFrequency = m_pWaveBuffer->PeakValueFrequency(pWaveFormatEx->nSamplesPerSec, &dMaxAmplitude);
...
//   graphicsMem->FillSolidRect(0, 0, iWidth, iHeight, RGB(0,  0, 0));
   ...
   for(x = 0, i = 0; x < iWidth;  x++, i++)
   {
      ...
      dy = pfData[(int) x * m_pWaveBuffer->m_uiInterestSize / iWidth];
      m_dcBack->MoveTo(x, iHeight);
      m_dcBack->LineTo(x, (int) (iHeight - dy * iHeight));
...
//int AudioSpectrumView::width()
//{
//   return m_iRight - m_iLeft;
//}

//int AudioSpectrumView::height()
//{
//   return m_iBottom - m_iTop;
//}

void AudioSpectrumView::SetPlacement(
...
//void AudioSpectrumView::GetPlacement(LPRECT lprect)
//{
   //lpRect->left = m_iLeft;
...
//int AudioSpectrumView::GetBottom()
//{
//   return m_iBottom;

//}

/*bool AudioSpectrumView::GetVisible()
{
    return m_bVisible;
}*/

/*void AudioSpectrumView::SetVisible(bool bVisible)
{
    m_bVisible = bVisible;
//    if(!m_bVisible)
  //      EraseBackground();
...
VMSRESULT AudioSpectrumView::EraseBackground(::ca::graphics * pdc)
{
...
VMSRESULT AudioSpectrumView::SetDC(::ca::graphics *pdc, ::ca::graphics *pdcBack)
{
    m_pdc = pdc;
    m_pdcBack = pdcBack;
    return VMSR_SUCCESS;
...
void AudioSpectrumView::_001OnCreate(gen::signal_object * pobj) 
{
...
void AudioSpectrumView::_001OnDraw(::ca::graphics *pdc)
{
   to(pdc);
}

void AudioSpectrumView::_001OnSize(gen::signal_object * pobj) 
{
   pobj->previous();
...
void AudioSpectrumView::_001OnDestroy(gen::signal_object * pobj) 
{
...
void AudioSpectrumView::PostNcDestroy() 
{
   // TODO: add your specialized code here and/or call the base class
   
   ::userbase::view::PostNcDestroy();
}

bool AudioSpectrumView::Enable(bool bEnable)
{
   audWaveCentral & wavecentral = Application.GetAudioWaveCentral();
   audWaveIn * pwavein = wavecentral.GetWaveIn();
   if(pwavein == NULL)
   {
      System.simple_message_box(NULL, "Wave recording device is not working", MB_ICONINFORMATION);
      return false;
   }
...
void AudioSpectrumView::on_update(::view* pSender, LPARAM lHint, ::radix::object* pHint) 
{
   if(pHint != NULL)
   {
      if(base < CAudioSpectrumViewUpdateHint >::bases(pHint))
      {
         CAudioSpectrumViewUpdateHint * puh = (CAudioSpectrumViewUpdateHint *) pHint;
         Update(
            puh->m_lpwaveformatex,
            puh->m_dDeltaScore,
            puh->m_dScoreAddUp);
      }
/*      else if(base < CGenericViewUpdateHint >::bases(pHint))
      {
         CGenericViewUpdateHint * puh = (CGenericViewUpdateHint *) pHint;
         if(puh->m_iParamA == 8792)
         {
            m_pWaveBuffer = (audWaveBuffer *) puh->m_pvoid;
         }
      }*/
...
void AudioSpectrumView::OnDraw(::ca::graphics * pdc) 
{
//   TwiRedraw(pdc);
}

void AudioSpectrumView::OnContextMenu(::ca::window* pWnd, point point) 
{
/*   SimpleMenu menu(BaseMenuCentral::GetMenuCentral());
   if (menu.LoadMenu(IDR_POPUP_AUDIOSPECTRUM))
   {
      SimpleMenu* pPopup = (SimpleMenu *) menu.GetSubMenu(0);
      ASSERT(pPopup != NULL);
        simple_frame_window * pframe = dyaGetParentFrame();
...
void AudioSpectrumView::_001OnTimer(gen::signal_object * pobj)
{
...
void AudioSpectrumView::_001OnUserMessage(gen::signal_object * pobj)
{
   SCAST_PTR(::user::win::message::base, pbase, pobj)
   if(pbase->m_wparam == WM_TIMER)
   {
      OnTimerProc(pbase->m_lparam);
   }

}

void AudioSpectrumView::OnTimerProc(UINT nIDEvent)
{
   if(nIDEvent == 1)
   {
//      TwiRedraw();
...
void AudioSpectrumView::TimerProc(UINT uiIDEvent)
{
   SendMessage(WM_USER, WM_TIMER, uiIDEvent);
}


void AudioSpectrumView::WaveInDataProc(audWaveIn *pwavein, DWORD dwSampleTime, LPWAVEHDR lpwavehdr)
{
   m_pWaveBuffer->FFTProcess(lpwavehdr);
}

void AudioSpectrumView::_001InstallMessageHandling(::user::win::message::dispatch * pinterface)
{
   IGUI_WIN_MSG_LINK(WM_CREATE, pinterface, this, &AudioSpectrumView::_001OnCreate);
   IGUI_WIN_MSG_LINK(WM_SIZE, pinterface, this, &AudioSpectrumView::_001OnSize);
   IGUI_WIN_MSG_LINK(WM_USER, pinterface, this, &AudioSpectrumView::_001OnUserMessage);
   IGUI_WIN_MSG_LINK(WM_TIMER, pinterface, this, &AudioSpectrumView::_001OnTimer);
}