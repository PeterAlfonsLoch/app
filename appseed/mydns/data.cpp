...
)
{
	...
	for (origin = fqdn; *origin && !soa; origin++)
		if (origin == fqdn || *origin == '.')
		{
			...
			soa =(MYDNS_SOA *) zone_cache_find(t, 0, NULL, DNS_QTYPE_SOA, origin, end-origin, &errflag, NULL);
...
#endif
	rr = (MYDNS_RR *)zone_cache_find(t, soa->id, soa->origin, type, name, strlen(name), &errflag, soa);
