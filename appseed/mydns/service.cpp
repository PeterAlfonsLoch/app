...
server_status(void)
{
	...
	b += _snprintf(b, sizeof(buf)-(b-buf), "%s ", hostname);
	b += _snprintf(b, sizeof(buf)-(b-buf), "%s %s (%lus) ", _("up"), strsecs(uptime), (unsigned long)uptime);
	b += _snprintf(b, sizeof(buf)-(b-buf), "%lu %s ", requests, _("questions"));
	b += _snprintf(b, sizeof(buf)-(b-buf), "(%.0f/s) ", requests ? AVG(requests, uptime) : 0.0);
	b += _snprintf(b, sizeof(buf)-(b-buf), "NOERROR=%u ", Status.results[DNS_RCODE_NOERROR]);
	b += _snprintf(b, sizeof(buf)-(b-buf), "SERVFAIL=%u ", Status.results[DNS_RCODE_SERVFAIL]);
	b += _snprintf(b, sizeof(buf)-(b-buf), "NXDOMAIN=%u ", Status.results[DNS_RCODE_NXDOMAIN]);
	b += _snprintf(b, sizeof(buf)-(b-buf), "NOTIMP=%u ", Status.results[DNS_RCODE_NOTIMP]);
	b += _snprintf(b, sizeof(buf)-(b-buf), "REFUSED=%u ", Status.results[DNS_RCODE_REFUSED]);

	/* If the server is getting TCP queries, report on the percentage of TCP queries */
	if (Status.tcp_requests)
		b += _snprintf(b, sizeof(buf)-(b-buf), "(%d%% TCP, %lu queries)",
...
//	   return (0);
   ...
   service::service(::ca::application * papp) :
      ...
      plain_service(papp)
   {
      m_papp = dynamic_cast < ::mydns::application * > (papp);
   }
