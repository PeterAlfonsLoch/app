...
)
{
   UNREFERENCED_PARAMETER(priority);
	...
	if (err_file && errappend)
		len = _snprintf(out, sizeof(out) - 2, "%s [%d]: %s: %s", progname, getpid(), msg, strerror(errno));
	else if (err_file)
...
_error_assert_fail(const char *assertion)
{
	...
	_snprintf(msg, sizeof(msg), "assertion failed: %s", assertion);
	...
	const char *fmt, ...)
{
...
#if USE_PGSQL
	{
		...
		_snprintf(out, sizeof(out), "%s: %s (errno=%d)", msg, errmsg, errno);
	}
#else
	_snprintf(out, sizeof(out), "%s: %s (errno=%d)", msg, mysql_error(mysql), errno);
#endif
...
)
{
...
#if USE_PGSQL
	{
		...
		_snprintf(msg, sizeof(msg), "%s (errno=%d)", errmsg, errno);
	}
#else
	_snprintf(msg, sizeof(msg), "%s (errno=%d)", mysql_error(mysql), errno);
#endif
	return (msg);
}
/*--- sql_errmsg() ------------------------------------------------------------------------------*/
