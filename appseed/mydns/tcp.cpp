...
read_tcp_length(TASK *t)
{
	...
	if (!(t->query = (char *) calloc(1, t->len + 1)))
...
read_tcp_query(TASK *t)
{
	...
	return new_task(t, (unsigned char *) t->query, t->len);
}
/*--- read_tcp_query() --------------------------------------------------------------------------*/
...
write_tcp_length(TASK *t)
{
	...
	if ((rv = send(t->fd, len + t->offset, SIZE16 - t->offset, MSG_NOSIGNAL)) < 0)
	{
		if (errno == EINTR)
			return 0;
...
write_tcp_reply(TASK *t)
{
	...
	/* Write the reply */
	if ((rv = send(t->fd, t->reply + t->offset, t->replylen - t->offset, MSG_NOSIGNAL)) < 0)
	{
		if (errno == EINTR)
			return (0);
		dequeue(Tasks, t);
