...
typedef struct _update_query
{
	...
	dns_class_t	dnsclass;											/* The zone's dnsclass */
...
check_update(TASK *t, MYDNS_SOA *soa)
{
	...
	querylen = _snprintf(query, sizeof(query), "SELECT update_acl FROM %s WHERE id=%u%s",
...
update_rrdump(TASK *t, char *section, int which, UQRR *rr)
{
	...
	Debug("%s: DNS UPDATE: >>> %s %d: name=[%s] type=%s dnsclass=%s ttl=%u rdlength=%u rdata=[%s]",
			desctask(t), section, which, rr->name,
			mydns_qtype_str(rr->type), mydns_class_str(rr->dnsclass),
...
update_gobble_rr(TASK *t, MYDNS_SOA *soa, char *query, size_t querylen, char *current, UQRR *rr)
{
	...
	DNS_GET16(dns_qtype_t, rr->type, src);
	DNS_GET16(dns_class_t, rr->dnsclass, src);
	DNS_GET32(rr->ttl, src);
	DNS_GET16(uint16_t, rr->rdlength, src);
...
parse_update_query(TASK *t, MYDNS_SOA *soa, UQ *q)
{
	...
	*/
	...
	DNS_GET16(dns_qtype_t, q->type, src);
	DNS_GET16(dns_class_t, q->dnsclass, src);
#if DEBUG_ENABLED && DEBUG_UPDATE
	Debug("%s:   ZONE: name=[%s]  type=%s  dnsclass=%s", desctask(t), q->name, mydns_qtype_str(q->type), mydns_class_str(q->dnsclass));
#endif
	...
	*/
	q->numPR = t->ancount;
	if (!(q->PR = (UQRR*)calloc(q->numPR, sizeof(UQRR))))
	...
	*/
	q->numUP = t->nscount;
	if (!(q->UP =(UQRR*) calloc(q->numUP, sizeof(UQRR))))
		Err(_("out of primitive::memory"));
	for (n = 0; n < q->numUP; n++)
	...
	*/
	q->numAD = t->arcount;
	if (!(q->AD = (UQRR*)calloc(q->numAD, sizeof(UQRR))))
		Err(_("out of primitive::memory"));
	for (n = 0; n < q->numAD; n++)
	...
	*aux = 0;
	...
	switch (rr->type)
	{
		case DNS_QTYPE_A:
			if (rr->rdlength != 4)
				return -1;
			_snprintf(data, datalen, "%d.%d.%d.%d", rr->rdata[0], rr->rdata[1], rr->rdata[2], rr->rdata[3]);
			return 0;
		...
		case DNS_QTYPE_HINFO:
			{
				...
				_snprintf(data, datalen, "%s%s%s %s%s%s",
		...
		case DNS_QTYPE_RP:
			{
				...
				_snprintf(data, datalen, "%s %s", data1, data2);
			}
			return 0;
		...
		case DNS_QTYPE_SRV:
			{
				...
				DNS_GET16(uint32_t, *aux, src);
				DNS_GET16(uint32_t, weight, src);
				DNS_GET16(uint32_t, port, src);

				if (!(src = name_unencode(t->query, t->len, src, data1, sizeof(data1))))
					return formerr(t, DNS_RCODE_FORMERR, (task_error_t)data1[0], NULL);

				_snprintf(data, datalen, "%u %u %s", weight, port, data1);
			}
			return 0;

		default:
			_snprintf(data, datalen, "Unknown type %s", mydns_qtype_str(rr->type));
			break;
	}
	return (-1);
}
...
update_zone_has_name(TASK *t, MYDNS_SOA *soa, UQ *q, UQRR *rr)
{
   ...
   if (!(xname = (char *)calloc(strlen(rr->name) * 2 + 1, sizeof(char))))
      Err(_("out of primitive::memory"));
	sql_escstr(sql, xname, rr->name, strlen(rr->name));

	querylen = _snprintf(query, sizeof(query),
...
check_prerequisite(TASK *t, MYDNS_SOA *soa, UQ *q, UQRR *rr)
{
	...
	Debug("%s: DNS UPDATE: check_prerequsisite: rr->dnsclass=%s", desctask(t), mydns_class_str(rr->dnsclass));
	Debug("%s: DNS UPDATE: check_prerequsisite: q->dnsclass=%s", desctask(t), mydns_class_str(q->dnsclass));
	...
	if (rr->dnsclass == DNS_CLASS_ANY)
	{
		if (rr->rdlength)
		{
#if DEBUG_ENABLED && DEBUG_UPDATE
			Debug("%s: DNS UPDATE: check_prerequisite failed: dnsclass is ANY but rdlength is nonzero", desctask(t));
#endif
			return dnserror(t, DNS_RCODE_FORMERR, ERR_INVALID_DATA);	
		}
	...
	else if (rr->dnsclass == DNS_CLASS_NONE)
	{
		if (rr->rdlength != 0)
		{
#if DEBUG_ENABLED && DEBUG_UPDATE
			Debug("%s: DNS UPDATE: check_prerequisite failed: dnsclass is NONE but rdlength is zero", desctask(t));
#endif
			return dnserror(t, DNS_RCODE_FORMERR, ERR_INVALID_DATA);	
		}
		if (rr->type == DNS_QTYPE_ANY)
	...
	else if (rr->dnsclass == q->dnsclass)
	{
		...
		{
			if (!q->num_tmprr)
				q->tmprr = (TMPRR**)calloc(1, sizeof(TMPRR *));
			else
				q->tmprr = (TMPRR**)realloc(q->tmprr, sizeof(TMPRR *) * (q->num_tmprr + 1));

			if (!q->tmprr)
				Err(_("out of primitive::memory"));

			/* Add this stuff to the new tmprr */
			if (!(q->tmprr[q->num_tmprr] = (TMPRR*)malloc(sizeof(TMPRR))))
				Err(_("out of primitive::memory"));
...
prescan_update(TASK *t, MYDNS_SOA *soa, UQ *q, UQRR *rr)
{
	...
	{
#if DEBUG_ENABLED && DEBUG_UPDATE
		Debug("%s: DNS UPDATE: prescan_update failed test 1 (check dnsclass)", desctask(t));
#endif
	...
	if ((rr->dnsclass != DNS_CLASS_ANY) && !update_rrtype_ok(rr->type))
	{
#if DEBUG_ENABLED && DEBUG_UPDATE
	...
	if (((rr->dnsclass == DNS_CLASS_ANY) || (rr->dnsclass == DNS_CLASS_NONE)) && (rr->ttl != 0))
	{
#if DEBUG_ENABLED && DEBUG_UPDATE
	...
	/* "For any RR whose CLASS is ANY, check the RDLENGTH to make sure that it is zero (0) (that
		...
		requestor." */
	if ((rr->dnsclass == DNS_CLASS_ANY) && (rr->rdlength != 0))
	{
#if DEBUG_ENABLED && DEBUG_UPDATE
	...
	if ((rr->dnsclass == DNS_CLASS_ANY) && (!update_rrtype_ok(rr->type) && rr->type != DNS_QTYPE_ANY))
	{
#if DEBUG_ENABLED && DEBUG_UPDATE
...
update_add_rr(TASK *t, MYDNS_SOA *soa, UQ *q, UQRR *rr)
{
	...
	Debug("%s: UPDATE_ADD_RR: %s %u %s %s %u %s", desctask(t),
			rr->name, rr->ttl, mydns_class_str(rr->dnsclass), mydns_qtype_str(rr->type), aux, data);
#endif
   ...
   if (!(xdata = (char *) calloc(strlen(data) * 2 + 1, sizeof(char))))
      Err(_("out of primitive::memory"));
	...
	_snprintf(query, sizeof(query), "ADD %s %u IN %s %u %s",
...
update_delete_rrset_all(TASK *t, MYDNS_SOA *soa, UQ *q, UQRR *rr)
{
	...
	Debug("%s: UPDATE_DELETE_RRSET_ALL: %s %u %s %s", desctask(t),
			rr->name, rr->ttl, mydns_class_str(rr->dnsclass), mydns_qtype_str(rr->type));
#endif
   ...
   if (!(xhost = (char *) calloc(strlen(rr->name) * 2 + 1, sizeof(char))))
      Err(_("out of primitive::memory"));
	querylen = _snprintf(query, sizeof(query), "%.*s", strlen(rr->name) - strlen(soa->origin) - 1, rr->name);
	sql_escstr(sql, xhost, query, querylen);

	querylen = _snprintf(query, sizeof(query), "DELETE FROM %s WHERE zone=%u AND (name='%s' OR name='%s')",
	...
	/* Output info to verbose log */
	_snprintf(query, sizeof(query), "DELETE_ALL_RRSET %s", rr->name);
	task_output_info(t, query);
	t->update_done++;
...
update_delete_rr(TASK *t, MYDNS_SOA *soa, UQ *q, UQRR *rr)
{
   ...
   if (!(xdata = (char *) calloc(strlen(data) * 2 + 1, sizeof(char))))
	...
	/* Output info to verbose log */
	_snprintf(query, sizeof(query), "DELETE %s IN %s %s", rr->name, mydns_qtype_str(rr->type), data);
	task_output_info(t, query);
	t->update_done++;
...
update_delete_rrset(TASK *t, MYDNS_SOA *soa, UQ *q, UQRR *rr)
{
   ...
   if (!(xhost = (char *) calloc(strlen(rr->name) * 2 + 1, sizeof(char))))
	...
	/* Output info to verbose log */
	_snprintf(query, sizeof(query), "DELETE %s IN %s", rr->name, mydns_qtype_str(rr->type));
	task_output_info(t, query);
	t->update_done++;
...
{
#if DEBUG_ENABLED && DEBUG_UPDATE
	Debug("%s: DNS UPDATE: process_update: q->name=[%s], q->type=%s, q->dnsclass=%s", desctask(t),
			q->name, mydns_qtype_str(q->type), mydns_class_str(q->dnsclass));
	Debug("%s: DNS UPDATE: process_update: rr->name=[%s], rr->type=%s, rr->dnsclass=%s", desctask(t),
			rr->name, mydns_qtype_str(rr->type), mydns_class_str(rr->dnsclass));
#endif

	/* 2.5.1: Add to an RRset */
	if (rr->dnsclass == q->dnsclass)
	{
#if DEBUG_ENABLED && DEBUG_UPDATE
