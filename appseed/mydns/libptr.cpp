...
**************************************************************************************************/
MYDNS_PTR *
mydns_ptr_dup(MYDNS_PTR * start, int recurse)
{
...
**************************************************************************************************/
inline size_t
mydns_ptr_size(MYDNS_PTR * first)
{
...
mydns_ptr_load(SQL *sqlConn, MYDNS_PTR **rptr, struct in_addr *addr)
{
	...
	while ((row = sql_getrow(res)))
	{
		...
		if (!(ptrnew = mydns_parse_ptr(row)))
		{
			sql_free(res);
			return (-1);
		}
		if (!first) first = ptrnew;
		if (last) last->next = ptrnew;
		last = ptrnew;
	}
