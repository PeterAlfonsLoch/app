...
mydns_set_rr_where_clause(char *where)
{
	...
	{
		if (!(mydns_rr_where_clause = _strdup(where)))
				  ...
				  dns_qtype_t type, char *name, char *origin)
{
   ...
   string query;
		char		namequery[DNS_MAXNAMELEN + DNS_MAXNAMELEN + DNS_MAXNAMELEN + 25] = "";
	...
	if (name)
	{
		...
		{
			if (!name[0])
				_snprintf(namequery, sizeof(namequery), "(name='' OR name='%s')", origin);
			else
			{
#ifdef DN_COLUMN_NAMES
				_snprintf(namequery, sizeof(namequery), "name='%s'", name);
#else
				_snprintf(namequery, sizeof(namequery), "(name='%s' OR name='%s.%s')", name, name, origin);
#endif
			}
		}
		else
   ...
   query.Format("SELECT "MYDNS_RR_FIELDS"%s FROM %s WHERE "
#ifdef DN_COLUMN_NAMES
			"zone_id=%u%s"
#else
			"zone=%u%s"
...
/*	querylen = _snprintf(query, sizeof(query),
	...
	while ((row = sql_getrow(res)))
	{
		...
		if (!(prrNew = mydns_rr_parse(row, origin)))
			continue;
		...
		if (origin && (cp = strstr(prrNew->name, origin)) && !(cp - prrNew->name))
			*cp = '\0';

		if (!first) first = prrNew;
		if (last) last->next = prrNew;
		last = prrNew;
	}
