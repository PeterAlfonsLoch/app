...
sql_open(char *user, char *password, char *host, char *database)
{
	...
	if (PQstatus(sql) == CONNECTION_BAD)
	{
		...
		_snprintf(out, sizeof(out), "%s %s: %s (errno=%d)",
...
sql_istable(SQL *sqlConn, const char *tablename)
{
	...
	if (!(xtablename = (unsigned char *) calloc(strlen(tablename) * 2 + 1, sizeof(unsigned char))))
		Err(_("out of primitive::memory"));
	sql_escstr(sqlConn, ( char *) xtablename, (char *)tablename, strlen(tablename));
...
sql_iscolumn(SQL *sqlConn, const char *tablename, const char *columnname)
{
	...
	if (!(xtablename = (unsigned char  *) calloc(strlen(tablename) * 2 + 1, sizeof(unsigned char))))
		Err(_("out of primitive::memory"));
	if (!(xcolumnname =(unsigned char  *)  calloc(strlen(columnname) * 2 + 1, sizeof(unsigned char))))
		Err(_("out of primitive::memory"));
	sql_escstr(sqlConn, (char *)xtablename, (char *)tablename, strlen(tablename));
	sql_escstr(sqlConn,(char *)  xcolumnname, (char *)columnname, strlen(columnname));
