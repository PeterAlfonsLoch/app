...
new_task(TASK *t, unsigned char *data, size_t len)
{
	...
	src = data;
	DNS_GET16(uint16_t, t->id, src);
	memcpy(&t->hdr, src, SIZE16); src += SIZE16;
	DNS_GET16(uint16_t, t->qdcount, src);
	DNS_GET16(uint16_t, t->ancount, src);
	DNS_GET16(uint16_t, t->nscount, src);
	DNS_GET16(uint16_t, t->arcount, src);

	/* Discard queries where the response bit is set; it might be a spoofed packet
		asking us to talk to ourselves */
	if (t->hdr.qr)
	{
		formerr(t, DNS_RCODE_FORMERR, ERR_RESPONSE_BIT_SET, (char *) "response bit set on query");
	...
	if (t->qdlen <= 0)
		return formerr(t, DNS_RCODE_FORMERR, ERR_MALFORMED_REQUEST, (char *) "question has zero length");
	if (!(t->qd = (unsigned char *) malloc(t->qdlen)))
		Err(_("out of primitive::memory"));
	memcpy(t->qd, src, t->qdlen);
	qdtop = src;

	/* Get query name */
	if (!(src = (unsigned char *) name_unencode((char *) t->qd, t->qdlen, (char *)src, (char *) qname, sizeof(qname))))
	{
		Warnx("%s: FORMERR in query", desctask(t));
		return formerr(t, DNS_RCODE_FORMERR, (task_error_t)qname[0], NULL);
	}
	strncpy(t->qname, (const char *) qname, sizeof(t->qname)-1);
	...
	if (src + SIZE16 > data + len)
		return formerr(t, DNS_RCODE_FORMERR, ERR_MALFORMED_REQUEST, (char *) "query too short; no qtype");
	DNS_GET16(dns_qtype_t, t->qtype, src);
	...
	/* Get query dnsclass */
	if (src + SIZE16 > data + len)
		return formerr(t, DNS_RCODE_FORMERR, ERR_MALFORMED_REQUEST,  (char *)"query too short; no qclass");
	DNS_GET16(dns_class_t, t->qclass, src);

	t->qdlen = src - qdtop;

	/* Request must have at least one question */
	if (!t->qdcount)
		return formerr(t, DNS_RCODE_FORMERR, ERR_NO_QUESTION,  (char *) "query contains no questions");

	/* Server can't handle more than 1 question per packet */
	if (t->qdcount > 1)
		return formerr(t, DNS_RCODE_FORMERR, ERR_MULTI_QUESTIONS, (char *) "query contains more than one question");

	/* Server won't accept truncated query */
	if (t->hdr.tc)
		return formerr(t, DNS_RCODE_FORMERR, ERR_QUESTION_TRUNCATED, (char *) "query is truncated");
	...
	/* Check dnsclass (only IN or ANY are allowed unless status is enabled) */
...
desctask(TASK *t)
{
	...
	_snprintf(desc, sizeof(desc), "%s: %s %s",
...
#if HAVE_IPV6
	if (ptaskNew->family == AF_INET6)
		memcpy(&ptaskNew->addr6, addr, sizeof(struct sockaddr_in6));
	else
#endif
		memcpy(&ptaskNew->addr4, addr, sizeof(struct sockaddr_in));
	ptaskNew->internal_id = Status.udp_requests + Status.tcp_requests;
	ptaskNew->timeout = current_time + task_timeout;
	ptaskNew->minimum_ttl = DNS_MINIMUM_TTL;
	ptaskNew->reply_cache_ok = 1;

#if DEBUG_ENABLED && DEBUG_TASK
	Debug("%s: task_init(%p) from %s:%d", desctask(ptaskNew), ptaskNew, file, line);
#endif

	if (enqueue(Tasks, ptaskNew) < 0)
	{
		task_free(ptaskNew);
		return (NULL);
	}
...
/**************************************************************************************************
	_TASK_FREE
	Free the primitive::memory used by a task.
**************************************************************************************************/
void
...
//	struct tm *tm;
   ...
   memset(&tv, 0, sizeof(tv));
	...
	Verbose(
		...
		t->qname,
		mydns_rcode_str((dns_rcode_t)t->hdr.rcode),
		...
		mydns_opcode_str((dns_opcode_t)t->hdr.opcode),
