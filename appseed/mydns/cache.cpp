...
_cache_init(uint32_t limit, uint32_t expire, const char *desc)
{
	...
	if (!(C = (CACHE *) calloc(1, sizeof(CACHE))))
...
#	error Hash method unknown or unspecified
	...
	if (!(C->nodes = (CNODE **) calloc(C->slots, sizeof(CNODE *))))
		Err(_("out of primitive::memory"));
...
#elif (HASH_TYPE == ADDITIVE_HASH)
{
	...
	for (hash = initval, p = (unsigned char *)buf; p < (unsigned char *)((unsigned char *)buf + buflen); p++)
		hash += *p;
	return (hash % ThisCache->slots);
}
#elif (HASH_TYPE == ROTATING_HASH)
					 ...
					 char *name, size_t namelen, int *errflag, MYDNS_SOA *parent)
{
	...
	*errflag = 0;
			...
			if ((n->namelen == namelen) && (n->zone == zone) && (n->type == type))
			{
				...
				if (!memcmp(n->name, name, namelen))
				{
					...
					if (type == DNS_QTYPE_SOA)
						return (n->data ? mydns_soa_dup((MYDNS_SOA *) n->data, 1) : NULL);
					else
						return (n->data ? mydns_rr_dup((MYDNS_RR *) n->data, 1) : NULL);
				}
			}
		}
	}
	...
	/* Add to cache */
	C->in++;
	if (!(n =(CNODE *)  calloc(1, sizeof(CNODE))))
		Err(_("out of primitive::memory"));
	n->hash = hash;
	n->zone = zone;
	n->type = type;
	...
	for (n = ReplyCache->nodes[hash]; n; n = n->next_node)
	{
		if ((n->namelen == t->qdlen) && (n->type == t->qtype) && (n->protocol == t->protocol))
		{
			...
			if (!memcmp(n->name, t->qd, t->qdlen))
			{
				...
				if (!(t->reply = (char *) malloc(t->replylen)))
					Err(_("out of primitive::memory"));
				p = n->data;

				/* Copy DNS header */
				memcpy(&t->hdr, p, sizeof(DNS_HEADER));
				((unsigned char *&)p) += sizeof(DNS_HEADER);

				/* Copy reason */
				memcpy(&t->reason, p, sizeof(task_error_t));
				((unsigned char *&)p) += sizeof(task_error_t);
				...
				DNS_GET16(uint16_t, t->an.size, p);
				DNS_GET16(uint16_t, t->ns.size, p);
				DNS_GET16(uint16_t, t->ar.size, p);
