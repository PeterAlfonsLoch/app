...
#ifdef WIN32
		   unsigned long ul = 1;
	ioctlsocket(t->recursive_fd, FIONBIO, &ul);
...
recursive_fwd_write(TASK *t)
{
	...
	if (!(query = dns_make_question(t->id, t->qtype, t->qname, 1, &querylen)))
		return dnserror(t, DNS_RCODE_FORMERR, (task_error_t) querylen);
...
recursive_fwd_read(TASK *t)
{
	...
	if ((replylen = recvfrom(t->recursive_fd, (char *) &reply, sizeof(reply), 0,
   ...
   if (!replylen)
	...
	if (!(t->reply = (char *) malloc(replylen)))
	...
	DNS_GET16(uint16_t, id, r);
	memcpy(&hdr, r, SIZE16); r += SIZE16;
	DNS_GET16(uint16_t, qdcount, r);
	DNS_GET16(uint16_t, ancount, r);
	DNS_GET16(uint16_t, nscount, r);
	DNS_GET16(uint16_t, arcount, r);
