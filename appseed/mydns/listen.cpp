...
addrlist_add(int family, void *address, int port)
{
	...
	if (!(A = (ADDRLIST *) calloc(1, sizeof(ADDRLIST))))
...
ip6_extra(const struct in6_addr *a)
{
	...
	*b = '\0';
	...
	if (IN6_IS_ADDR_UNSPECIFIED(a)) b += _snprintf(b, sizeof(buf)-(b-buf), "UNSPECIFIED ");
	if (IN6_IS_ADDR_LOOPBACK(a)) b += _snprintf(b, sizeof(buf)-(b-buf), "LOOPBACK ");
	if (IN6_IS_ADDR_MULTICAST(a)) b += _snprintf(b, sizeof(buf)-(b-buf), "MULTICAST ");
	if (IN6_IS_ADDR_LINKLOCAL(a)) b += _snprintf(b, sizeof(buf)-(b-buf), "LINKLOCAL ");
	if (IN6_IS_ADDR_SITELOCAL(a)) b += _snprintf(b, sizeof(buf)-(b-buf), "SITELOCAL ");
	if (IN6_IS_ADDR_V4MAPPED(a)) b += _snprintf(b, sizeof(buf)-(b-buf), "V4MAPPED ");
	if (IN6_IS_ADDR_V4COMPAT(a)) b += _snprintf(b, sizeof(buf)-(b-buf), "V4COMPAT ");

	if (IN6_IS_ADDR_MULTICAST(a))
	{
		if (IN6_IS_ADDR_MC_NODELOCAL(a)) b += _snprintf(b, sizeof(buf)-(b-buf), "MC_NODELOCAL ");
		if (IN6_IS_ADDR_MC_LINKLOCAL(a)) b += _snprintf(b, sizeof(buf)-(b-buf), "MC_LINKLOCAL ");
		if (IN6_IS_ADDR_MC_SITELOCAL(a)) b += _snprintf(b, sizeof(buf)-(b-buf), "MC_SITELOCAL ");
		if (IN6_IS_ADDR_MC_ORGLOCAL(a)) b += _snprintf(b, sizeof(buf)-(b-buf), "MC_ORGLOCAL ");
		if (IN6_IS_ADDR_MC_GLOBAL(a)) b += _snprintf(b, sizeof(buf)-(b-buf), "MC_GLOBAL ");
	}
...
**************************************************************************************************/
class address_load_class :
   virtual public ip_enum
{
public:
   int m_iPort;
   address_load_class(::ca::application * papp, int port) :
      ca(papp),
      ip_enum(papp)
   {
      m_iPort = port;
...
addrlist_load(int port)
{
   ...
   address_load_class list(::ca::get_thread()->m_papp, port);
   list.enumerate();

/*	for(int i = 0; i < list.m_addra.get_count(); i++)									
	{
		//struct in_addr addr4;
		//struct in6_addr addr6;
...
//		switch (ifr->ifr_addr.sa_family)
	...
	//		case AF_INET:
		//		memcpy(&addr4, &((struct sockaddr_in *)&ifr->ifr_addr)->sin_addr, sizeof(struct in_addr));
         addrlist_add(AF_INET, &list.m_addra[i], port);
				break;
#if HAVE_IPV6
			case AF_INET6:
