...
namespace winactionarea
{
      ...
      m_toolbar(papp)
   {
      ...
      m_pimagelist = NULL;
      m_iFrameData = 10;
      m_dataid = "ca2::winactionarea::frame";
      m_iAnimateStep = 0;
      m_bTimerHide = false;
      m_bHover = false;
   ...
   void frame::_001OnCreate(gen::signal_object * pobj)
   {
      ...
      if(!initialize_message_window("ca2::fontopus::message_wnd::winactionarea"))
      {
      ...
      string str = Application.file().as_string(Application.dir().matter("winactionarea\\toolbar.xml"));
   ...
   {
      UNREFERENCED_PARAMETER(bShowWinActionArea);
      if(!IsWindowVisible() || !m_pimpl->IsWindowVisible())
      {
         Dock();
         show_task_bar(SW_HIDE);
      }
   ...
   void frame::Dock()
   {
      SetTimer(5000, 284, NULL);
      if(m_ppaneview != NULL)
      {
         m_ppaneview->on_before_dock();
      }
      ...
      case position_left:
         ...
         ptHotspot.x = rectDesktop.left;
         ptHotspot.y = rectDesktop.bottom;
         break;
      case position_top:
         SetWindowPos(ZORDER_TOPMOST, 0, 0, rectDesktop.right, 400, SWP_SHOWWINDOW);
         ptHotspot.x = rectDesktop.left;
         ptHotspot.y = rectDesktop.top;
         break;
...
//      SCAST_PTR(::user::win::message::base, pbase, pobj)
   ...
   {
      ::ca::window * pwnd = System.FindWindow("Shell_TrayWnd", "");
      if(pwnd == NULL)
         return;
   ...
   void frame::_000OnDraw(::ca::graphics * pdc)
   {
      ...
      && !System.savings().should_save(gen::resource_display_bandwidth)
      && !System.savings().should_save(gen::resource_memory))
      {
