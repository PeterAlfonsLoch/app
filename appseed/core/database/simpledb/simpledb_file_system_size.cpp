...
file_size_table::file_size_table(sp(::aura::application) papp) :
   element(papp)
{
   m_hmap = NULL;
...
void file_size_table::item::ls(sp(::aura::application) papp, index & iIteration)
{
   if(m_bDir)
   {
      stringa               straPath;
...
file_size_table::item * file_size_table::item::FindItem(sp(::aura::application) papp, const char * pszPath, index & iIteration)
{
   string strName;
...
index file_size_table::item::FindName(sp(::aura::application) papp, const char * pszName, index & iIteration)
{
   if(m_bPendingLs)
   {
      ls(papp, iIteration);
...
void file_size_table::item::update_size(sp(::aura::application) papp, index & iIteration)
{
...
void file_size_table::item::update_size_recursive(sp(::aura::application) papp, index & iIteration)
{
   if(m_bPendingLs)
   {
      ls(papp, iIteration);
...
DBFileSystemSizeSet::DBFileSystemSizeSet(sp(::aura::application) papp) :
...
FileSystemSizeWnd::FileSystemSizeWnd(sp(::aura::application) papp) :
   element(papp),
   ::window_sp(papp)
{
}
...
FileSystemSizeServerThread::FileSystemSizeServerThread(sp(::aura::application) papp) :
   element(papp),
   thread(papp)
{
}
