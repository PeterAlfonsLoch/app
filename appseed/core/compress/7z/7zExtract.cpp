...
namespace n7z
{
   ...
   HRes handler::Extract(const uint32_t *indices, uint32_t numItems,
       int32_t testModeSpec, ::libcompress::archive_extract_callback_interface *extractCallbackSpec)
   {
     bool testMode = (testModeSpec != 0);
     smart_pointer < ::libcompress::archive_extract_callback_interface > extractCallback = extractCallbackSpec;
     ...
     */
     ...
     smart_pointer_array<CExtractFolderInfo> extractFolderInfoVector;
     ...
     for (int32_t i = 0;; i++, totalUnpacked += curUnpacked, totalPacked += curPacked)
     {
       ...
       smart_pointer < ::file::writer > outStream(folderOutStream);
