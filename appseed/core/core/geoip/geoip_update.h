...
int16_t GeoIP_update_database (base_application * papp, char * license_key, int32_t verbose, void (*f)( char *));

/* More generalized update function that works more databases */
int16_t GeoIP_update_database_general (base_application * papp, char * user_id, char * license_key,char * data_base_type, int32_t verbose,char ** client_ipaddr, void (*f)( char *));
