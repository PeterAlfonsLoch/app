  ...
  for(int32_t idx = 0; idx < hrdLocV->get_size(); idx++)
  {
    if (hrdLocV->element_at(idx).has_char())
    {
       ...
       try
       {
          ...
          ::core::byte_stream spfile(Application.file().get_byte_stream(strPath, ::file::mode_read | ::file::type_binary));
...
TextHRDMapper *ParserFactory::createTextMapper(string nameID){
  ...
  for(int32_t idx = 0; idx < hrdLocV->get_size(); idx++)
  {
    if (hrdLocV->element_at(idx).has_char())
    {
       try
       {
          ::core::byte_stream stream(Application.file().get_byte_stream(hrdLocV->element_at(idx), ::file::mode_read |::file::type_binary));
