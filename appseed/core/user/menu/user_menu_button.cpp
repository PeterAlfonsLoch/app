...
namespace user
{
   ...
   menu_button::menu_button(sp(::base::application) papp) :
   ...
   void menu_button::_001DrawCheck(::draw2d::graphics * pdc)
   {
      ...
      if(uiImage != 0xffffffffu)
      {
         ...
         if(m_echeck == check::checked)
         {
            ...
            pdc->Draw3dRect(rectImageBorder, BaseSession.get_default_color(COLOR_3DSHADOW), BaseSession.get_default_color(COLOR_3DHILIGHT));
         }
         pimagelist->draw(
            pdc,
            uiImage,
   ...
   menu_button_cmd_ui::menu_button_cmd_ui(sp(::base::application) papp) :
      element(papp),
      cmd_ui(papp)
   {
   }
