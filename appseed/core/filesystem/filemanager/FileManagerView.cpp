...
void FileManagerAView::on_update(sp(::user::view) pSender, LPARAM lHint, object* phint)
{
...
void FileManagerView::on_update(sp(::user::view) pSender, LPARAM lHint, object* phint)
{
   FileManagerViewInterface::on_update(pSender, lHint, phint);
   if(phint != NULL)
   {
   ...
   //xxx               pframe->WindowDataLoadWindowRect();
        //xxx          pframe->WindowDataEnableSaveWindowRect(true);
            ...
            else if(puh->is_type_of(FileManagerViewUpdateHint::TypeCreateBars))
            {
               ...
               sp(FileManagerChildFrame) pchildframe = ((sp(::user::window)) GetParentFrame());
               if(pchildframe != NULL)
               {
                  ASSERT(pchildframe != NULL);
                  ASSERT(base < FileManagerChildFrame > :: bases(pchildframe));
