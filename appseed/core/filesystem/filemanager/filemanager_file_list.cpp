...
//      int32_t iStepSetSleep = 23;
   ...
   bool file_list::_001CreateImageListStep()
   {
      ...
      if (pcolumn != NULL && pcolumn->m_iSubItem == m_iNameSubItem)
      {
         ...
         item.m_iImage = Bergedge.userex()->shellimageset().GetImage(
            get_handle(),
            item.m_strPath,
   ...
   void file_list::_001InsertColumns()
   {
      ...
      if (get_filemanager_data()->m_bListSelection)
      {
         ...
         {
            column.m_pil = Bergedge.userex()->shellimageset().GetImageList48();
         }
         else
         {
            column.m_pil = Bergedge.userex()->shellimageset().GetImageList16();
         }
         _001AddColumn(column);
      ...
      if (get_filemanager_data()->m_iIconSize >= 48)
      {
         column.m_pilHover = Bergedge.userex()->shellimageset().GetImageList48Hover();
         column.m_pil = Bergedge.userex()->shellimageset().GetImageList48();
      }
      else
      {
