...
//      SCAST_PTR(::user::win::message::create, pcreate, pobj)
      ...
      windesk::frame * pframe = dynamic_cast < windesk::frame * > (GetParentFrame());
      pframe->m_ppaneview = this;

      add_tab("3-action-launch", windesk::PaneViewThreeActionLaunch);
      add_tab("menu", windesk::PaneViewContextMenu);
      add_tab("win desk", windesk::PaneViewWinActionArea);
      add_tab("options", windesk::PaneViewConfiguration);
      add_tab("file manager", windesk::PaneViewFileManager);
   ...
   void pane_view::on_update(::view * pSender, LPARAM lHint, ::radix::object* pHint) 
   {
      ...
      if(lHint == 543218)
      {
         set_cur_tab_by_id(windesk::PaneViewWinActionArea);
      }
      if(pHint != NULL)
      {
...
//               int iTab;
   ...
   void pane_view::on_show_view() 
   {
      ...
      if(get_view_id() == windesk::PaneViewFileManager)
      {
         pframe->m_bAutoHideOnOutClick = false;
         pframe->ShowWindow(SW_MAXIMIZE);
      }
      else if(get_view_id() == windesk::PaneViewContextMenu)
      {
         /*windesk::menu_view * pview = dynamic_cast < windesk::menu_view *  > (get_view());
         ::filemanager::document * pdoc = dynamic_cast < ::filemanager::document * >(pview->get_document());
         pdoc->FileManagerBrowse(Application.dir().userappdata("windesk\\menu"));*/
      }
      else if(get_view_id() == windesk::PaneViewConfiguration)
      {
   /*      ::user::interaction * pui = m_pformOptions->ve_display_bandwidth");
         check_interface * pcheck = dynamic_cast < check_interface * > (puie);
         if(Application.savings().save().is_signalized(gen::save_display_bandwidth))
         {
      ...
      else if(get_view_id() == windesk::PaneViewWinActionArea)
      {
         string strDirName;
   ...
   void pane_view::on_create_view(::user::view_creator_data * pcreatordata)
   {
      ...
      case PaneViewWinActionArea:
         {
            ...
            if(pdoc != NULL)
            {
               ...
               pdoc->get_filemanager_data()->m_strDISection.Format("windesk.pane(%d)", m_iDisplay);
      ...
      case PaneViewThreeActionLaunch:
         {
            ...
            if(pdoc != NULL)
            {
               ...
               pdoc->update_all_views(NULL, 123458);
               ::view * pview = pdoc->get_view();
               string strDir = Application.dir().userappdata("windesk\\3-action-launch");
      ...
      case PaneViewConfiguration:
      {
         ...
         uh.m_strForm = "windesk\\options.xhtml";
...
//         form_child_frame * pframe = dynamic_cast < form_child_frame * >(pcreatordata->m_pwnd);
   ...
   {
      windesk::EPaneView eviewNew;
      switch(get_view_id())
      {
      case windesk::PaneViewContextMenu:
         eviewNew = windesk::PaneViewWinActionArea;
         break;
      case windesk::PaneViewWinActionArea:
         eviewNew = windesk::PaneViewFileManager;
         break;
      case windesk::PaneViewFileManager:
         eviewNew = windesk::PaneViewContextMenu;
         break;
      default:
         eviewNew = windesk::PaneViewWinActionArea;
      }
...
//      SCAST_PTR(::user::win::message::mouse, pmouse, pobj);
      /*if(get_view_id() == windesk::PaneViewWinActionArea)
      {
         ...
         menu.LoadXmlMenu("windesk\\popup_winactionarea.xml");
   ...
   void pane_view::OnFileManagerOpenContextMenu(::filemanager::data * pdata)
   {
	...
	/*void pane_view::OnFileManagerOpenContextMenuFolder(::filemanager::data * pdata, FileManagerItem & item)
   {
   }
*/
