...
/* returns a message id between 1 to poplast() or NULL if bad id */
char* resp=NULL;
char* msg=NULL;
	...
	if(!msg){
#ifdef EBUG
		fprintf(stderr, "retr2msg returned NULL!");
#endif
		msg=resp;
	}else{
		if(resp){
...
char* resp;
	...
	if((!resp) || pop3_error(resp)){
        ...
        free(session->server); session->server=NULL;
		free(session->connection); session->connection=NULL;
