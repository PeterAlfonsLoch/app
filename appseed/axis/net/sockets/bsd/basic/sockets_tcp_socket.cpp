...
void ssl_sigpipe_handle( int x );
   ...
   */
   ...
   bool tcp_socket::open(const ::net::address & ad, const ::net::address & bind_ad,bool skip_socks)
   {
      if (!ad.is_valid())
      {
         log("open", 0, "Invalid ::net::address", ::base::log::level_fatal);
         SetCloseAndDelete();
         return false;
      }
      if (Handler().get_count() >= FD_SETSIZE)
      {
         log("open", 0, "no space left in fd_set", ::base::log::level_fatal);
         SetCloseAndDelete();
         return false;
      }
      SetConnecting(false);
      ...
      if (Handler().PoolEnabled())
      {
         ...
         if (pools)
         {
            ...
            log("SetCallOnConnect", 0, "Found pooled connection", ::base::log::level_info);
            return true;
         }
      }
      ...
      if (!skip_socks && ISNT_ZERO(addrSocks4) && GetSocks4Port())
      {
         ::net::address sa(GetSocks4Host(), GetSocks4Port());
         {
            ...
            log("open", 0, "Connecting to socks4 server @ " + sockshost + ":" + ::str::from(GetSocks4Port()), ::base::log::level_info);
         }
         SetSocks4();
      ...
      if (n == -1)
      {
         ...
         if (Reconnect())
         {
            ...
            log("connect: failed, reconnect pending", iError, StrError(iError), ::base::log::level_info);
            attach(s);
            SetConnecting( true ); // this flag will control fd_set's
         }
         else
         {
            string strError = StrError(iError);
            log("connect: failed", iError, StrError(iError), ::base::log::level_fatal);
            SetCloseAndDelete();
            ::closesocket(s);
            return false;
         }
   ...
   void tcp_socket::OnResolved(int32_t id, const ::net::address & a)
   {
      ...
      if (id == m_resolver_id)
      {
         ...
         {
            log("OnResolved", 0, "Resolver failed", ::base::log::level_fatal);
            SetCloseAndDelete();
         }
      }
      else
      {
         log("OnResolved", id, "Resolver returned wrong job id", ::base::log::level_fatal);
         SetCloseAndDelete();
      }
   }
...
//         n = (int32_t) recv(GetSocket(), buf, nBufSize, SO_NOSIGPIPE);
         ...
         if (n == -1)
         {
            log("recv", Errno, StrError(Errno), ::base::log::level_fatal);
            OnDisconnect();
            SetCloseAndDelete(true);
            SetFlushBeforeClose(false);
            SetLost();
   ...
   ::primitive::memory_size tcp_socket::read(void * buf, ::primitive::memory_size nBufSize)
   {
      ...
      if (n > 0 && n <= nBufSize)
      {
         ...
         if(!m_b_input_buffer_disabled)
         {
            ...
            catch(...)
            {
               ...
               log("tcp_socket::read", 0, "ibuf overflow", ::base::log::level_warning);
      ...
      {
         log("tcp_socket::read", (int32_t) n, "abnormal value from SSL_read", ::base::log::level_error);
      }
   ...
   void tcp_socket::OnWrite()
   {
      if (Connecting())
      {
         ...
         log("tcp: connect failed", err, StrError(err), ::base::log::level_fatal);
      ...
      if (IsSSL())
      {
         ...
         if (n == -1)
         {
            ...
            if ( errnr != SSL_ERROR_WANT_READ && errnr != SSL_ERROR_WANT_WRITE )
            {
               ...
               log("OnWrite/SSL_write", errnr, errbuf, ::base::log::level_fatal);
            }
            return 0;
         }
         else
   ...
   #endif
            {
               log("send", Errno, StrError(Errno), ::base::log::level_fatal);
               OnDisconnect();
               SetCloseAndDelete(true);
               SetFlushBeforeClose(false);
               SetLost();
...
*/
   ...
   void tcp_socket::write(const void * pdata, primitive::memory_size len)
   {
      ...
      if (!Ready() && !Connecting())
      {
         ...
         if (GetSocket() == INVALID_SOCKET)
            log("write", 0, " * GetSocket() == INVALID_SOCKET", ::base::log::level_info);
         if (Connecting())
            log("write", 0, " * Connecting()", ::base::log::level_info);
         if (CloseAndDelete())
            log("write", 0, " * CloseAndDelete()", ::base::log::level_info);
         return;
      }
      if (!IsConnected())
      {
   ...
   {
      Handler().log(this,"OnSocks4ConnectFailed",0,"connection to socks4 server failed, trying direct connection",::base::log::level_warning);
   ...
   bool tcp_socket::OnSocks4Read()
   {
      ...
      case 3:
         if (GetInputLength() > 3)
         {
            ...
            case 90:
               OnConnect();
               log("OnSocks4Read", 0, "Connection established", ::base::log::level_info);
               break;
            case 91:
            case 92:
            case 93:
               Handler().log(this,"OnSocks4Read",m_socks4_cd,"socks4 server reports connect failed",::base::log::level_fatal);
               SetConnecting(false);
               SetCloseAndDelete();
               OnConnectFailed();
               break;
            default:
               Handler().log(this,"OnSocks4Read",m_socks4_cd,"socks4 server unrecognized response",::base::log::level_fatal);
               SetCloseAndDelete();
               break;
            }
         }
   ...
   bool tcp_socket::SSLNegotiate()
   {
      if(!IsSSLServer()) // client
      {
         ...
         if (r > 0)
         {
            ...
            {
               log("SSLNegotiate/cert_common_name_check", 0, "cert_common_name_check failed", ::base::log::level_info);
               SetSSLNegotiate(false);
               SetCloseAndDelete();
               OnSSLConnectFailed();
               return false;
   ...
   //         CheckCertificateChain( "");//ServerHOST);
            ...
            log("SSLNegotiate/SSL_connect", 0, "Connection established", ::base::log::level_info);
            if(m_spsslclientcontext->m_psession == NULL)
            {
         ...
         else if(!r)
         {
            ...
            log("SSLNegotiate/SSL_connect", 0, "Connection failed", ::base::log::level_info);
            SetSSLNegotiate(false);
            SetCloseAndDelete();
            OnSSLConnectFailed();
            skip:;
         ...
         else
         {
            ...
            {
               log("SSLNegotiate/SSL_connect", -1, "Connection failed", ::base::log::level_info);
   ...
   //         CheckCertificateChain( "");//ClientHOST);
            ...
            OnAccept();
            log("SSLNegotiate/SSL_accept", 0, "Connection established", ::base::log::level_info);
            return true;
         }
         else
         if (!r)
         {
            log("SSLNegotiate/SSL_accept", 0, "Connection failed", ::base::log::level_info);
            SetSSLNegotiate(false);
            SetCloseAndDelete();
            OnSSLAcceptFailed();
         }
         else
         {
            ...
            if (r != SSL_ERROR_WANT_READ && r != SSL_ERROR_WANT_WRITE)
            {
               log("SSLNegotiate/SSL_accept", -1, "Connection failed", ::base::log::level_info);
   ...
   {
      log("InitSSLServer", 0, "You MUST implement your own InitSSLServer method", ::base::log::level_fatal);
      SetCloseAndDelete();
   }
   ...
   void tcp_socket::InitializeContext(const string & context,const string & keyfile,const string & password, const SSL_METHOD *meth_in)
   {
      ...
      if(!SSL_CTX_use_certificate_chain_file(m_ssl_ctx, keyfile))
      {
         ...
         {
            log("tcp_socket InitializeContext", 0, "Couldn't read certificate file " + keyfile, ::base::log::level_fatal);
         }
      }
      ...
      {
         log("tcp_socket InitializeContext", 0, "Couldn't read private key file " + keyfile, ::base::log::level_fatal);
      }
   }
   ...
   void tcp_socket::close()
   {
      ...
      if (!Lost() && IsConnected() && !(GetShutdown() & SHUT_WR))
      {
         ...
         {
            // failed...
            log("shutdown", Errno, StrError(Errno), ::base::log::level_error);
         }
      }
      //
      char tmp[1000];
      if (!Lost() && (n = (int32_t) ::recv(GetSocket(),tmp,1000,0)) >= 0)
      {
         if (n)
         {
            log("read() after shutdown", n, "bytes read", ::base::log::level_warning);
         }
      }
   #ifdef HAVE_OPENSSL
      if (IsSSL() && m_ssl)
...
//#endif
  /*       struct sigaction m_saPipe;
   ...
   {
      if (!m_ssl_ctx)
         log("GetSslContext", 0, "SSL Context is NULL; check InitSSLServer/InitSSLClient", ::base::log::level_warning);
      return m_ssl_ctx;
   }
   ...
   {
      if (!m_ssl)
         log("GetSsl", 0, "SSL is NULL; check InitSSLServer/InitSSLClient", ::base::log::level_warning);
      return m_ssl;
   }
   #endif
   ...
   bool tcp_socket::SetTcpNodelay(bool x)
   {
      ...
      {
         log("setsockopt(IPPROTO_TCP, TCP_NODELAY)", Errno, StrError(Errno), ::base::log::level_fatal);
         return false;
      }
      return true;
   #else
      log("socket option not available", 0, "TCP_NODELAY", ::base::log::level_info);
      return false;
   #endif
   }
   ...
   {
      log("connect", -1, "connect timeout", ::base::log::level_fatal);
