...
//      ::DestroyWindow((oswindow) m_spwindowMessage->Detach());
   ...
   */
   ...
   void window_draw::_asynch_redraw()
   {
      if(!m_bProDevianMode)
      {
         m_pthreadimpl->m_spqueue->message_queue_post_message(WM_USER + 5000);
      }
   }
...
/*      if(!m_bProDevianMode && ::IsWindow((oswindow) m_spwindowMessage->get_os_data()))
   ...
   void window_draw::_synch_redraw()
   {
      ...
      //         if(!Session.savings().is_trying_to_save(::ca2::resource_blur_background))
      //         {
      ...
      //            Session.savings().try_to_save(::ca2::resource_blur_background);
      //         }
      //         else if(!Session.savings().is_trying_to_save(::ca2::resource_blurred_text_embossing))
      //         {
      ...
      //            Session.savings().try_to_save(::ca2::resource_blurred_text_embossing);
      //         }
      //         else if(!Session.savings().is_warning(::ca2::resource_processing))
      //         {
      ...
      //            Session.savings().warn(::ca2::resource_processing);
      //         }
      //         else if(!Session.savings().is_trying_to_save(::ca2::resource_processing))
      //         {
...
//
//         //(dynamic_cast < ::android::graphics * >(pdc))->SetViewportOrg(rectWindow.left, rectWindow.top);
//
//
//         if(ptwi != NULL)
//         {
...
//               (WPARAM)((dynamic_cast<::android::graphics * >(pdc))->get_os_data()),
...
////            (HDC)(dynamic_cast<::android::graphics * >(m_pbuffer->GetBuffer()))->get_os_data(),
