...
STDMETHODIMP CHandler::GetArchiveProperty(PROPID propID, PROPVARIANT *value)
{
  ...
  switch(propID)
  {
    ...
    case kpidNumVolumes: prop = (UInt32)_archives.get_count(); break;
    ...
    {
      UInt32 numBlocks = 0;
      for (int i = 0; i < _refItems.get_count(); i++)
        if (!IsSolid(i))
          numBlocks++;
      prop = (UInt32)numBlocks;
      break;
    }
    case kpidError: if (!_errorMessage.is_empty()) prop = _errorMessage; break;
  }
  prop.Detach(value);
  return S_OK;
  COM_TRY_END
...
STDMETHODIMP CHandler::GetProperty(UInt32 index, PROPID propID,  PROPVARIANT *value)
{
  ...
  switch(propID)
  {
    ...
    {
      UString u;
      if (item.HasUnicodeName() && !item.UnicodeName.is_empty())
        u = item.UnicodeName;
      else
    ...
    case kpidSize: prop = item.get_count; break;
...
{
  {
    ...
    for (;;)
    {
      ...
      for (;;)
      {
        ...
        {
          CRefItem refItem;
          refItem.ItemIndex = _items.get_count();
          refItem.NumItems = 1;
          refItem.VolumeIndex = _archives.get_count();
          _refItems.add(refItem);
        }
        _items.add(item);
        if (openCallback && _items.get_count() % 100 == 0)
        {
          UInt64 numFiles = _items.get_count();
    ...
    Int32 testMode, IArchiveExtractCallback *extractCallback)
{
  ...
  if (numItems == 0)
    return S_OK;
  int lastIndex = 0;
  base_array<int> importantIndexes;
  base_array<bool> extractStatuses;
  ...
  for (UInt32 t = 0; t < numItems; t++)
  {
    ...
    const CRefItem &refItem = _refItems[index];
    const CItemEx &item = _items[refItem.ItemIndex];
    censoredTotalUnPacked += item.get_count;
    ...
    for (j = lastIndex; j <= index; j++)
    {
      ...
      importantTotalUnPacked += item.get_count;
  ...
  array_ptr_alloc<CMethodItem> methodItems;

  NCompress::CCopyCoder *copyCoderSpec = new NCompress::CCopyCoder;
  CMyComPtr<ICompressCoder> copyCoder = copyCoderSpec;

  CFilterCoder *filterStreamSpec = new CFilterCoder;
  CMyComPtr<::ex1::reader> filterStream = filterStreamSpec;
  ...
  CMyComPtr<::ex1::reader> folderInStream;
  ...
  for (int i = 0; i < importantIndexes.get_count(); i++,
      ...
      currentImportantTotalPacked += currentPackSize)
  {
    ...
    currentUnPackSize = item.get_count;
    ...
    if (i < importantIndexes.get_count() - 1)
    {
    ...
    */
    ...
    switch(item.Method)
    {
      ...
      case '5':
      {
        ...
        */
        int m;
        for (m = 0; m < methodItems.get_count(); m++)
          if (methodItems[m].RarUnPackVersion == item.UnPackVersion)
            break;
        if (m == methodItems.get_count())
        {
          CMethodItem mi;
    ...
    HRESULT result = commonCoder->Code(inStream, outStream, &packSize, &item.get_count, progress);
    if (item.IsEncrypted())
