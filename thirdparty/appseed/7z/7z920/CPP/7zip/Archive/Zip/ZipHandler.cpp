...
static const method_id kMethodId_ZipBase = 0x040100;
static const method_id kMethodId_BZip2 = 0x040202;
...
static AString BytesToString(const ::ex1::byte_buffer &data)
{
  AString s;
...
HRESULT CLzmaDecoder::Code(::ex1::reader *inStream, ISequentialOutStream *outStream,
    const UInt64 * /* inSize */, const UInt64 *outSize, ICompressProgressInfo *progress)
{
  Byte buf[9];
...
class CZipDecoder
{
  ...
  CMyComPtr<::ex1::reader> filterStream;
    ...
    UInt32 numThreads, Int32 &res)
{
  ...
  {
    HRESULT result = S_OK;
    CMyComPtr<::ex1::reader> inStreamNew;
    if (item.IsEncrypted())
    {
      if (!filterStream)
      {
