...
#define FT_COMPONENT  trace_objs
    ...
    if ( slot )
    {
      ...
      while ( cur )
      {
        if ( cur == slot )
        {
          ...
          if ( slot->m_generic.finalizer )
            slot->m_generic.finalizer( slot );
  ...
  {
    /* finalize client-specific data */
    if ( size->m_generic.finalizer )
      size->m_generic.finalizer( size );
                ...
                FT_Driver  driver )
  {
    ...
    if ( face->m_generic.finalizer )
      face->m_generic.finalizer( face );
...
#define FREETYPE_VER_FIXED  ( ( (FT_Long)FREETYPE_MAJOR << 16 ) | \
                  ...
                  FT_Bool           set )
  {
    FT_Module*           cur;
    FT_Module*           limit;
    FT_Module_Interface  moduleinterface;
    ...
    moduleinterface = cur[0]->clazz->get_interface( cur[0],
    ...
    service = (FT_Service_Properties)moduleinterface;
