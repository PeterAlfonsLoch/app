...
public:
	...
	int_bool isNull() const {
		return (_data == NULL);
	}

	int_bool read(FreeImageIO *io, fi_handle handle, size_t size) {
...
public:
	...
	int_bool isNull() { return _begin == NULL;}
...
*/
static void
loadTrueColor(FIBITMAP* dib, int width, int height, int file_pixel_size, FreeImageIO* io, fi_handle handle, int_bool as24bit) {
...
template <int nBITS>
inline static void
_assignPixel(BYTE* bits, BYTE* val, int_bool as24bit = FALSE) {
...
template <>
inline void
_assignPixel<8>(BYTE* bits, BYTE* val, int_bool as24bit) {
	*bits = *val;
}

template <>
inline void
_assignPixel<16>(BYTE* bits, BYTE* val, int_bool as24bit) {
...
template <>
inline void
_assignPixel<24>(BYTE* bits, BYTE* val, int_bool as24bit) {
...
template <>
inline void
_assignPixel<32>(BYTE* bits, BYTE* val, int_bool as24bit) {
	if (as24bit) {
...
template<int bPP>
static void
loadRLE(FIBITMAP* dib, int width, int height, FreeImageIO* io, fi_handle handle, long eof, int_bool as24bit) {
...
Load(FreeImageIO *io, fi_handle handle, int page, int flags, void *data) {
	...
	try {
		...
		const int_bool header_only =  (flags & FIF_LOAD_NOPIXELS) == FIF_LOAD_NOPIXELS;
		...
		if(isTARGA20(io, handle)) {
			...
			SwapFooter(&footer);
#endif
			int_bool hasExtensionArea = footer.extension_offset > 0;
			if(hasExtensionArea) {
				...
				int_bool hasThumbnail = (postage_stamp_offset > 0) && (postage_stamp_offset < (DWORD)footer_offset);
				if(hasThumbnail) {
...
*/
static inline void
flushPacket(BYTE*& dest, unsigned pixel_size, BYTE* packet_begin, BYTE*& packet, BYTE& packet_count, int_bool& has_rle) {
	if (packet_count) {
