...
{
public:
	FIBITMAP* convert(FIBITMAP *src, int_bool scale_linear);
};

template<class Tsrc> FIBITMAP*
CONVERT_TO_BYTE<Tsrc>::convert(FIBITMAP *src, int_bool scale_linear) {
	FIBITMAP *dst = NULL;
	unsigned x, y;
...
FreeImage_ConvertToStandardType(FIBITMAP *src, int_bool scale_linear) {
	FIBITMAP *dst = NULL;
...
FIBITMAP* DLL_CALLCONV
FreeImage_ConvertToType(FIBITMAP *src, FREE_IMAGE_TYPE dst_type, int_bool scale_linear) {
	FIBITMAP *dst = NULL;
