...
static ParseStatus NewFrame(const MemBuffer* const mem,
  ...
  *frame = (Frame*)memory_calloc(1, sizeof(**frame));
...
static ParseStatus ParseAnimationFrame(
  ...
  if (!added_frame) memory_free(frame);
  return status;
}
...
static ParseStatus ParseFragment(WebPDemuxer* const dmux,
  ...
  if (!added_fragment) memory_free(frame);
  return status;
}
...
static int StoreChunk(WebPDemuxer* const dmux,
  ...
  Chunk* const chunk = (Chunk*)memory_calloc(1, sizeof(*chunk));
...
static ParseStatus ParseSingleImage(WebPDemuxer* const dmux) {
  ...
  if (!image_added) memory_free(frame);
  return status;
}
...
WebPDemuxer* WebPDemuxInternal(const WebPData* data, int allow_partial,
  ...
  dmux = (WebPDemuxer*)memory_calloc(1, sizeof(*dmux));
...
void WebPDemuxDelete(WebPDemuxer* dmux) {
  ...
  for (f = dmux->frames_; f != NULL;) {
    ...
    f = f->next_;
    memory_free(cur_frame);
  }
  for (c = dmux->chunks_; c != NULL;) {
    Chunk* const cur_chunk = c;
    c = c->next_;
    memory_free(cur_chunk);
  }
  memory_free(dmux);
}
