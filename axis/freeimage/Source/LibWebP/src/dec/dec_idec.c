...
static int AppendToMemBuffer(WebPIDecoder* const idec,
  ...
  if (mem->end_ + data_size > mem->buf_size_) {  // Need some memory_free memory
...
static void ClearMemBuffer(MemBuffer* const mem) {
  ...
  if (mem->mode_ == MEM_MODE_APPEND) {
    memory_free(mem->buf_);
    memory_free((void*)mem->part0_buf_);
  }
}
...
static int CopyParts0Data(WebPIDecoder* const idec) {
  ...
  if (mem->mode_ == MEM_MODE_APPEND) {
    ...
    uint8_t* const part0_buf = (uint8_t*)memory_alloc(psize);
    if (part0_buf == NULL) {
      return 0;
    }
...
WebPIDecoder* WebPINewDecoder(WebPDecBuffer* output_buffer) {
  WebPIDecoder* idec = (WebPIDecoder*)memory_calloc(1, sizeof(*idec));
  if (idec == NULL) {
    return NULL;
  }
