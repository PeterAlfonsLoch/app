...
static HashChain* HashChainNew(int size) {
  int i;
  HashChain* const p = (HashChain*)memory_alloc(sizeof(*p));
  if (p == NULL) {
    return NULL;
  }
...
static int BackwardReferencesHashChainDistanceOnly(
  ...
  CostModel* cost_model = (CostModel*)memory_alloc(sizeof(*cost_model));
