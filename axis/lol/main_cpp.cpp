...
extern "C"
int lol_c(char * buffer,unsigned int length);

thread_pointer < string > t_pstring;

int LOL(string & strOut, const string & str)
{
   t_pstring = &strOut;
   unsigned int size = 0;
   ...
   m.get_data()[str.get_length()] = '\0';

   buffer = (char *) m.get_data();

   return lol_c(buffer,m.get_length() -1);
...
extern "C"
void lol_print_string(const char * psz)
{
   t_pstring->operator+=(psz);
}


extern "C"
void lol_put_char(char ch)
{
   t_pstring->operator+=(string(ch));
}