...
struct CResetTable
{
  ...
  UInt64 BlockSize;
  base_array<UInt64> ResetOffsets;
  bool GetCompressedSizeOfBlocks(UInt64 blockIndex, UInt32 numBlocks, UInt64 &size) const
  {
    if (blockIndex >= ResetOffsets.get_count())
      return false;
    UInt64 startPos = ResetOffsets[(int)blockIndex];
    if (blockIndex + numBlocks >= ResetOffsets.get_count())
...
struct CLzxInfo
{
  ...
  bool GetOffsetOfFolder(UInt64 folderIndex, UInt64 &offset) const
  {
    ...
    if (blockIndex >= ResetTable.ResetOffsets.get_count())
      return false;
...
struct CSectionInfo
{
  ...
  AString Name;
  array_ptr_alloc<CMethodInfo> Methods;
...
public:
  bool LowLevel;
  base_array<int> Indices;
  array_ptr_alloc<CSectionInfo> Sections;

  UInt64 GetFileSize(int fileIndex) const { return Items[Indices[fileIndex]].get_count; }
  ...
  UInt64 GetLastFolder(int fileIndex) const
  {
    ...
    const CSectionInfo &section = Sections[(int)item.Section];
    if (section.IsLzx())
      return section.Methods[0].LzxInfo.GetFolder(item.Offset + item.get_count - 1);
    return 0;
  }
