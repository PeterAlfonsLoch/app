...
STDMETHODIMP ::ex1::limited_reader::Read(void *data, UInt32 size, UInt32 *processedSize)
{
...
STDMETHODIMP ::ex1::limited_writer::Write(const void *data, UInt32 size, UInt32 *processedSize)
{
  HRESULT result = S_OK;
  if (processedSize != NULL)
    *processedSize = 0;
