...
void TransportTLSCommon::read(string &xml_payload, const int &timeout)
{
  ...
  while (!done) {
	...
	case SSL_ERROR_NONE:
	  if (total_length == 0) {
	    if (r < (int)TOTAL_LENGTH_SZ) { // Could not read total get_length
	      throw TransportException(TransportException::MISSING_TOTAL_LENGTH,
				       "Missing total get_length of the data unit");
	    }
	    ...
	    if (total_length <= TOTAL_LENGTH_SZ) { // No payload to be read
	      throw TransportException(TransportException::INVALID_TOTAL_LENGTH,
				       "Invalid total get_length of the data unit");
	    }
...
void TransportTLSCommon::write(const char *xml_payload, const int &timeout) 
{ 
  struct timeval to;
  to.tv_sec = timeout;
  to.tv_usec = 0;
  ...
  const uint32_t TOTAL_LENGTH_SZ = 4;
  const uint32_t PAYLOAD_SZ = (uint32_t) utf8_xml_payload.get_length();
...
long TransportTLSCommon::cert_common_name_check(const char *common_name)
{
...
void TransportTLSCommon::setup_context(const char *cert_file,
...
void TransportTLSCommon::set_non_blocking()
{
  ...
  if (fcntl(fd, F_SETFL, flags | O_NONBLOCK) < 0) {
    throw TransportException(TransportException::SET_NONBLOCKING_ERR,
			     "Unable to comparable_array SSL underlying I/O descriptor "
			     "as non-blocking");
  }
}
