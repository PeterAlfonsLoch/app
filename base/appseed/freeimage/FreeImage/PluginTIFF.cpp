...
static FIBITMAP* CreateImageType(int_bool header_only, FREE_IMAGE_TYPE fit, int width, int height, uint16 bitspersample, uint16 samplesperpixel);
...
static int_bool tiff_read_iptc_profile(TIFF *tiff, FIBITMAP *dib);
static int_bool tiff_read_xmp_profile(TIFF *tiff, FIBITMAP *dib);
static int_bool tiff_read_exif_profile(TIFF *tiff, FIBITMAP *dib);
static void ReadMetadata(TIFF *tiff, FIBITMAP *dib);

static int_bool tiff_write_iptc_profile(TIFF *tiff, FIBITMAP *dib);
static int_bool tiff_write_xmp_profile(TIFF *tiff, FIBITMAP *dib);
...
*/
static FIBITMAP*
CreateImageType(int_bool header_only, FREE_IMAGE_TYPE fit, int width, int height, uint16 bitspersample, uint16 samplesperpixel) {
	FIBITMAP *dib = NULL;
...
FindLoadMethod(TIFF *tif, FREE_IMAGE_TYPE image_type, int flags) {
	...
	int_bool bIsTiled = (TIFFIsTiled(tif) == 0) ? FALSE:TRUE;
...
Load(FreeImageIO *io, fi_handle handle, int page, int flags, void *data) {
	...
	try {
		...
		const int_bool asCMYK = (flags & TIFF_CMYK) == TIFF_CMYK;
		...
		if(loadMethod == LoadAsRBGA) {
			...
			int_bool isCMYKA = (photometric == PHOTOMETRIC_SEPARATED) && (samplesperpixel > 4);
...
Save(FreeImageIO *io, FIBITMAP *dib, fi_handle handle, int page, int flags, void *data) {
	...
	const int_bool bHasThumbnail = (FreeImage_GetThumbnail(dib) != NULL);
